...
對於複雜的網站，我們還建議設定[**視覺回歸測試**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)，這是確保網站視覺效果保持不變的好方法。Docusaurus v3 主要升級依賴項，預計不會產生任何視覺變更。

:::

:::note

請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的發布說明，並瀏覽 pull-requests 以獲取更多有用資訊及此處提到的每個變更背後的動機。
...
升級至 MDX 將伴隨 [MDX v2](https://mdxjs.com/blog/v2/) 與 [MDX v3](https://mdxjs.com/blog/v3/) 版本公告中記載的所有破壞性變更。多數變更源自 MDX v2，而 MDX v3 屬於相對小幅的更新。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/)中關於[如何更新 MDX 文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的章節對我們尤其重要。同時請務必閱讀[MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，該頁面能協助您解讀常見的 MDX 錯誤訊息。

請同時查閱我們更新後的[**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx)文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場將成為您的最佳夥伴。它能幫助您理解內容如何**被編譯為 React 元件**，並可隔離排查編譯或渲染問題。
...
並排使用兩個 MDX 遊樂場時，您會很快發現某些內容在 v2 中的編譯結果不同或無法編譯。

:::tip[讓您的內容具備未來兼容性]

目標是重構有問題的內容，使其**能同時兼容兩個 MDX 版本**。如此一來，當您升級至 Docusaurus v3 時，這些內容將可直接無縫運作。

:::

### 使用 MDX 檢查器 CLI

我們提供了 [docusaurus-mdx-checker](https://github.com/slorber/docusaurus-mdx-checker) CLI 工具，可輕鬆標記出有問題的內容。在您的網站上執行此命令，即可獲取無法在 MDX v3 下編譯的文件清單。

```bash
npx docusaurus-mdx-checker
```

對於每個編譯問題，CLI 會記錄文件路徑及需查看的行號。
...
此 CLI 僅為最佳努力工具，且**僅會回報編譯錯誤**。

它不會回報那些不產生錯誤但可能影響內容顯示的細微編譯變更。要捕捉這類問題，我們建議使用[視覺回歸測試](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)。

:::

### 常見 MDX 問題

Docusaurus 無法詳盡記錄 MDX 帶來的所有變更。這屬於 [MDX v2](https://mdxjs.com/migrating/v2/) 與 [MDX v3](https://mdxjs.com/migrating/v3/) 遷移指南的責任範疇。

然而，透過升級數個 Docusaurus 網站，我們注意到大多數問題可歸納為幾種常見案例，我們已為您記錄這些案例。

#### 錯誤使用 `{`

`{` 字元用於開啟 [JavaScript 表達式](https://mdxjs.com/docs/what-is-mdx/#expressions)。若您置於 `{expression}` 中的內容非有效表達式，MDX 現在會直接報錯。
...
使用標準 Markdown 連結，或移除 `<` 和 `>`。MDX 和 GFM 已能自動將字面值轉為連結。
...
以下範例中的 `**`（除了 `` `**` ``）在 Docusaurus 2 中會被正確解析為強調效果，但在 Docusaurus 3 中不再適用。
...
MDX 生態系統中的所有官方套件（Unified、Remark、Rehype...）現在都[僅支援 ES 模組](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)，不再支援 [CommonJS](https://nodejs.org/api/modules.html#modules-commonjs-modules)。

實際上這意味著您不能再使用 `require("remark-plugin")` 這種寫法。

:::tip[升級方法]

Docusaurus v3 現在支援使用 [ES 模組](https://flaviocopes.com/es-modules/)格式的設定檔。我們建議您將設定檔遷移至 ES 模組，這樣可以輕鬆導入 Remark 插件：
...
如果您想繼續使用 CommonJS 模組，可以使用動態導入作為變通方案，這允許您在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用異步函數](/docs/configuration#syntax-to-declare-docusaurus-config)來實現這一點。
...
如果您創建了自定義的 Remark 或 Rehype 插件，由於新的 AST 結構，您可能需要重構這些插件，甚至完全重寫它們。我們創建了一個[專門的支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來幫助插件開發者升級他們的代碼。

:::

### 格式化工具

最常見的格式化工具 Prettier 目前僅支援舊版 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可以在代碼不相容的部分前添加 `{/* prettier-ignore */}` 來使其與 Prettier 兼容。
...
您也可以更新網站的 `package.json` 文件，避免使用舊版不受支援的 Node.js：
...
在升級到 Docusaurus v3 之前，先將 Docusaurus v2 網站升級至 Node.js 18 環境。
...
React 18 自帶了一些破壞性變更，根據您為網站編寫的自訂 React 程式碼量，處理這些變更的難度會有所不同。官方主題和外掛程式已相容 React 18。

:::info[如何升級]

請閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，並檢查您自己的 React 程式碼，找出哪些元件可能會受到此次升級影響。
...
這是包含破壞性變更的新主版本函式庫，我們無法保證嚴格的向後相容性。[`prism-react-renderer` v2 發布說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)並不十分詳盡，但 Docusaurus 使用者需要注意 3 個主要變更。
...
然而，這是一個包含破壞性變更的新主版本庫，我們無法保證嚴格的向後相容性。若遇到問題，請查閱 [v3](https://github.com/FormidableLabs/react-live/releases/tag/v3.0.0) 和 [v4](https://github.com/FormidableLabs/react-live/releases/tag/v4.0.0) 的變更日誌。
...
多數 Docusaurus 使用者無需操作。使用表情符號短代碼的使用者應閱讀[變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md)，並確認表情符號仍能如預期渲染。

> **破壞性變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 更新至 v2。此變更新增了對許多新表情符號的支援，並移除了 GitHub 上不再有效的舊表情符號短代碼。
...
然而，這是一個包含破壞性變更的新主版本庫，我們無法保證嚴格的向後相容性。若遇到問題，請查閱 [v10](https://github.com/mermaid-js/mermaid/releases/tag/v10.0.0) 變更日誌。
...
官方 Docusaurus TypeScript 配置已從外部套件 [`@tsconfig/docusaurus`](https://www.npmjs.com/package/@tsconfig/docusaurus) 重新內化至我們的新單體儲存庫套件 [`@docusaurus/tsconfig`](https://www.npmjs.com/package/@docusaurus/tsconfig)。

此新套件與其他 Docusaurus 核心套件同步版本，將用於確保 TypeScript 的向後相容性及主版本升級時的破壞性變更。
...
Docusaurus v3 將其內部配置載入函式庫從 [`import-fresh`](https://github.com/sindresorhus/import-fresh) 更換為 [`jiti`](https://github.com/unjs/jiti)。該函式庫負責載入如 `docusaurus.config.js`、`sidebars.js` 等檔案及 Docusaurus 插件。
...
然而，其顏色與圖示始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，將其納入文件並修正了顏色與圖示。
...
若您先前使用了未公開的 `:::warning` 提示框，請逐一確認黃色是否為合適的新樣式。若需維持紅色樣式，請改用 `:::danger`。

Docusaurus v3 亦[棄用了 `:::caution`](https://github.com/facebook/docusaurus/pull/9308) 提示框。請將 `:::caution`（黃色）重構為 `:::warning`（黃色）或 `:::danger`（紅色）。
...
此重大變更僅影響 **在 `v2.0.0-beta.10`（2021年12月）前** 就開始版本化文件的早期 Docusaurus v2 使用者。

當建立 `v1.0.0` 版本時，側邊欄檔案包含 `version-v1.0.0/` 前綴，而[Docusaurus v3 不再支援此做法](https://github.com/facebook/docusaurus/pull/9310)。
...
`@docusaurus/plugin-content-blog` 現在預設將 RSS 饋送限制為最近20篇文章。對於大型 Docusaurus 部落格，這是更合理的預設值，可避免 RSS 檔案過度膨脹。
...
對於已替換（swizzle）文件相關主題元件（如 `@theme/DocPage`）的使用者，這些元件已大幅重構以簡化自訂流程。

技術上**這不屬於重大變更**，因這些元件**標記為不建議替換**，但許多 Docusaurus 網站已替換文件相關元件，需注意其自訂內容可能導致 Docusaurus 異常。

:::info[如何升級]

刪除所有已替換的元件，重新替換後，再於更新後的元件上重新套用您的自訂內容。

或可參考[拉取請求說明](https://github.com/facebook/docusaurus/pull/7966)了解新主題元件樹結構，嘗試手動修補已替換的元件。

:::

## 可選變更

部分變更非強制性，但了解後有助於充分發揮 Docusaurus v3 的優勢。
...
當你在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，我們建議使用 `.mdx` 副檔名。這在語義上更為正確，並提高了與外部工具（IDE、格式化工具、linter 等）的兼容性。
...
確保在 Docusaurus v3（使用 MDX v3）中使用 `remark-math 6` 和 `rehype-katex 7`。我們無法保證其他版本能夠正常運作。
...
在 Docusaurus v3 中，`hast-util-is-element` 已不再需要。如果你已安裝它且未在其他地方使用，只需執行 `npm uninstall hast-util-is-element` 即可移除。
...
Docusaurus 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（透過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實現警告，而提供指令標籤的語法需要使用方括號：
...
- 清楚描述問題，避免僅使用模糊的「無法運作」等表述
- 附上完整相關材料：程式碼片段、儲存庫網址、Git 分支連結、完整堆疊追蹤、螢幕截圖與影片
- 以清晰簡潔的方式呈現請求，展現您已盡力協助我們解決問題

您也可以考慮付費請 [Docusaurus 服務供應商](https://github.com/facebook/docusaurus/discussions/9281) 代為升級。若您的專案是開源的，亦可向社群尋求[免費的善意協助](https://github.com/facebook/docusaurus/discussions/9283)。