...
Docusaurus v1 僅在文件內容不同時創建版本化文件。如果你的文件從 v1.0.0 到 v1.1.0 只有 `hello.md` 發生變化，你的文件結構可能看起來像這樣。
...
在 v2 中，你需要填充缺失的 `versioned_docs` 和 `versioned_sidebars`（並確保前置元數據和 ID 引用正確）。
