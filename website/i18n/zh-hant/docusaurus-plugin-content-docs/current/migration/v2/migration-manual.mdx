...
需按以下方式更新 `package.json` 中的 `"scripts"` 區塊：
...
Docusaurus 2 則簡化為 `website/build`，請確保部署配置已更新至正確讀取建置目錄。
...
既有 D1 網站的 README 檔案可修改為反映 D2 變更，或直接替換為 [Docusaurus v2 預設 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數實現主題功能。相關文檔尚未完備，後續會更新說明。要覆蓋 Infima 的 CSS 變數，請建立自訂 CSS 檔案（如 `./src/css/custom.css`）並通過 `@docusaurus/preset-classic` 的選項全局導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 工具為您選擇的主色調尋找不同色階。

或者使用以下工具生成網站所需的各色階變數，並將變數複製到 `src/css/custom.css` 中：
...
您的 Algolia DocSearch v1 配置（位於[此處](https://github.com/algolia/docsearch-configs/blob/master/configs)）需要針對 Docusaurus v2 進行更新（參見[範例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

可聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們能協助更新配置並觸發站點重新爬取以恢復搜索功能（否則需等待最多24小時的下次定期爬取）
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 文件，並填入您的自訂網域。執行建置命令時，`static` 資料夾中的文件將被複製到 `build` 資料夾的根目錄中。
...
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中配置，且預設為啟用。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
如果您在 v1 中設置了 `cleanUrl: false`，可能有人已發布了指向 `/installation.html` 的連結。

為了 SEO 並避免連結失效，您應在託管服務提供商處配置伺服器端重定向規則。
...
舊版本不允許嵌套側邊欄分類，且分類僅能包含文件 ID。而 v2 支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
可將 CommonJS 遷移至 ES6 的 imports/exports 語法。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件中可能有需更新的無效語法。常見範例如：HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合（寫為 `<img/>` 和 `<br/>`）。MDX 文件中的所有標籤都必須是有效的 JSX。
...
部落格的 Docusaurus front matter 欄位命名已從駝峰式改為蛇底式，以與文件規範保持一致。
