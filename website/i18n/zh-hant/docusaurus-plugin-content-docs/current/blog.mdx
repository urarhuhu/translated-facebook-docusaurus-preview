...
在部落格文章中使用 `<!--truncate-->` 標記來表示在瀏覽所有已發佈文章時將顯示的摘要。`<!--truncate-->` 上方的所有內容將成為摘要的一部分。請注意，截斷標記上方的部分必須是可獨立渲染的 Markdown。例如：
...
預設情況下，每個部落格列表頁面會顯示 10 篇文章，但您可以透過插件配置中的 `postsPerPage` 選項控制分頁。如果設定 `postsPerPage: 'ALL'`，將停用分頁功能並在第一頁顯示所有文章。您也可以為部落格列表頁面添加元描述以提升 SEO：
...
此命名慣例為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若需要更精確的發布時間，可使用 front matter。例如，若有多篇文章在同一天發布，可依照當天時間排序：
...
使用 `authors` front matter 欄位來宣告部落格文章作者。作者至少需具備 `name` 或 `image_url`。Docusaurus 會使用 `url`、`email` 和 `title` 等資訊，但也允許其他任何資訊。
...
對於固定的部落格文章作者，在每篇文章中內嵌維護作者資訊可能相當繁瑣。
...
無論是透過 front matter 或作者對映宣告的作者，都需具備名稱或頭像，或兩者皆有。若文章的所有作者皆無名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[饋送生成]

[RSS 饋送](#feed)需設定作者的電子郵件，作者才會出現在饋送中。
...
只有[全域作者](#global-authors)才能啟用此功能。[內聯作者](#inline-authors)不支援此功能。
...
可以定義內聯標籤，或引用預先定義在[`標籤檔案`](api/plugins/plugin-content-blog.mdx#tags-file)中的標籤（選用，通常為`blog/tags.yml`）。

在以下範例中：

- `docusaurus`引用在`blog/tags.yml`中預先定義的標籤鍵
...
`readingTime`回調函數接收以下參數：

- `content`: 部落格內容文字（字串形式）
- `frontMatter`: front matter（以字串鍵及其值組成的記錄）
- `locale`: 當前Docusaurus站點的語言環境
- `defaultReadingTime`: 預設內建的閱讀時間計算函數。它會返回一個數字（閱讀時間分鐘數）或`undefined`（停用此頁面的閱讀時間顯示）。
...
通過配置 `feedOptions` 可生成 RSS/Atom/JSON 訂閱源。預設會生成 RSS 和 Atom 訂閱源。若要禁用訂閱源生成，請將 `feedOptions.type` 設為 `null`。
...
您可以運行不包含獨立首頁的 Docusaurus 站點，將部落格文章列表頁設為首頁。將 `routeBasePath` 設為 `'/'` 可使部落格通過根路徑 `example.com/` 而非子路徑 `example.com/blog/` 訪問。
...
請務必刪除現有的首頁文件 `./src/pages/index.js`，否則會出現兩個文件映射到同一路由的情況！

:::

:::warning

若停用文檔插件，請記得同時刪除配置文件中其他對文檔插件的引用。特別注意需移除導航欄中與文檔相關的項目。

:::

:::tip

對於僅需使用文檔功能的用戶，另有「純文檔模式」可供選擇。詳見[純文檔模式](./guides/docs/docs-introduction.mdx)獲取詳細說明或關於 `routeBasePath` 的更深入解釋。

:::

### 多部落格 {#multiple-blogs}

經典主題預設假設每個網站僅有一個部落格，因此只包含一個部落格插件實例。若需在單一網站運行多個部落格，可通過在 `docusaurus.config.js` 的 `plugins` 選項中配置額外的部落格插件實現。
