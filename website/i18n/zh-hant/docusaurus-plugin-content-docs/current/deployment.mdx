...
您可以將網站部署至靜態網站託管服務，例如 [Vercel](https://vercel.com/)、[GitHub Pages](https://pages.github.com/)、[Netlify](https://www.netlify.com/)、[Render](https://render.com/docs/static-sites) 和 [Surge](https://surge.sh/help/getting-started-with-surge)。
...
在部署至生產環境之前，本地測試建置非常重要。Docusaurus 提供了 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令來實現這一點：
...
將潛在敏感資訊放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是唯一與 Node.js 環境互動的介面（參見[我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接存取 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞至客戶端。
...
- 使用 HTTP 伺服器（如 Apache2 或 Nginx）進行[自託管](#self-hosting)。
...
沒有一個萬全之策。您需要權衡需求和資源後做出選擇。

## 自行託管 {#self-hosting}

Docusaurus 可透過 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 自行託管。使用 `--port` 變更連接埠，並使用 `--host` 變更主機。
...
相較於靜態託管服務/CDN，這並非最佳選擇。

:::

:::warning

在以下章節中，我們將介紹幾種常見的託管服務供應商，以及如何配置它們以最有效率地部署 Docusaurus 網站。Docusaurus 與這些服務並無隸屬關係，此處提供的資訊僅供參考。部分說明由第三方提供，最新的 API 變更可能未反映在我們這邊。若您發現內容過時，歡迎提交 PR。

由於我們僅能基於最大努力原則提供這些內容，我們已停止接受新增託管選項的 PR。不過，您可以將您的說明發布在其他網站（例如您的部落格或供應商的官方網站），並要求我們加入您說明的連結。

:::

## 部署至 Netlify {#deploying-to-netlify}

要將您的 Docusaurus 網站部署至 [Netlify](https://www.netlify.com/)，請先確認以下選項已正確配置：
...
接著，[在 Netlify 上建立您的網站](https://app.netlify.com/start)。
...
若您未配置這些建置選項，仍可在網站建立後前往「網站設定」→「建置與部署」進行設定。

正確配置上述選項後，您的網站應會部署並在合併至部署分支（預設為 `main`）時自動重新部署。

:::warning

部分 Docusaurus 網站將 `docs` 資料夾置於 `website` 之外（多為舊版 Docusaurus v1 網站）：
...
若您決定將 `website` 資料夾設為 Netlify 的基礎目錄，Netlify 在您更新 `docs` 資料夾時不會觸發建置，此時需配置[自訂 `ignore` 指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
建議停用 Netlify 設定「後處理 > 資源優化 > 美化網址」以避免小寫 URL、不必要的重新導向及 404 錯誤。

**請特別注意**：全域核取方塊「停用資源優化」實際上並未真正停用「美化網址」設定。請務必**獨立取消勾選此選項**。

若您希望保留 Netlify 的「美化網址」設定，請相應調整 Docusaurus 的 `trailingSlash` 配置。
...
將 Docusaurus 專案部署至 [Vercel](https://vercel.com/) 可為您帶來[多項優勢](https://vercel.com/)，包括效能與易用性方面的提升。

要透過 [Vercel 的 Git 整合功能](https://vercel.com/docs/concepts/git)部署 Docusaurus 專案，請確保專案已推送至 Git 儲存庫。
...
通常，發布流程會涉及兩個儲存庫（至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務的建置輸出的分支。在以下教程中，它們將分別被稱為「來源」和「部署」。

每個 GitHub 儲存庫都與一個 GitHub Pages 服務相關聯。如果部署儲存庫名為 `my-org/my-project`（其中 `my-org` 是組織名稱或用戶名），則部署的網站將出現在 `https://my-org.github.io/my-project/`。如果部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），則網站將出現在 `https://my-org.github.io/`。

:::info

如果您想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄中的任何內容都會被複製到 `build` 目錄的根目錄以進行部署。使用自訂網域時，您應該可以從 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。
...
我們提供了一個 `docusaurus deploy` 命令，可以幫助您透過一個命令將網站從來源分支部署到部署分支：克隆、建置和提交。
...
GitHub Pages 預設會為 Docusaurus URL 添加尾部斜線。建議設置 `trailingSlash` 配置（`true` 或 `false`，而非 `undefined`）。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 運行發布的檔案。由於 Jekyll 會丟棄任何以 `_` 開頭的檔案，建議您透過在 `static` 目錄中添加一個名為 `.nojekyll` 的空檔案來禁用 Jekyll。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（_來源分支_為 `main`），且您的[發布來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)已設定為[使用自訂 GitHub Actions 工作流程發布](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的 pull request 提交到 `main` 分支時，執行一個確保網站能成功建構（但不實際部署）的動作。此任務將命名為 `test-deploy`。
2. 當 pull request 合併到 `main` 分支或有人直接推送至 `main` 分支時，將建構並部署到 GitHub Pages。此任務將命名為 `deploy`。

以下是兩種使用 GitHub Actions 部署文件的方法。根據您的部署儲存庫位置，選擇下方對應的標籤頁：
...
雖然您可以將兩個任務定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為「已跳過」，這無法反映實際狀態且對審查流程無益。因此，我們建議將它們作為獨立的工作流程來管理。
...
1. 生成一個新的 [SSH 金鑰](https://help.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。由於此 SSH 金鑰將用於 CI 環境，請確保不要輸入任何密碼短語。
...
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其作為 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys) 貼到部署存儲庫中。如果命令行對您無效，請直接複製文件內容。在保存部署金鑰之前，勾選 `允許寫入訪問` 的選項。
4. 您需要將私鑰設為 [GitHub 秘密](https://help.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets)，以允許 Docusaurus 為您運行部署。
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私鑰，並在源存儲庫上創建一個名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令行對您無效，請直接複製文件內容。保存您的秘密。
6. 在 `.github/workflows/` 目錄中創建您的 [文檔工作流程](https://docs.github.com/en/actions/use-cases-and-examples/creating-an-example-workflow)。在此示例中，它是 `deploy.yml` 文件。

此時，您應該具備：

- 源存儲庫已設置 GitHub 工作流程，並將私鑰 SSH 金鑰設為 GitHub 秘密，以及
- 您的部署存儲庫已在 GitHub 部署金鑰中設置公鑰 SSH 金鑰。
...
持續整合（CI）服務通常用於在將新提交檢入源代碼控制時執行例行任務。這些任務可以是運行單元測試和整合測試、自動化構建、將套件發佈到 npm 以及將變更部署到您的網站的任意組合。要自動化網站的部署，您只需在網站更新時調用 `yarn deploy` 腳本。以下部分介紹如何使用流行的持續整合服務提供商 [Travis CI](https://travis-ci.com/) 來實現這一點。
...
3. 打開您的 Travis CI 儀表板。URL 類似於 `https://travis-ci.com/USERNAME/REPO`，並導航到存儲庫的 `更多選項 > 設置 > 環境變量` 部分。
4. 創建一個名為 `GH_TOKEN` 的新環境變量，其值為您新生成的令牌，然後是 `GH_EMAIL`（您的電子郵件地址）和 `GH_NAME`（您的 GitHub 用戶名）。
5. 在存儲庫的根目錄創建一個 `.travis.yml` 文件，內容如下：
...
[Buddy](https://buddy.works/) 是一個易於使用的 CI/CD 工具，允許您自動將門戶部署到不同的環境，包括 GitHub Pages。

按照以下步驟創建一個管道，當您將變更推送到所選項目分支時，自動部署網站的新版本：
...
建立此簡單管線後，每次推送至您選定分支的新提交都會使用 `yarn deploy` 將網站部署至 GitHub Pages。閱讀[本指南](https://buddy.works/guides/react-docusaurus)以瞭解更多關於設定 Docusaurus CI/CD 管線的資訊。
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可直接從您的 Git 儲存庫自動建置並將 Web 應用程式部署至 AWS Fargate。它讓您能完整檢視並進行基礎架構變更，不受傳統 PaaS 的限制。
...
3. 配置建置設定。
   ...
   - **應用程式目錄**指向建置後包含專案檔案的目錄。如果在步驟 2 中選擇了包含網站內容（或 `my_website`）目錄的儲存庫，您可以將其留空。
...
Surge 是一個[靜態網頁託管平台](https://surge.sh/help/getting-started-with-surge)，您可以在幾秒內從命令行部署您的 Docusaurus 專案。將專案部署到 Surge 既簡單又免費（包括自訂網域和 SSL 憑證）。
...
首次使用 Surge 的用戶將被提示從命令行建立帳號（僅此一次）。

確認您要發佈的網站位於 `build` 目錄中。系統會自動生成一個隨機子網域 `*.surge.sh subdomain`（可編輯）。
...
將您的網域儲存在 CNAME 檔案中，以便未來部署使用以下指令：
...
您可以將 Docusaurus 專案部署到 [Stormkit](https://www.stormkit.io)，這是一個用於靜態網站、單頁應用程式（SPA）和無伺服器功能的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項能從程式碼儲存庫直接建置並部署全端網頁應用至 Azure 的服務，簡化了 CI/CD 的開發者體驗。Static Web Apps 將網頁應用的靜態資源與動態 (API) 端點分離。靜態資源由全球分佈的內容伺服器提供，讓客戶端能透過就近的伺服器更快取得檔案。動態 API 則採用基於事件驅動的無伺服器架構進行擴展，這種方式更具成本效益且能按需擴展。依照[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，幾分鐘內即可開始使用。
...
依照我們的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章指引，只需點擊幾下即可開始使用。