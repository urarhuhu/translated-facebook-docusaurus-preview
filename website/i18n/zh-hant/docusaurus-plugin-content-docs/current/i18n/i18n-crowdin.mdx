...
如需協助，請參閱 **[Crowdin 文件](https://support.crowdin.com/)** 和 **[Crowdin 支援](mailto:support@crowdin.com)**。
...
Crowdin 是一個翻譯 SaaS，為開源專案提供[免費方案](https://crowdin.com/page/open-source-project-setup-request)。
...
Crowdin 提供一個 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳原始檔案**和**下載翻譯**，讓您可以自動化翻譯流程。
...
在 `website/i18n/en` 為預設語言產生 JSON 翻譯檔案：
...
使用 `Hide String` 功能確保翻譯人員**不會翻譯不應修改的內容**：
...
先使用**[機器預翻譯](https://support.crowdin.com/pre-translation-via-machine/)**，再**手動修正預翻譯錯誤**（請先於設定中啟用全域翻譯記憶庫）。

建議先使用 `Hide String` 功能，因 Crowdin 的預翻譯機制可能過度樂觀地翻譯不該修改的內容。
...
我們將設定 CI 在**建置時自動下載 Crowdin 翻譯**，並將這些檔案排除在 Git 版本控制外。

將 `website/i18n` 加入 `.gitignore`。

在 CI 環境中設定 `CROWDIN_PERSONAL_TOKEN` 環境變數。

建立 npm 腳本來 `sync` Crowdin（提取來源檔案、上傳來源檔案、下載翻譯）：
...
在 CI 流程中呼叫 `npm run crowdin:sync` 腳本，需在建置 Docusaurus 網站前執行。

:::tip

保持部署預覽速度：特徵分支不需下載翻譯，可使用 `npm run build -- --locale en`。

:::

:::warning

Crowdin 不支援多個並行上傳/下載：建議僅在正式部署時包含翻譯，保持部署預覽為未翻譯狀態。
...
需特別注意 MDX 文件中的 JSX 片段！

:::

Crowdin **官方不支援 MDX**，但已新增 **`.mdx` 副檔名支援**，會將此類檔案視為 Markdown（而非純文字）。

#### MDX 常見問題 {#mdx-problems}

Crowdin 會將 JSX 語法誤判為嵌入式 HTML，可能在下載翻譯時破壞 JSX 標記，導致網站因無效 JSX 而建置失敗。
...
如果您有一個 id 為 `ios` 的文件插件實例，您也需要配置這些原始檔案
...
當您的原始碼重構時，您應該使用 Crowdin UI **手動更新 Crowdin 檔案**：

![Crowdin UI：重新命名檔案](/img/crowdin/crowdin-files-rename.png)

### 版本控制系統 (Git) 整合 {#vcs-git-integrations}

Crowdin 提供多種 VCS 整合，支援 [GitHub](https://support.crowdin.com/github-integration/)、GitLab、Bitbucket。
...
理論上這本應有助於在 Git 和 Crowdin 上同時編輯翻譯，並在兩個系統之間實現**雙向同步**。

實際上，**這並不太可靠**，原因如下：

- Crowdin -> Git 同步運作正常（透過 pull request）
- Git -> Crowdin 同步是手動的（需要按按鈕觸發）
- Crowdin 用於匹配現有 Markdown 翻譯與原始 Markdown 的啟發式方法並非 100% 可靠，每次從 Git 同步後都需在 Crowdin UI 上驗證結果
- 兩名用戶同時在 Git 和 Crowdin 上編輯可能導致翻譯丟失
- 需要將 `crowdin.yml` 檔案放在儲存庫根目錄
...
遺憾的是，由於技術原因目前尚無法使用，但我們有希望解決這個問題。

Crowdin 會將 Markdown 字串替換為技術性 ID（如 `crowdin:id12345`），但替換過程過於激進，包括隱藏字串，並會破壞 front matter、admonitions、JSX...
...
Crowdin 近期對 Markdown 檔案格式進行重大調整，連結的處理方式與過往不同。先前連結被視為標籤，現在則顯示為純文字。由於這些變更，純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，導致翻譯錯誤（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被翻譯為 `Check out [my wonderful page](/my-wonderful-page)`，這會破壞 Docusaurus 的國際化工作流程，因為頁面名稱不應被翻譯）。
