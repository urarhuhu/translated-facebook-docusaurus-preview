...
允許透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化部落格 XML 訂閱源在瀏覽器的顯示效果。

- 設為 `true` 使用內建的 `.xsl` 和 `.css` 檔案樣式
...
Markdown 文件可使用以下被 `---` 包圍的 [前言](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 元資料欄位。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors)，在 Markdown 檔案中可透過 [`authors` 前言](#markdown-front-matter) 以鍵值參照這些作者。
