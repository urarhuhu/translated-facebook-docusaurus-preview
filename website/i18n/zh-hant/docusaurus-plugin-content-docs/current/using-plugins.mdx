...
我們維護了一份[官方插件列表](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您需要新增任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務...請務必查閱該列表：可能已有現成解決方案！
...
最基本的插件使用方式，僅需提供插件名稱或插件路徑即可。

但插件可透過「Babel 風格」的配置方式支援選項設定——在配置中將插件名稱與選項物件包裝成二元組。
...
所有 Docusaurus 內容插件均支援多實例運行。例如，您可能需要[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)。每個插件實例需分配唯一ID，預設插件ID為 `default`。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 與 `plugins` 條目可互換使用。唯一細微差別在於主題在插件之後載入，且[主題可覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析規則](#module-shorthands)，若想利用簡寫功能，請務必使用正確的配置條目！
...
如果名稱有作用域（以 `@` 開頭），則會先以第一個斜線將名稱拆分為作用域與套件名稱：
...
以下是一些範例，適用於在 `plugins` 欄位註冊的外掛程式。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同，這些工具強制要求外掛程式遵循一致的命名慣例，而 Docusaurus 允許更大的命名自由，因此解析方式並非絕對，而是遵循上述優先順序。
