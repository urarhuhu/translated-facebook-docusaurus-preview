...
預設建議將這些資源放在 `static` 資料夾中。**該目錄下的所有檔案**都會被複製到生成 `build` 資料夾的根目錄，並保持目錄層級結構。例如若在 static 資料夾中添加名為 `sun.jpg` 的檔案，它會被複製到 `build/sun.jpg`。
...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並非理想做法，因為變更網站 `baseUrl` 會**導致這些連結失效**。對於在 `https://example.com/test` 提供的圖片 `<img src="/img/docusaurus.png" />`，瀏覽器會嘗試從 URL 根目錄解析，即 `https://example.com/img/docusaurus.png`，這將失敗因為實際提供路徑為 `https://example.com/test/img/docusaurus.png`。
...
在 Markdown 中，**使用 Markdown 語法**撰寫連結或圖片時可以堅持使用絕對路徑，因為 Docusaurus 在解析 Markdown 時會將它們視為 `require` 調用而非 URL。詳見 [Markdown 靜態資源](./guides/markdown-features/markdown-features-assets.mdx)。
...
在 CSS 中，`url()` 函數通常用於引用字型或圖片等資源。要引用靜態資源，請使用絕對路徑：
...
- 撰寫 JSX 時假設您有像 `/test/` 這樣的基礎 URL，因此不使用像 `src="/img/thumbnail.png"` 這樣的絕對 URL 路徑，而是透過 `require` 引用資源。
- 撰寫 Markdown 或 CSS 時假設它是 `/`，因此始終使用不含基礎 URL 的絕對路徑。
...
- 若透過硬編碼的絕對路徑引用缺失檔案，將無法在編譯時被偵測，並導致 404 錯誤。
