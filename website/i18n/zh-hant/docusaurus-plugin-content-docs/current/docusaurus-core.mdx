...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，將渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。
...
相比普通的 `<a>` 標籤，更推薦使用此組件，因為 Docusaurus 在使用 `<Link>` 時會進行許多優化（例如檢測錯誤路徑、預取、應用基礎 URL 等）。
...
插件 ID 僅在單一插件被重複使用於同個站點時有意義。每個插件實例可建立獨立的全域資料。
...
React Hook 用於存取 Docusaurus 的壞鏈結檢查器 API，提供一種方式讓 Docusaurus 頁面回報並收集其鏈結與錨點。
...
僅在**無法使用元件**的**特殊情況**下才需使用此命令式 API，例如：
