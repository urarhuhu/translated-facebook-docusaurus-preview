...
查閱 [**`docusaurus.config.js` API 參考文檔**](api/docusaurus.config.js.mdx) 獲取完整的選項列表。

:::

Docusaurus 對配置有獨特的處理方式。我們鼓勵您將網站相關信息集中到一個地方。我們會保護此文件的字段，並確保整個網站都能方便地訪問這些數據。

維護良好的 `docusaurus.config.js` 可以幫助您、您的協作者和開源貢獻者專注於文檔撰寫，同時仍能自定義網站。

## 聲明 `docusaurus.config.js` 的語法 {#syntax-to-declare-docusaurus-config}

`docusaurus.config.js` 文件在 Node.js 環境中運行，應該導出以下之一：
...
Docusaurus 允許我們以多種**等效方式**聲明配置，以下所有配置示例都會產生完全相同的結果：
...
使用異步配置創建器可以幫助導入僅支持 ESM 的模塊（特別是大多數 Remark 插件）。通過動態導入可以導入這些模塊：
...
它們用於多個地方，例如網站的標題和標題、瀏覽器標籤圖標、社交分享（Facebook、X）信息，甚至用於生成提供靜態文件的正確路徑。
...
Docusaurus 會防止 `docusaurus.config.js` 中出現未知欄位。若要新增自訂欄位，請在 `customFields` 中定義它們。
...
Docusaurus 預設使用 Babel 轉譯網站的原始碼。如果您想自訂 Babel 配置，可以在專案根目錄中建立一個 `babel.config.js` 檔案來實現。
...
大多數情況下，預設的預設集配置都能正常運作。如果您想自訂 Babel 配置（例如新增對 Flow 的支援），可以直接編輯此檔案。為了讓變更生效，您需要重新啟動 Docusaurus 開發伺服器。