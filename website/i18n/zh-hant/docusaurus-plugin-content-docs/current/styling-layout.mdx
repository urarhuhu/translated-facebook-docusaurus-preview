...
根據您的偏好和您嘗試構建的網站類型，有幾種可行的方法/框架。行為更像Web應用程序且具有高度交互性的網站將受益於更現代的樣式設定方法，這些方法將樣式與組件放在一起。當您希望自定義或swizzle組件時，組件樣式設定也可能特別有用。

## 全局樣式 {#global-styles}

這是最傳統的樣式設定方式，大多數開發人員（包括非前端開發人員）都會熟悉。對於沒有太多自定義的小型網站來說，這種方式效果很好。

如果您使用`@docusaurus/preset-classic`，可以創建自己的CSS文件（例如`/src/css/custom.css`），並通過將其作為經典主題的選項來全局導入它們。
...
如果要向任何元素添加CSS，可以在瀏覽器中打開DevTools以檢查其類名。類名有以下幾種：

- **主題類名**。這些類名在[下一小節](#theme-class-names)中詳細列出。它們沒有任何默認屬性。在自定義CSS中，您應始終優先針對這些穩定的類名。
...
- **CSS模塊類名**。這些類名以可能隨時間變化的哈希值結尾（`codeBlockContainer_RIuc`）。它們被視為實現細節，您幾乎應始終避免在自定義CSS中針對它們。如果必須，可以使用忽略哈希值的[屬性選擇器](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors)（`[class*='codeBlockContainer']`）。
...
### 使用Infima設定站點樣式 {#styling-your-site-with-infima}
...
可以透過遵循 `docusaurus-data-<key>` 模式的查詢字串參數來注入 `<html>` 資料屬性。這讓您能夠根據查詢字串靈活地為頁面設定不同的樣式。
...
如果您計劃透過 iframe 將某些 Docusaurus 頁面嵌入到另一個網站中，建立一個替代顯示模式並使用如 `https://mysite.com/docs/myDoc?docusaurus-data-mode=iframe` 的 iframe 網址會很有用。您需要負責提供額外的樣式，並決定要保留或隱藏哪些 UI 元素。
...
某些 React 元件（例如標頭和側邊欄）在行動裝置視圖中會實作不同的 JavaScript 邏輯。如果您在自訂 CSS 中變更了斷點值，可能還需要透過 [swizzling](./swizzling.mdx) 更新 `useWindowSize` 鉤子的呼叫，並傳入明確的選項參數。

:::

## CSS 模組 {#css-modules}

若要使用 [CSS 模組](https://github.com/css-modules/css-modules) 為您的元件設定樣式，請將樣式表檔案命名為 `.module.css` 後綴（例如 `welcome.module.css`）。Webpack 會將此類 CSS 檔案載入為 CSS 模組，您必須將類別名稱作為導入的 CSS 模組的屬性來引用（而不是使用純字串）。這與 [Create React App](https://facebook.github.io/create-react-app/docs/adding-a-css-modules-stylesheet) 中使用的慣例類似。
