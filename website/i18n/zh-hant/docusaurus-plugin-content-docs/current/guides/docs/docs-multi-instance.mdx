...
此功能僅適用於[版本化文檔](./versioning.mdx)。建議在閱讀本頁前先熟悉文檔版本化功能。若您僅需[多側邊欄](./sidebar/multiple-sidebars.mdx)，可在單一插件內實現。
...
若每個文檔實體量龐大，建議創建兩個獨立的 Docusaurus 站點。

當有人編輯 iOS 文檔時，重建所有內容（包括未變更的 Android 文檔）是否真有必要？
...
我們將 `product` 實例視為首要實例，通過不分配 ID 使其成為「默認」實例。
