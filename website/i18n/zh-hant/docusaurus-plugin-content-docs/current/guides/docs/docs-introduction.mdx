...
所有文件將透過子路由 `docs/` 提供。但若**您的站點僅包含文件**，或希望將文件置於根目錄以提升優先級該如何處理？
...
請注意，您**不一定需要放棄使用部落格**或其他插件；`routeBasePath: '/'` 的作用僅是將文件服務路徑從 `https://example.com/docs/some-doc` 變更為站點根目錄：`https://example.com/some-doc`。若啟用部落格，仍可透過 `blog/` 子路由存取。
...
若您已為某文件添加 `slug: /` 使其成為首頁，應刪除現有位於 `./src/pages/index.js` 的首頁，否則將有兩個檔案映射至相同路由！
...
對於僅需使用 Docusaurus 部落格功能的用戶，亦有「純部落格模式」可供選擇。您可採用上述相同方法，參照[純部落格模式](../../blog.mdx#blog-only-mode)的設定說明進行操作。
