...
當您將文件 Y 加入側邊欄 X 時，會建立雙向綁定：側邊欄 X 包含文件 Y 的連結，且瀏覽文件 Y 時會顯示側邊欄 X。但有時我們希望打破這種隱性綁定：

1. _如何在側邊欄 X 中生成文件 Y 的連結，但瀏覽 Y 時不顯示側邊欄 X？_ 例如當文件 Y 同時出現在多個側邊欄時，如何明確指定 Docusaurus 顯示特定側邊欄？
2. _如何讓瀏覽文件 Y 時顯示側邊欄 X，但側邊欄 X 不包含 Y 的連結？_ 例如當 Y 是「文件首頁」且側邊欄僅用於導航時？

Front matter 選項 `displayed_sidebar` 可強制設定側邊欄關聯。沿用相同範例，您仍可使用文件簡寫形式而無需特殊配置：
...
這會明確告知 Docusaurus 瀏覽 `commonDoc` 時顯示 `apiSidebar`。透過相同方法，您也能讓未包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
若設定 `displayed_sidebar: null`，該頁面將完全不顯示任何側邊欄，後續也不會產生分頁導航。

## 生成分頁導航 {#generating-pagination}

Docusaurus 使用側邊欄在每個文件頁面底部生成「下一頁」和「上一頁」分頁連結。它嚴格依據當前顯示的側邊欄：若無關聯側邊欄，則不會生成分頁導航。但被連結為「下一頁」或「上一頁」的文件，不一定會顯示相同側邊欄：它們雖屬於當前側邊欄，但其 front matter 可能設定了不同的 `displayed_sidebar`。

若透過 `displayed_sidebar` front matter 顯示的側邊欄未包含當前文件本身，則不會顯示分頁導航。
...
- 為 `doc2` 設定 `pagination_next: doc5` 並為 `doc5` 設定 `pagination_prev: doc2`（`ref` 在分頁生成過程中被忽略）