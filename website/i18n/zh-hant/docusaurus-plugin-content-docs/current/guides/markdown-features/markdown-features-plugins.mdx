...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警示框語法](./markdown-features-admonitions.mdx)也是透過 Remark 插件生成，您可依此模式實現自訂需求。
...
這些都是 Remark 插件的典型應用案例，也可作為開發自訂插件的參考範例。
...
許多官方 Remark/Rehype 插件僅支援 [ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），Docusaurus 已支援此規範。建議使用 [ES Modules 格式](https://flaviocopes.com/es-modules/)的設定檔，以便更輕鬆導入這類套件。
...
部分插件可接受自訂配置。此時請使用 `[插件名稱, 插件選項]` 語法，例如：
...
舉例來說，我們建立一個外掛程式，為每個 `h2` 標題加上 `Section X. ` 前綴。首先，在任何位置建立外掛程式原始檔——你甚至可以將其發佈為獨立的 npm 套件並如前述方式安裝。我們將外掛程式放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是一個接收 `options` 並返回操作 AST 的 `transformer` 的函式。
...
現在你可以在 `docusaurus.config.js` 中導入此外掛程式，並像使用已安裝的外掛程式一樣使用它！
...
Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題都加上了 `Section X.` 前綴，生成的目錄仍保持不變，因為 TOC 生成外掛程式在自訂外掛程式之前執行。若需在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`：
