...
這個實用的除錯工具能顯示 MDX 編譯器如何將 Markdown 轉換為 React。
...
前置資料位於文件最頂端，以三個破折號 `---` 包圍。內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
您可以重複使用預設解析器，並用自訂的專有邏輯進行包裝。這樣可以實現便利的前置元數據轉換、快捷方式，或是透過 Docusaurus 插件不支援的前置元數據與外部系統整合。
...
建議將 `<summary>` 保持為單行。請注意 [MDX 會為換行符創建額外的 HTML `<p>` 段落](https://mdxjs.com/migrating/v2/#jsx)。如有疑問，可使用 [MDX 遊樂場](https://mdxjs.com/playground/) 來排查 `<details>` 的渲染問題。
