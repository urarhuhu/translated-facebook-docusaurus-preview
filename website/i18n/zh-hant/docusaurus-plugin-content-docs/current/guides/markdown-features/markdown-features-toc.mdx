...
使用 **[`write-heading-ids`](../../cli.mdx#docusaurus-write-heading-ids-sitedir)** CLI 命令可為所有 Markdown 文件批量添加明確的標題 ID。
...
每個 Markdown 文件右上角會顯示目錄。預設情況下，目錄僅顯示 h2 和 h3 級別的標題，這通常足以呈現頁面結構概覽。若需調整顯示的標題層級範圍，您可以針對單一頁面或全局設定最小和最大標題層級。
...
即使已設定全局選項，您仍可透過 front matter 在單一頁面中覆寫這些設定。
...
透過 MDX 的支援，您也可以在 Markdown 文件中直接嵌入行內目錄。
...
請注意 `toc` 是扁平化陣列，因此您可以輕鬆過濾不需要的節點或插入額外節點，重新組織目錄樹狀結構。
...
非 Markdown 標題不會出現在目錄中。此特性可用來解決上述問題。
