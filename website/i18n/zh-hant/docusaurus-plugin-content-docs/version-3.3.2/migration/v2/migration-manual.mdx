...
此手動遷移程序應在[自動遷移程序](./migration-automated.mdx)之後執行，以補齊缺失部分或除錯遷移 CLI 輸出的問題。
...
在 Docusaurus 2 中，我們使用作用域名稱的套件：

- `docusaurus` → `@docusaurus/core`

這提供了官方套件與社群維護套件間的明確區別。換句話說，所有 Docusaurus 官方套件都歸屬於 `@docusaurus/` 命名空間下。

同時，Docusaurus 1 預設提供的文件站台功能現在由 `@docusaurus/preset-classic` 提供。因此，我們也需要添加此相依套件：
...
請使用最新的 Docusaurus 2 版本，您可在此處查看（使用 `latest` 標籤）。
...
在 Docusaurus 1 中，所有建構產出物都位於 `website/build/<PROJECT_NAME>`。

在 Docusaurus 2 中，現在移至 `website/build`。請確保更新您的部署配置，以從正確的 `build` 目錄讀取生成的文件。

如果您要部署到 GitHub 頁面，請確保執行 `yarn deploy` 而非 `yarn publish-gh-pages` 腳本。
...
D1 網站可能已有現有的 README 文件。您可以修改它以反映 D2 的變更，或複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
在 Docusaurus 2 中，我們將每項功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集是插件的組合包，為了向後兼容，我們建立了 `@docusaurus/preset-classic` 預設集，其中捆綁了 Docusaurus 1 中的大多數基本插件。

將以下預設集配置添加到您的 `docusaurus.config.js` 中。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄必須位於 `website` 內。將文檔放在網站目錄中也更為理想，這樣文檔和網站其餘程式碼就能集中在同一個 `website` 目錄下。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，該框架使用 CSS 變數進行主題設定。相關文檔尚未完全就緒，我們將在準備完成後更新此處內容。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
您可以聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 尋求支援。他們可以為您更新配置並觸發重新爬取網站以恢復搜索功能（否則您可能需要等待最多 24 小時才能完成下一次排程爬取）
...
以下欄位均已棄用，您可以從設定檔中移除它們。

- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本控制功能尚未移植。如果您正在使用版本控制，目前無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中設定，且預設為開啟狀態。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
舊版本不允許嵌套側邊欄分類，且分類僅能包含文件 ID。而 v2 版本支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
此操作會將當前主題使用的 `<Footer />` 元件複製到網站根目錄下的 `src/theme/Footer` 資料夾，後續可編輯此元件進行客製化。

請勿僅為在左側添加標誌而替換頁尾元件。v2 版本已刻意移除左側標誌並移至底部，請直接透過 `docusaurus.config.js` 中的 `themeConfig.footer` 配置：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 頁面運作機制。閱讀後請注意：需將 v1 版中的 `pages/en` 檔案移至 `src/pages`。

Docusaurus v1 的頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改從 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本需在每個頁面套用主題佈局元件。Layout 元件接收元數據 props。

v2 已棄用 `CompLibrary`，需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，敬請見諒！期間可檢查 V2 官網或參閱 https://infima.dev/ 查看可用樣式）。
...
Docusaurus 2 採用 [MDX](https://mdxjs.com/) 語法，需修正現有文件中不相容的語法。常見範例如：HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
`authorFBID` 和 `authorTwitter` 欄位已被棄用。這些欄位僅用於產生作者頭像，現在可透過 `authors` 欄位實現相同功能。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動產生，若使用自訂網域，請確保已在 `/static/CNAME` 建立該檔案。

部落格 RSS 訂閱網址現改為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以維持使用者訂閱功能。
