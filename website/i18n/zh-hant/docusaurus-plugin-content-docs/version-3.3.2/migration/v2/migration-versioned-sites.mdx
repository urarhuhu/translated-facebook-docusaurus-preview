...
因為在 v1 中，很可能有人創建了一個新的文件，其前置資料 ID 為 `"version-${version}-${id}"`，這可能會與 `versioned_docs` 的 ID 衝突。
...
Docusaurus v1 僅在文件內容不同時創建版本化文件。如果你的文件結構從 v1.0.0 到 v1.1.0 只有 `hello.md` 發生變化，那麼你的文件結構可能看起來像這樣。
