...
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文件主要是靜態內容，不需繁重的 JS 函式庫或 polyfill
...
- **自動語言檢測**：此功能具主觀性，最好由[伺服器（您的託管服務商）](../deployment.mdx)處理
...
部分插件會讀取需整體本地化的外部資料檔案。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/`路徑下建立副本進行翻譯。
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應資料夾下提供自有翻譯內容，而`code.json`檔案則定義React程式碼中使用的文字標籤。

每個內容插件或主題皆有**自訂的翻譯檔案存放位置**：
