...
這**並非對 Crowdin 作為唯一翻譯 Docusaurus 網站的選擇背書**，但 Facebook 已成功使用它來翻譯如 [Jest](https://jestjs.io/)、[Docusaurus](https://docusaurus.io/) 和 [ReasonML](https://reasonml.github.io/) 等文件專案。
...
[`crowdin.yml` 設定檔](https://support.crowdin.com/configuration-file/) 對 Docusaurus 來說很方便，並允許**將本地化的翻譯文件下載到預期位置**（在 `i18n/[locale]/..` 中）。
...
![建立一個以英文為來源語言、法文為目標語言的 Crowdin 專案](/img/crowdin/crowdin-create-project.png)
...
- 上傳來源檔案（JSON 和 Markdown）的存放位置
- 翻譯完成後下載檔案的目的地（位於 `i18n/[locale]` 目錄下）
...
Crowdin 使用專屬語法定義來源/翻譯路徑：

- `**/*`：子資料夾內所有內容
- `%two_letters_code%`：Crowdin 目標語言的雙字母代碼（本例為 `fr`）
- `**/%original_file_name%`：翻譯檔案將保留原始目錄/檔案結構
...
可在 [個人設定頁面](https://crowdin.com/settings#api-key) 取得 `Personal Access Token`。
...
**切勿提交**此權杖至版本控制，建議為企業建立專屬 **Crowdin 帳號**而非使用個人帳戶。

:::

#### 其他配置欄位 {#other-configuration-fields}

- `project_id`：可硬編碼，位置在 `https://crowdin.com/project/<MY_PROJECT_NAME>/settings#api`
- `preserve_hierarchy`：在 Crowdin 介面中保留文件目錄結構，而非扁平化顯示
...
在本機設定 `CROWDIN_PERSONAL_TOKEN` 環境變數，讓 CLI 能通過 Crowdin API 認證。
...
我們將設定 CI 在建置時**從 Crowdin 下載翻譯檔案**，並將其保留在 Git 之外。

將 `website/i18n` 加入 `.gitignore`。

在 CI 中設定 `CROWDIN_PERSONAL_TOKEN` 環境變數。

建立一個 npm 腳本來 `sync` Crowdin（提取原始檔、上傳原始檔、下載翻譯）：
...
Crowdin 不支援多個同時進行的上傳/下載：建議僅在正式部署時包含翻譯檔案，保持部署預覽未翻譯狀態。
...
Crowdin 會將 JSX 語法視為嵌入式 HTML，可能在下載翻譯時破壞 JSX 標記，導致網站因無效 JSX 而無法建置。

使用簡單字串屬性的 JSX 片段（如 `<Username name="Sebastien"/>`）可以正常運作；但使用物件/陣列屬性的複雜 JSX 片段（如 `<User person={{name: "Sebastien"}}/>`）更容易因語法不像 HTML 而失敗。
...
- 遺憾地會脫離 MDX 工具鏈支援（IDE 語法突顯、Prettier...）

### 文件版本控制 {#docs-versioning}

為 `website/versioned_docs` 資料夾配置翻譯檔案。

建立新版本時，原始字串通常與當前版本 (`website/docs`) 高度相似，您不會希望反覆重新翻譯新版本文檔。

Crowdin 提供 `Duplicate Strings` 設定選項。

![Crowdin 重複字串選項設定](/img/crowdin/crowdin-settings-duplicate-strings.png)

我們建議使用 `Hide`，但最佳設定取決於各版本間的差異程度。

:::warning

不使用 `Hide` 會導致配額中的 `source strings` 數量大幅增加，並影響計費。
...
若您有 `id=ios` 的文件插件實例，則需同時配置這些原始檔案：
...
有時您會在 Git 上**刪除或重新命名原始檔案**，Crowdin CLI 將顯示警告：

![Crowdin CLI：下載翻譯警告](/img/crowdin/crowdin-download-translations-warning.png)

當原始檔重構時，您應使用 Crowdin UI **手動更新 Crowdin 檔案**：
...
理論上這能實現同時在 Git 和 Crowdin 編輯翻譯，並建立兩系統間的**雙向同步**。

實際上**運作效果並不理想**，原因如下：

- Crowdin -> Git 同步運作正常（透過拉取請求）
- Git -> Crowdin 同步需手動觸發（需點擊按鈕）
- Crowdin 用於匹配現有 Markdown 翻譯與原始檔的啟發式方法並非 100% 可靠，同步後需在 Crowdin UI 驗證結果
- 用戶同時在 Git 和 Crowdin 編輯可能導致翻譯遺失
...
遺憾的是，由於技術限制目前尚不可用，但我們認為未來可能解決此問題。

Crowdin 會將 Markdown 字串替換為技術性 ID（如 `crowdin:id12345`），但此替換過於激進，包含隱藏字串，並會破壞 front matter、admonitions、JSX 等結構...
...
您可透過 `editUrl` 函數自訂各語系的編輯網址，將編輯按鈕改為連結至 Crowdin 介面。
...
**Docusaurus 設定檔**是同時使用版本控制與多實例的良好範例：
...
Crowdin 近期對 Markdown 檔案格式進行重大變更，連結的處理方式與過往不同。先前連結被視為標籤，現在則顯示為純文字。此變更導致純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，因而破壞翻譯（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被譯為 `Check out [my wonderful page](/my-wonderful-page)`，這會中斷 docusaurus 的 i18n 工作流程，因頁面名稱不應被翻譯）。

截至 2023 年 12 月 7 日，Crowdin 無意變更連結處理邏輯，若您計劃搭配使用 Crowdin 與 MT，請務必留意此問題。