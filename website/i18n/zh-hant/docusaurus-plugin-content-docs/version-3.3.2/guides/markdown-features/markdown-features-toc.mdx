...
每個標題都有一個可自動生成或手動指定的 ID。標題 ID 允許您在 Markdown 或 JSX 中連結至特定章節：
...
預設情況下，Docusaurus 會根據標題文字自動生成 ID。例如 `### Hello World` 會生成 `hello-world` 作為 ID。
...
每個 Markdown 文件都會在右上角顯示目錄。預設情況下，目錄僅顯示 h2 和 h3 級別的標題，這通常足以呈現頁面結構概覽。若需調整顯示的標題層級範圍，您可以針對單一頁面或全域進行客製化設定。
...
`themeConfig` 選項會套用至網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），而 front matter 選項僅影響右上角的目錄。如需客製化每個 `<TOCInline />` 元件，需使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性。
...
`toc` 變數可在任何 MDX 文件中使用，包含該文件的所有標題資訊。預設僅顯示 `h2` 和 `h3` 級別標題。您可透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示的標題層級範圍。
...
請注意 `toc` 全域變數是扁平化陣列，因此您可以輕鬆過濾不需要的節點或插入額外節點，進而創建新的目錄樹狀結構。
...
目錄是透過 [Remark 插件](./markdown-features-plugins.mdx) 解析 Markdown 原始碼生成的。已知在某些邊緣情況下會產生誤判（false-positives）和漏判（false-negatives）。
