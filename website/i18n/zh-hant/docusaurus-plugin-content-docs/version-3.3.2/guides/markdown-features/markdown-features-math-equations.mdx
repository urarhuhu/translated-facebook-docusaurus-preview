...
從 CDN 來源載入樣式表、字型和 JavaScript 函式庫對於熱門函式庫和資源來說是良好的做法，因為這能減少您需要自行託管的資源量。若您偏好自行託管 `katex.min.css`（連同必要的 KaTeX 字型），您可以從 [KaTeX GitHub 發佈頁面](https://github.com/KaTeX/KaTeX/releases)下載最新版本，解壓縮後將 `katex.min.css` 和 `fonts` 目錄（僅需 `.woff2` 字型類型即可）複製到您網站的 `static` 目錄中，並在 `docusaurus.config.js` 中將樣式表的 `href` 從 CDN 網址替換為本地路徑（例如 `/katex/katex.min.css`）。
