...
Docusaurus 採用 **[Markdown](https://commonmark.org/)** 作為其主要內容撰寫格式。
...
**[MDX](https://mdxjs.com/)** 編譯器會將 **Markdown 文件轉換為 React 元件**，並允許您在 Markdown 內容中使用 JSX。這讓您能輕鬆地在內容中穿插 React 元件，創造愉悅的學習體驗。
...
Docusaurus 使用 MDX 編譯器將 `.md` 和 `.mdx` 文件編譯為 React 元件，但**語法的解釋方式可能因設定而異**。
...
您可以透過 [`siteConfig.markdown.format`](../../api/docusaurus.config.js.mdx#markdown) 設定或 `mdx.format: md` 前置資料**選擇使用 CommonMark**。

:::tip[如何使用 CommonMark]

如果您計劃使用 CommonMark，我們建議使用 [`siteConfig.markdown.format: 'detect'`](../../api/docusaurus.config.js.mdx#markdown) 設定。系統會根據文件副檔名自動選擇適當的格式：
...
Markdown 是一種語法，能讓您以易讀的格式撰寫格式化內容。
...
前置資料用於為您的 Markdown 文件添加元數據。所有內容插件都有各自的前置資料結構，並利用前置資料來豐富從內容或其他配置推斷出的預設元數據。

前置資料位於文件的最頂端，由三個破折號 `---` 包圍。內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
:::tip[增強你的 front matter]

使用 [Markdown 配置中的 `parseFrontMatter` 函數](../../api/docusaurus.config.js.mdx#markdown) 來提供你自己的 front matter 解析器，或是增強預設的解析器。

你可以重複使用預設的解析器，並用自訂的專有邏輯來包裝它。這樣可以實現方便的 front matter 轉換、快捷方式，或是整合外部系統中 Docusaurus 插件不支援的 front matter。
...
你可能會希望將 `<summary>` 保持在一行內。請注意 [MDX 會為換行創建額外的 HTML `<p>` 段落](https://mdxjs.com/migrating/v2/#jsx)。如果有疑問，可以使用 [MDX 遊樂場](https://mdxjs.com/playground/) 來排查 `<details>` 的渲染問題。
