...
您可透過三種方式顯示圖片：Markdown 語法、CommonJS require 或 ES 模組導入語法。
...
使用 ES `import` 語法搭配 JSX 圖片標籤顯示圖片：
...
若您使用 [@docusaurus/plugin-ideal-image](../../api/plugins/plugin-ideal-image.mdx)，需按照文件說明使用專用圖片元件。

:::

## 文件 {#files}

同樣地，您可以透過 `require` 引用現有資源，並將返回的 URL 用於 `video` 標籤、`a` 錨點連結等場景。
...
若使用 Markdown 圖片或連結語法，所有資源路徑都會被 Docusaurus 解析為檔案路徑並自動轉換為 `require()` 調用。除非使用 JSX 語法（需自行處理），否則無需在 Markdown 中手動使用 `require()`。
...
此功能對於需要透過 CSS 修改 SVG 局部樣式非常有用。例如，可根據當前主題變更 SVG 的某部分顏色。
...
Docusaurus 支援主題化圖片：內建主題中的 `ThemedImage` 元件可根據當前主題切換圖片來源。
...
要使用路徑片段切換圖片顯示（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），請在自訂 CSS 中加入以下代碼（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
若 Markdown 連結或圖片使用絕對路徑，該路徑將被視為檔案路徑，並從靜態資源目錄解析。例如，若您設定[靜態資源目錄](../../static-assets.mdx)為 `['public', 'static']`，則以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑下尋找該檔案。連結將被轉換為 `require()` 調用而非保持為 URL 格式。這種處理方式在以下兩個方面具有優勢：

1. 您無需擔心基礎 URL 問題，Docusaurus 會在提供資源時自動處理；
2. 圖片會進入 Webpack 的建置流程，其名稱將附加雜湊值，這使得瀏覽器能積極快取圖片並提升網站效能。
