...
如果您試用 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才會輸出 JSX。MDX 插件也分為兩種形式：
...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警告框語法](./markdown-features-admonitions.mdx)也是透過 Remark 插件生成，您可依相同模式實現自訂需求。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），而 Docusaurus 支援此規範。我們建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 設定檔來簡化這類套件的導入流程。
...
若現有套件無法滿足你的客製化需求，你可以自行建立 MDX 外掛程式。
...
舉例來說，我們建立一個外掛程式，為每個 `h2` 標題加上 `第 X 節. ` 前綴。首先，在任何位置建立外掛原始檔（亦可發佈為獨立 npm 套件並依前述方式安裝）。此範例將檔案置於 `src/remark/section-prefix.js`。remark/rehype 外掛本質上是接收 `options` 並返回能操作 AST 的 `transformer` 函式。
...
現在即可於 `docusaurus.config.js` 導入此外掛，用法與安裝的套件完全相同！
...
`transformer` 的第二參數 [`vfile`](https://github.com/vfile/vfile) 可用於取得當前 Markdown 檔案路徑：
...
Docusaurus 預設外掛會先於自訂 remark 外掛執行，意味著圖片/連結此時已轉換為帶有 `require()` 呼叫的 JSX。例如上方案例中，即便所有 `h2` 標題皆加上 `第 X 節. ` 前綴，產生的目錄仍維持原樣，因為目錄生成外掛執行順序早於自訂外掛。若需在預設外掛前處理 MDAST，請使用 `beforeDefaultRemarkPlugins` 與 `beforeDefaultRehypePlugins`：
