...
Docusaurus 如何知道瀏覽 `commonDoc` 時該顯示哪個側邊欄？答案是：它不知道，我們也無法保證它會選擇哪個側邊欄。

當您將文件 Y 加入側邊欄 X 時，會建立雙向綁定：側邊欄 X 包含指向文件 Y 的連結，且瀏覽文件 Y 時會顯示側邊欄 X。但有時我們希望打破其中一種隱含綁定：

1. _如何在側邊欄 X 中生成指向文件 Y 的連結，而不讓側邊欄 X 在 Y 上顯示？_ 例如，當我在上述範例中將文件 Y 加入多個側邊欄，且我想明確告訴 Docusaurus 顯示某個側邊欄時？
2. _如何讓瀏覽文件 Y 時顯示側邊欄 X，但側邊欄 X 不應包含指向 Y 的連結？_ 例如，當 Y 是「文件首頁」且側邊欄僅用於導覽時？

Front matter 選項 `displayed_sidebar` 會強制設定側邊欄關聯。對於相同範例，您仍可使用文件簡寫而無需任何特殊配置：
...
這明確告訴 Docusaurus 在瀏覽 `commonDoc` 時顯示 `apiSidebar`。使用相同方法，您可以讓不包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
如果您設定 `displayed_sidebar: null`，則該頁面不會顯示任何側邊欄，後續也不會顯示分頁導覽。

## 生成分頁導覽 {#generating-pagination}

Docusaurus 使用側邊欄在每個文件頁面底部生成「下一頁」和「上一頁」分頁連結。它嚴格使用顯示的側邊欄：如果沒有關聯的側邊欄，也不會生成分頁導覽。然而，作為「下一頁」和「上一頁」連結的文件不保證會顯示相同的側邊欄：它們包含在此側邊欄中，但它們的 front matter 可能有不同的 `displayed_sidebar`。

如果透過設定 front matter `displayed_sidebar` 來顯示側邊欄，且此側邊欄不包含文件本身，則不會顯示分頁導覽。
...
「windows」上的下一頁分頁連結指向「linux」，但這不合理：您會希望讀者在安裝後繼續閱讀「getting started」。此時，您可以手動設定分頁：
