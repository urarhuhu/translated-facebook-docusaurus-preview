...
所有文件都將在子路由 `docs/` 下提供。但如果**您的站點僅包含文件**，或您想將文件置於根目錄以優先展示該怎麼辦？
...
若您已為某個文件添加 `slug: /` 使其成為首頁，應刪除現有的 `./src/pages/index.js` 首頁，否則會出現兩個檔案映射至相同路由的情況！
...
對於僅想使用 Docusaurus 部落格功能的用戶，亦有「純部落格模式」。您可使用上述相同方法。請遵循[純部落格模式](../../blog.mdx#blog-only-mode)的設定說明。
