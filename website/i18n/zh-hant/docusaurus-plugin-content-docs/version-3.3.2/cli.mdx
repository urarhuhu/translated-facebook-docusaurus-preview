...
專案初始化後，網站原始碼將包含可透過套件管理器調用的 Docusaurus 腳本：
...
當從遠端伺服器或虛擬機（如 GitHub Codespaces）轉發 3000 端口時，可指定 `0.0.0.0` 讓開發伺服器監聽本地 IP。
