...
此插件在開發環境始終處於非活躍狀態，**僅在生產環境生效**，因為其運作依賴於構建輸出結果。
...
本插件會同時讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置項，當多個文件將被輸出至相同路徑時，據此調整日誌級別。
...
因此同一個"目標"可對應多個"來源"：我們會創建多個重定向至同一目標的HTML文件。但一個"來源"絕不能對應多個"目標"：生成的HTML文件必須有確定的重定向終點。
