...
僅安裝 service worker 不足以讓您的應用成為 PWA。您至少需要包含 [Web 應用程式清單](https://developer.mozilla.org/zh-TW/docs/Web/Manifest)，並在 `<head>` 中加入正確標籤（參見[選項 > pwaHead](#pwahead)）。
...
![應用程式安裝流程畫面錄製。瀏覽器網址列出現安裝按鈕，點擊後顯示「安裝此應用程式？」對話框。點擊「安裝」按鈕後，作業系統會開啟新應用程式並顯示 Docusaurus 首頁。](/img/pwa_install.gif)
...
[workbox-precaching](https://developers.google.com/web/tools/workbox/modules/workbox-precaching) 頁面說明核心概念：

> service worker 的功能之一是在安裝時將一組檔案保存至快取，此過程常稱為「預先快取」，因為您是在 service worker 啟用前就完成快取。
>
> 主要目的是讓開發者能控制快取機制，決定檔案何時與多久被快取，並能直接提供給瀏覽器而無需連網，藉此建立可離線運作的網路應用程式。
>
> Workbox 透過簡化 API 與確保資源高效下載，大幅降低預先快取的實作難度。

預設情況下，當網站以應用程式形式安裝時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

網站完成預先快取後，service worker 會為後續訪問提供快取回應。當新版本部署並伴隨新版 service worker 時，新版本會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗要求使用者更新內容。在使用者清除應用程式快取或點擊彈窗中的 `reload` 按鈕前，service worker 會持續提供舊版內容。
...
適用於額外的 Workbox 規則。您可以在這裡執行任何 service worker 能做的事，並充分利用 workbox 函式庫的功能。程式碼會被轉譯，因此您可以使用現代 ES6+ 語法。
...
在 Docusaurus 應用之前添加一個入口，以便註冊可以在應用運行前完成。預設的 `registerSW.js` 檔案足以進行簡單的註冊。
...
當新的 Service Worker 等待安裝時，系統會渲染 `@theme/PwaReloadPopup` 元件並建議使用者重新載入。您可以[替換](../../swizzling.mdx)此元件並實作自己的 UI 介面。該元件會接收一個 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時觸發，此操作將指示 Service Worker 安裝等待中的版本並重新載入頁面。
