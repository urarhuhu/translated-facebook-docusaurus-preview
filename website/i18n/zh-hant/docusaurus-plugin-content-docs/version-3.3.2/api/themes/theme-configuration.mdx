...
當需要在網站發布公告時，可透過公告欄實現。此為非固定式且可關閉的面板，位於導覽列上方。所有配置皆位於 `announcementBar` 物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根目錄。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文件網站託管於主網站子目錄時特別實用（此時可能不需讓標誌連結至主網站時開啟新分頁）。
...
可透過 `themeConfig.navbar.items` 新增項目至導覽列。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向指定 `docId` 文件的連結。只要瀏覽同一側邊欄下的文件，該連結就會獲得 `navbar__link--active` 類別。
...
使用者可在同一文件（只要文件 ID 跨版本保持不變）的不同版本間切換。
...
若您使用版本化文件，此特殊導覽列項目類型會連結至當前瀏覽的文件版本（依據目前網址），並在無法取得時自動回退至最新版本。
...
若您使用[搜尋功能](../../search.mdx)，預設搜尋欄會位於導覽列最右側。

但透過此特殊導覽列項目類型，您可變更其預設位置。
...
可啟用此UI特效：當使用者向下滾動頁面時自動隱藏導覽列，向上滾動時再次顯示。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景色。請參閱[行標記說明文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)以獲得指引。
...
可透過`themeConfig.footer`新增商標與版權資訊至頁尾。商標圖檔可置於[靜態資源目錄](static-assets.mdx)，其URL設定方式與導覽列商標相同。
...
可透過`themeConfig.footer.links`新增連結至頁尾。頁尾配置分為兩種型態：**多欄式頁尾**與**簡易式頁尾**。
...
用於存取色彩模式的React鉤子。此上下文提供切換淺色/深色模式的函式，並暴露當前使用模式的布林值變數。
