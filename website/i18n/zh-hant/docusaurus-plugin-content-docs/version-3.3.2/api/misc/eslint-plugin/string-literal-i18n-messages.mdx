...
Docusaurus提供[`docusaurus write-translations`](../../../cli.mdx#docusaurus-write-translations-sitedir) API，可靜態提取標記為可翻譯的文字標籤。在`<Translate>`或`translate()`調用中使用的動態值將無法被提取。此規則將確保所有翻譯調用皆可靜態提取。
