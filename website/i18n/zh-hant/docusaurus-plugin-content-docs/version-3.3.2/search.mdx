...
該服務對任何開發者文件或技術部落格**免費開放**：請務必閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請加入 DocSearch 計畫](https://docsearch.algolia.com/apply)。

DocSearch 每週會爬取您的網站一次（排程可透過網頁介面設定），並將所有內容彙整至 Algolia 索引中。這些內容隨後會直接透過 Algolia API 從前端查詢。
...
### 索引設定 {#algolia-index-configuration}

在您的申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到您的專案中。您可以透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取作業。索引在部署後即可立即使用，因此通常不需要手動設定。

:::danger[使用推薦的爬蟲設定]

強烈建議使用我們官方的 [**Docusaurus v3 爬蟲設定**](https://docsearch.algolia.com/docs/templates/#docusaurus-v3-template)。若您選擇不同的爬蟲設定，我們將無法提供支援。

:::

:::warning[更新爬蟲設定時]

爬蟲設定中包含 `initialIndexSettings`，僅在 Algolia 索引尚未建立時用於初始化。

若您更新 `initialIndexSettings` 爬蟲設定，可以透過介面手動更新索引，但 [Algolia 團隊建議刪除索引後重新開始爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新設定重新初始化。
...
若在重大變更後搜尋仍無法使用，請透過 Algolia 儀表板 **手動觸發重新爬取**。
...
當瀏覽 v2 文件時，若出現 v1 文件的搜尋結果會顯得突兀。有時 v1 與 v2 文件內容相似，可能導致相同查詢出現重複結果（每個版本各出現一次）。

同理，瀏覽法文網站時，若出現英文文件的搜尋結果也不合理。

為解決此問題，上下文搜尋功能會自動識別您當前瀏覽的文件版本與語言，並動態建立對應的搜尋篩選條件。

- 在 `/en/docs/v1/myDoc` 頁面，搜尋結果將僅包含 **英文** 的 **v1** 文件內容（+ 其他未版本化的頁面）
- 在 `/fr/docs/v2/myDoc` 頁面，搜尋結果將僅包含 **法文** 的 **v2** 文件內容（+ 其他未版本化的頁面）
...
DocSearch 預設採用專為無障礙設計的精心調校主題，確保色彩與對比度符合標準。
...
Algolia DocSearch 支援多種 [參數選項](https://docsearch.algolia.com/docs/api/)，您可透過 `docusaurus.config.js` 檔案中的 `algolia` 欄位傳遞這些參數。
...
若需編輯 Algolia 搜尋的 React 元件，請對 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 元件進行 [swizzle 操作](swizzling.mdx)：
...
我們[推薦特定的爬蟲配置](#algolia-index-configuration)有其原因。若您選擇不同配置，我們將無法提供支援。
...
5. 確認現在能正常顯示搜尋結果（即使啟用[情境式搜尋](#contextual-search)）
...
若搜尋索引規模較小，可下載至使用者瀏覽器時，您可使用本地搜尋外掛程式。

您可在[社群資源清單](https://docusaurus.io/community/resources#search)中找到社群支援的本地搜尋外掛程式。
...
這會在專案資料夾中建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯元件後，Docusaurus 就會改用您的自訂 `SearchBar` 元件。

**注意**：您也可以[從 Algolia SearchBar 替換](#editing-the-algolia-search-component)，並以此為基礎建立自己的搜尋元件。