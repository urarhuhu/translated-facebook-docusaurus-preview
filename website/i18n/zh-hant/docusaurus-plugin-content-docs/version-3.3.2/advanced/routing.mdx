...
Docusaurus 的路由系統遵循單頁應用程式(SPA)的慣例：一個路由對應一個元件。本節將先介紹三種內容外掛（文檔、部落格和頁面）內部的路由機制，再深入探討底層路由系統的運作原理。

## 內容外掛中的路由 {#routing-in-content-plugins}

每個內容外掛都提供 `routeBasePath` 選項，用於定義外掛附加路由的基礎路徑。預設情況下，文檔外掛的路由位於 `/docs` 下；部落格外掛位於 `/blog`；頁面外掛則直接使用根路徑 `/`。您可以將路由結構想像成：
...
系統會將所有路由與此嵌套路由配置進行匹配，直到找到相符項目。例如當處理 `/docs/configuration` 路由時，Docusaurus 會先進入 `/docs` 分支，再於文檔外掛創建的子路由中搜索。

修改 `routeBasePath` 可有效改變網站路由結構。例如在[純文檔模式](../guides/docs/docs-introduction.mdx#docs-only-mode)中，將文檔外掛設為 `routeBasePath: '/'` 意味著文檔路由將不再帶有 `/docs` 前綴，但這不影響其他外掛創建如 `/blog` 的子路由。

接下來我們分別檢視這三種外掛如何組織各自的「子路由箱」。
...
文檔外掛是唯一會創建**嵌套路由**的外掛。頂層註冊了[版本路徑](../guides/docs/versioning.mdx)：`/`, `/next`, `/2.0.0-beta.13`... 這些路徑提供版本上下文（包括佈局和側邊欄），確保切換文檔時能保持側邊欄狀態，並可透過導航欄下拉選單切換版本而不離開當前文檔。使用的元件是 `@theme/DocPage`。
...
個別文檔內容會渲染在導航欄、頁尾、側邊欄等由 `DocPage` 元件提供的框架之後。例如本頁面 <URLPath /> 是由檔案 <FilePath /> 生成，使用的元件是 `@theme/DocItem`。
...
- 若路徑沒有副檔名，則為 URL 路徑。例如，在 URL 為 `/docs/advanced/routing` 的頁面上，連結 `[page](../plugins)` 會指向 `/docs/plugins`。Docusaurus 僅在建置站點時（當其知曉完整路由結構時）檢測斷開連結，但不會對檔案存在與否做任何假設。這完全等同於在 JSX 檔案中撰寫 `<a href="../plugins">page</a>`。
...
以下目錄結構可幫助您直觀理解這種檔案→URL 的映射關係。假設所有頁面均未使用 slug 自訂功能。
...
關於內容插件的說明暫告段落。現在讓我們退後一步，談談 Docusaurus 應用程式中路由的通用運作原理。
...
路由 `/docs/advanced/routing` 可能對應 `/docs/advanced/routing/index.html` 或 `/docs/advanced/routing.html`。部分託管服務商會根據結尾斜線的存在與否進行區分，且可能（或不可能）容忍另一種形式。詳見 [結尾斜線指南](https://github.com/slorber/trailing-slash-guide)。
...
舉例來說，產生的 HTML 檔案會包含如 `<link rel="preload" href="/assets/js/runtime~main.7ed5108a.js" as="script">` 的連結。由於絕對網址是從主機端解析，若打包後的檔案被放置在 `https://example.com/base/` 路徑下，該連結會指向 `https://example.com/assets/js/runtime~main.7ed5108a.js`（顯然不存在）。透過指定 `/base/` 作為基礎網址，連結才能正確指向 `/base/assets/js/runtime~main.7ed5108a.js`。
...
所有路由會匯總在 `.docusaurus/routes.js` 中，可透過除錯外掛的[路由面板](/__docusaurus/debug/routes)查看。

在客戶端，我們提供 `@docusaurus/router` 來存取頁面路由。`@docusaurus/router` 是 [`react-router-dom`](https://www.npmjs.com/package/react-router-dom/v/5.3.0) 套件的重新匯出。例如可用 `useLocation` 取得當前頁面的[位置物件](https://developer.mozilla.org/en-US/docs/Web/API/Location)，或用 `useHistory` 存取[歷史物件](https://developer.mozilla.org/en-US/docs/Web/API/History)（功能類似瀏覽器 API，但實作不同，詳見 React Router 文件）。
...
Docusaurus 建構的是[單頁應用程式](https://developer.mozilla.org/en-US/docs/Glossary/SPA)，路由轉換透過 React Router 的 `history.push()` 方法在客戶端完成。此操作的前提是目標網址已註冊於路由系統，否則路由會攔截路徑並顯示 404 頁面。

若將 HTML 檔案置於 `static` 資料夾，它們會被複製到建置輸出中而成為網站的一部分，但不會納入 Docusaurus 路由系統。我們提供 `pathname://` 協定，可像外部連結般以非 SPA 方式跳轉至網域內其他路徑。
...
`pathname://` 協定適用於參照 static 資料夾內的任何內容。例如 Docusaurus 會[將所有 Markdown 靜態資源轉為 require() 呼叫](../guides/markdown-features/markdown-features-assets.mdx#static-assets)，使用 `pathname://` 可保持原始連結形式，避免被 Webpack 雜湊處理。
