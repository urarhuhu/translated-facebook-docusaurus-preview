...
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文件主要為靜態內容，無需繁重的 JS 函式庫或填充程式
- **可擴展的建置時間**：允許獨立建置和部署本地化網站
- **本地化資源**：網站圖片可能包含需翻譯的文字
- **無耦合**：不強制使用任何 SaaS，但仍可整合
- **易於與 [Crowdin](https://crowdin.com/) 搭配使用**：許多 Docusaurus v1 網站使用 Crowdin，應能順利遷移至 v2
- **良好的 SEO 預設值**：我們為您設置實用的 SEO 標頭，如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
...
- **自動語言檢測**：此為觀點性功能，最好由[伺服器（您的託管服務商）](../deployment.mdx)處理
...
部分插件會讀取需整體本地化的外部資料檔案。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/`下建立副本進行翻譯。
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應資料夾下提供自有翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤。

每個內容插件或主題皆有差異，並**自行定義翻譯檔案存放位置**：
