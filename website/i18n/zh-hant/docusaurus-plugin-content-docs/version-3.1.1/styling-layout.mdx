...
根據您的偏好和您嘗試構建的網站類型，有幾種可行的方法/框架。行為更像網絡應用程式且具有高度互動性的網站將受益於更現代的樣式設定方法，這些方法將樣式與組件放在一起。當您希望自定義或 swizzle 組件時，組件樣式設定也可能特別有用。
...
如果要為任何元素添加 CSS，您可以在瀏覽器中打開 DevTools 來檢查其類名。類名有幾種類型：

- **主題類名**。這些類名在[下一小節](#theme-class-names)中詳細列出。它們沒有任何默認屬性。在您的自定義 CSS 中，應始終優先針對這些穩定的類名。
...
- **CSS 模塊類名**。這些類名在生產環境中帶有哈希值（`codeBlockContainer_RIuc`），在開發環境中則附加有長文件路徑。它們被視為實現細節，您幾乎應始終避免在自定義 CSS 中針對它們。如果必須，可以使用忽略哈希值的[屬性選擇器](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors)（`[class*='codeBlockContainer']`）。

### 主題類名 {#theme-class-names}

我們提供了一些穩定的 CSS 類名，用於實現健壯且可維護的全局佈局樣式設定。這些名稱與主題無關，旨在供自定義 CSS 使用。
...
如果您計劃透過 iframe 在其他網站嵌入 Docusaurus 頁面，可以建立替代顯示模式並使用如 `https://mysite.com/docs/myDoc?docusaurus-data-mode=iframe` 的 iframe 網址。您需自行提供額外樣式，並決定保留或隱藏哪些 UI 元素。

:::

### 行動裝置視圖 {#mobile-view}

Docusaurus 使用 `996px` 作為行動裝置螢幕寬度與桌機的切換點。若您希望在行動視圖中呈現不同佈局，可使用媒體查詢。
