...
- 如何使用圖片語法 (`![](https://youtu.be/yKNxeF4KMsY)`) 嵌入 YouTube 影片？
- 如何為獨立成行的連結設定不同樣式（例如社交卡片樣式）？
- 如何讓每個頁面開頭自動加入版權聲明？

答案就是：建立 MDX 插件！MDX 內建[插件系統](https://mdxjs.com/advanced/plugins/)，可用於自訂 Markdown 文件如何被解析並轉換為 JSX。MDX 插件有三種典型使用情境：
