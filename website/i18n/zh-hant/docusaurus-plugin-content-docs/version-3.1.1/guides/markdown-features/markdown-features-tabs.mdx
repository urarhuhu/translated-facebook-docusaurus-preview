...
預設會顯示第一個標籤頁，若要覆寫此行為，可在任一標籤項目添加 `default` 屬性。您也可將 `Tabs` 元件的 `defaultValue` 屬性設為指定的標籤值。例如在上方範例中，無論是為 `value="apple"` 的標籤設定 `default`，或是為 `Tabs` 設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若為 `Tabs` 提供的 `defaultValue` 指向不存在的值，Docusaurus 會拋出錯誤。若希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤選擇能彼此同步。例如，您可能想為 Windows 和 macOS 使用者提供不同操作指示，並希望透過單次點擊變更所有作業系統專用的指示標籤。為實現此功能，可為所有相關標籤設定相同的 `groupId` 屬性。請注意，此設定會將選擇持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不需完全相同。若某標籤群組選擇的值不存在於另一具有相同 `groupId` 的標籤群組中，該標籤群組不會變更其標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方標籤群組不會變動。
...
您可能想自訂特定標籤組的外觀。可透過 `className` 屬性傳遞字串，指定的 CSS 類別將被添加至 `Tabs` 元件：
...
您也可透過 `attributes` 欄位獨立自訂每個標籤標題。額外屬性可透過 `Tabs` 的 `values` 屬性傳遞，或像宣告 `label` 一樣透過每個 `TabItem` 的屬性傳遞。
...
可以將選取的標籤狀態持久化到 URL 的搜尋參數中。這讓您能分享一個預先選定標籤的頁面連結——例如從您的 Android 應用程式連結到已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動到標籤位置。
...
當標籤被點擊時，URL 末端會立即加入搜尋參數：`?current-os=android` 或 `?current-os=ios`。
...
頁面載入時，會優先從查詢字串恢復標籤選擇狀態（而非從 `localStorage` 讀取 `groupId` 的選擇記錄）。
