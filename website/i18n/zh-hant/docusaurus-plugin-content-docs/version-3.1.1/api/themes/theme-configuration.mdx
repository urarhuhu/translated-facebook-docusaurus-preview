...
有時您需要在網站發布公告。為此可添加公告欄——這是位於導覽列上方、非固定且可關閉的面板。所有配置皆位於 `announcementBar` 物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文檔網站託管於主站子目錄時特別有用——此情境下您可能不希望標誌連結至主站時開啟新分頁。
...
項目行為可依據 `type` 欄位而異。以下章節將介紹所有可用的導覽列項目類型。

#### 導覽列連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文件夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配。
...
請注意，下拉基底項目本身也是可點擊的連結，因此該項目可接受[純導航欄連結](#navbar-link)的任何屬性。
...
若您啟用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導覽列項目類型會顯示包含所有可用語系的下拉選單。
...
若您啟用[搜尋功能](../../search.mdx)，搜尋框預設會出現在導覽列最右側。

但透過此特殊導覽列項目類型，您可以變更預設位置。
...
您亦可使用此導覽列項目類型，在導覽列項目中渲染自訂HTML標記。
...
您可以啟用此精緻的UI功能：當使用者向下滾動頁面時自動隱藏導覽列，並在使用者向上滾動時重新顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。您可於[Infima文件](https://infima.dev/docs/components/navbar/)預覽樣式效果。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現程式碼區塊語法突顯。所有設定皆位於`prism`物件中。
...
預設採用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作為語法突顯主題。您可從[可用主題清單](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)指定自訂主題，亦可為深色模式設定不同的語法突顯主題。
...
您可以為程式碼區塊設定預設語言（當開頭三個反引號後未指定語言時生效）。請注意，必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
可透過 `themeConfig.footer` 添加標誌與版權聲明。標誌可置於[靜態資源目錄](static-assets.mdx)，其 URL 設定方式與導覽列標誌相同。
...
可透過 `themeConfig.footer.links` 添加頁尾連結。支援兩種配置類型：**多欄式頁尾**與**簡易頁尾**。

多欄式頁尾連結需為每欄定義 `title` 及一組 `FooterItem` 清單。
...
用於存取色彩模式的 React 鉤子。此上下文包含設定淺色/深色模式的函式，並暴露當前使用模式的布林值變數。
...
呼叫 `useColorMode` 的元件必須是 `Layout` 元件的子層級。
