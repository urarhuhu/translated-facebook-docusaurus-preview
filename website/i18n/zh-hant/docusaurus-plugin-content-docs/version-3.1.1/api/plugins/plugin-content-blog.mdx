...
提供 [Blog](blog.mdx) 功能，是 Docusaurus 的預設部落格插件。

:::warning[部分功能僅在生產環境生效]

[訂閱功能](../../blog.mdx#feed)通過解析構建輸出實現，因此**僅在生產環境生效**。
...
可通過[預設選項](../../using-plugins.mdx#docusauruspreset-classic)配置本插件。
...
Markdown 文檔可使用以下 [前置元數據](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)字段，需用 `---` 分隔線包裹。
...
### 翻譯文件路徑 {#translation-files-location}

- **基礎路徑**: `website/i18n/[locale]/docusaurus-plugin-content-blog`
- **多實例路徑**: `website/i18n/[locale]/docusaurus-plugin-content-blog-[pluginId]`
- **JSON 文件**: 通過 [`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir) 導出
