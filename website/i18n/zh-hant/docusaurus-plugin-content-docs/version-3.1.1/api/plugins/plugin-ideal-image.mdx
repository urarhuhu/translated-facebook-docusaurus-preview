...
預設情況下，此插件在開發環境中處於**非啟用狀態**，以便您始終能查看完整尺寸的圖片。若需調試理想圖片功能，可將 [`disableInDev`](#disableInDev) 選項設為 `false`。
...
此插件會註冊一個 [Webpack 加載器](https://webpack.js.org/loaders/)，改變導入/引用圖片的類型：
