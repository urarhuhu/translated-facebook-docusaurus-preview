...
Docusaurus 的預設頁面插件。經典模板預設包含此插件配置。該插件提供[建立頁面](guides/creating-pages.mdx)功能。
...
若使用 `@docusaurus/preset-classic` 預設配置，則無需額外安裝此插件作為依賴項。
...
Markdown 頁面可使用以下 Markdown [前置元數據](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)欄位，需以 `---` 分隔線包圍。
