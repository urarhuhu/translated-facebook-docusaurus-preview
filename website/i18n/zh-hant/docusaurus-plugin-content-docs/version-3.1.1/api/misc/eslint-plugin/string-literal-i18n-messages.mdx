...
Docusaurus提供[`docusaurus write-translations`](../../../cli.mdx#docusaurus-write-translations-sitedir) API，可靜態提取標記為可翻譯的文字標籤。若在`<Translate>`或`translate()`調用中使用動態值，將導致提取失敗。本規則確保所有翻譯調用皆可靜態提取。
