...
Docusaurus 使用此組件來捕獲主題佈局內以及整個應用程序中的錯誤。
...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則會渲染 `@theme/Error` 作為替代。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。

:::warning

`fallback` 屬性是一個回調函數，**而不是一個 React 功能組件**。您不能在這個回調中使用 React hooks。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能功能稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經加載完成。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後在用戶可能導航到該資源時通過 `onMouseOver` 事件觸發高優先級請求。
...
優先使用此組件而非普通的 `<a>` 標籤，因為 Docusaurus 在使用 `<Link>` 時會進行大量優化（例如檢測無效路徑、預取、應用基礎 URL 等）。
...
當需要整合無法在 Node.js 環境運行的程式碼時（例如存取 `window` 或 `document` 物件），請使用此元件。
...
您甚至可以省略 children 屬性，在執行 `docusaurus write-translations` CLI 命令後，手動在 `code.json` 檔案中指定翻譯字串。
...
用於存取 Docusaurus 上下文(Context)的 React 鉤子。該上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
`siteConfig` 物件僅包含**可序列化的值**（即經過 `JSON.stringify()` 後仍保留的值）。函數、正則表達式等內容在客戶端會丟失。
...
在 React 渲染邏輯中應使用此鉤子(hook)，而非 `typeof windows !== 'undefined'`。

客戶端首次渲染輸出（瀏覽器端）**必須完全匹配**伺服器端渲染輸出（Node.js 端）。違反此規則可能導致非預期的水合行為，詳見 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/)。
...
全域資料按插件名稱命名空間，再按插件 ID 細分。

:::info

插件 ID 僅在單個插件被多次使用時才有意義。每個插件實例均可創建自己的全域資料。
...
可通過 `./docusaurus/globalData.json` 查看站點的全域資料
...
存取特定插件創建的全域資料。根據插件名稱，返回該插件所有實例的資料（按插件 ID 分類）。
...
React hook 用於存取 Docusaurus 的壞鏈結檢查 API，提供一種方式讓 Docusaurus 頁面回報並收集其連結與錨點。:::warning 這是**進階** API，**大多數 Docusaurus 使用者無需直接使用**。
...
若您實作自己的 `<Heading>` 或 `<Link>` 元件時，才需使用 `useBrokenLinks()`。
