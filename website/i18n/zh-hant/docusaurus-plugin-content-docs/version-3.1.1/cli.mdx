...
Docusaurus 提供一系列指令腳本協助您生成、運行及部署網站。

當您的網站初始化完成後，網站原始碼將包含可透過套件管理器調用的 Docusaurus 指令：
...
清除 Docusaurus 網站生成的靜態資源、快取與建置產物。
...
預設情況下，檔案會寫入 `website/i18n/<defaultLocale>/...` 路徑。
