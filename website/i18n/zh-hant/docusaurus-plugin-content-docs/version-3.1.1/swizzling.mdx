...
本節與[樣式與版面](./styling-layout.mdx)類似，但這次我們將自訂 React 元件本身，而非其外觀。我們將討論 Docusaurus 的核心概念：**元件替換 (swizzling)**，這項技術允許**更深層的網站客製化**。

實務上，元件替換可讓您**用自訂實作替換主題元件**，主要分為兩種模式：

- [**完整替換 (Ejecting)**](#ejecting)：建立原始元件的**副本**，可進行**全面自訂**
- [**包裝替換 (Wrapping)**](#wrapping)：建立原始元件的**包裝器**，可進行**功能增強**
...
完整替換是指**建立原始元件的副本**，您可**完全自訂與覆寫**該元件。

要完整替換元件，請使用互動式 swizzle CLI 或搭配 `--eject` 參數：
...
完整替換[**不安全**](#what-is-safe-to-swizzle)元件時，可能會複製大量內部程式碼，後續需自行維護。這可能增加 Docusaurus 升級難度，若接收的屬性(props)或使用的內部主題 API 發生變更，您需自行遷移客製化內容。

**請盡可能優先使用[包裝替換](#wrapping)**：需維護的程式量較少。

:::

:::tip[重新混寫]

在 Docusaurus 升級後，若要保持已抽出的元件處於最新狀態，請重新執行抽出指令並使用 `git diff` 比較變更。建議在檔案頂部撰寫簡短註解說明您所做的修改，以便在重新抽出後能更輕鬆地重新套用您的變更。
...
- **禁止**：混寫 CLI 會阻止您混寫此元件，因為其設計上完全不允許混寫
...
不必過度**害怕混寫不安全元件**：只需記住可能發生**破壞性變更**，且在次要版本升級時您可能需要手動更新自訂內容。
...
要達成預期效果時，往往不清楚具體該混寫哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 約有[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
3. **從高層級元件開始**。元件形成樹狀結構，某些元件會導入其他元件。每條路由都會關聯一個頂層元件（多數列於[內容外掛的路由系統](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。可先swizzle此元件，再向下追蹤元件樹找到目標元件。找到正確元件後，請刪除其他swizzle產生的檔案以避免維護過多元件。
...
我們也希望了解您更進階的客製化需求，歡迎[提交討論](https://github.com/facebook/docusaurus/discussions/5468)。
...
`<Root>`元件會渲染在React樹的最頂層（高於主題`<Layout>`），且**永不卸載**。非常適合用於添加不應在導航時重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
