...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委託給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；而獨立[頁面](./guides/creating-pages.mdx)則由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供。若未安裝任何插件，網站將不包含任何路由。

不過您通常無需逐一手動安裝常見插件：它們可以透過[預設集](#using-presets)以套件形式分發。對多數使用者而言，插件是透過預設集配置來設定的。

我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合其他服務...請務必查閱該清單：可能已有人為您實現了！
...
接著在您網站的 `docusaurus.config.js` 的 `plugins` 選項中新增：
...
所有 Docusaurus 內容插件皆支援多個插件實例。例如，使用[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)可能很有用。必須為每個插件實例分配唯一ID，預設情況下插件ID為 `default`。
...
最多只能有一個插件實例作為「預設插件實例」，方式是省略 `id` 屬性，或使用 `id: 'default'`。

:::

## 使用主題 {#using-themes}

主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目可互換使用。唯一細微差別在於主題在插件之後載入，且[主題可覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[簡寫解析方式](#module-shorthands)，因此若想利用簡寫功能，請務必使用正確的條目！
...
然後，在您的網站 `docusaurus.config.js` 的 `presets` 選項中添加它：
...
預設集是一個與 [插件構造函數](./api/plugin-methods/README.mdx#plugin-constructor) 形狀相同的函數。它應該返回一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 的物件，與它們在網站配置中的接受方式相同。例如，您可以指定一個包含以下主題和插件的預設集：
...
其中 `moduleType` 是 `'preset'`、`'theme'` 或 `'plugin'` 之一，取決於模組名稱聲明的欄位。第一個成功找到的模組名稱會被加載。

如果名稱是有作用域的（以 `@` 開頭），名稱會先按第一個斜線分割為作用域和套件名稱：
...
以下是一些在 `plugins` 欄位註冊的插件範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同（這些工具強制要求插件遵循一致的命名規範），Docusaurus 允許更大的命名自由，因此解析結果並非絕對，而是遵循上述優先順序。
