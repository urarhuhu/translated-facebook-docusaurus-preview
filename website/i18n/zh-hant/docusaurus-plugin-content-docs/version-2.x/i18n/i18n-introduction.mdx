...
- **自動語言檢測**：這屬於主觀選擇，最好由[伺服器（您的託管服務提供商）](../deployment.mdx)處理
...
部分插件會讀取需整體本地化的外部資料檔案。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑建立副本進行翻譯。
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir)指令初始化。各插件在對應資料夾下提供翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤。
