...
- **對非開發者不友好**：他們不精通 Git 和拉取請求
- **對專業翻譯人員不友好**：他們習慣使用 SaaS 翻譯軟體和進階功能
...
`Hello` 會顯示為 `(fr) Hello`，這能清楚表明缺少翻譯。
...
保持翻譯檔案與原始檔案的**一致性**可能具有挑戰性，尤其是對於 Markdown 文件。

### Markdown 翻譯 {#markdown-translations}

當未翻譯的 Markdown 文件被編輯時，**維護相應的翻譯檔案是您的責任**，我們目前沒有好的方法來幫助您完成此工作。

為了保持翻譯後網站的一致性，當 `website/docs/doc1.md` 文件被編輯時，您需要**將這些修改反向移植**到 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。

### JSON 翻譯 {#json-translations}

為了協助您維護 JSON 翻譯文件，您可以再次執行 [write-translations](../cli.mdx#docusaurus-write-translations-sitedir) CLI 命令：

```bash npm2yarn
npm run write-translations -- --locale fr
```

新的翻譯將會被追加，現有的翻譯則不會被覆蓋。

:::tip

使用 `--override` 選項來重置您的翻譯。
...
編輯按鈕將會連結到本地化的文件 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。