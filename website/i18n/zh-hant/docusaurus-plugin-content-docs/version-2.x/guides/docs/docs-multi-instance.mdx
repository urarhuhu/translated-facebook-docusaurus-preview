...
此功能僅對[版本化文檔](./versioning.mdx)有用。建議在閱讀本頁前先熟悉文檔版本化功能。若您只需要[多側邊欄](./sidebar/multiple-sidebars.mdx)，可以在單一插件內實現。
...
當有人編輯 iOS 文檔時，真的有必要重新構建包括未變更的 Android 文檔在內的所有內容嗎？
...
您可以為其中一個文檔插件實例省略 `id` 屬性（預設為 `default`）。

這樣實例路徑會更簡潔，且能與單實例配置保持向後兼容。
