...
同樣地，您可以透過 `require` 現有資源，並將返回的 URL 用於 `video`、`a` 錨點連結等。
...
若使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為檔案路徑並自動轉換為 `require()` 呼叫。除非使用 JSX 語法（需自行處理），否則在 Markdown 中無需手動使用 `require()`。
...
若要使用路徑片段切換圖片顯示（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），請在自訂 CSS 中加入以下內容（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。連結最終會被轉換為 `require()` 呼叫而非保持為 URL 格式，這種處理方式有兩大優勢：

1. 您無需擔心基礎 URL 問題，Docusaurus 在提供資源時會自動處理；
2. 圖片會進入 Webpack 的建置流程，其檔名將附加雜湊值，這讓瀏覽器能積極快取圖片並提升網站效能。
