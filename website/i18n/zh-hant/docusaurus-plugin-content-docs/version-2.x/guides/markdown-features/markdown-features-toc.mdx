...
每個標題都有一個可自動生成或手動指定的 ID。標題 ID 允許您在 Markdown 或 JSX 中連結至特定文件標題：
...
- 生成的 ID 可能不夠美觀
- 當您想變更或翻譯標題文字時，可能希望保留原有 ID

您可以使用特殊的 Markdown 語法來手動設定標題 ID：
...
使用 **[`write-heading-ids`](../../cli.mdx#docusaurus-write-heading-ids-sitedir)** CLI 指令可為所有 Markdown 文件添加明確的 ID。
...
每個 Markdown 文件會在右上角顯示目錄。預設情況下，此目錄僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構概覽。若需調整顯示的標題層級範圍，您可以針對單一頁面或全域設定最小與最大標題層級。

若要為特定頁面設定標題層級，可使用 front matter 中的 `toc_min_heading_level` 和 `toc_max_heading_level` 參數。
...
`themeConfig` 選項會套用至網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），但 front matter 參數僅影響右上角的目錄。您需要使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性來個別自訂每個 `<TOCInline />` 元件。
...
請注意 `toc` 全域變數是扁平陣列，因此您可以輕鬆移除不需要的節點或插入額外節點，建立新的目錄樹狀結構。
