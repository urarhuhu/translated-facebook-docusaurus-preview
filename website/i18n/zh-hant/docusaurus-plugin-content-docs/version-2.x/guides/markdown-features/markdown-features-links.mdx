...
相對檔案路徑會以當前檔案所在目錄為基準解析。而絕對檔案路徑則會相對於 **內容根目錄** 解析，通常是 `docs/`、`blog/` 或本地化目錄如 `i18n/zh-Hans/plugin-content-docs/current`。

絕對檔案路徑也可以相對於網站目錄。但請注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為當你建立新文件版本或進行本地化時，需要手動更新這些連結。
