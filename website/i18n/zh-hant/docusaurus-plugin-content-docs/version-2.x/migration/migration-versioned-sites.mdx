...
因為在 v1 中，很有可能有人創建了一個新的文件，其前置元數據 ID 為 `"version-${version}-${id}"`，這可能會與 `versioned_docs` 的 ID 衝突。
...
Docusaurus v1 只有在文件內容不同時才會創建版本化文件。如果你的文件結構從 v1.0.0 到 v1.1.0 只有 `hello.md` 發生了變化，那麼你的文件結構可能看起來像這樣。
...
在 v2 中，你必須填充缺失的 `versioned_docs` 和 `versioned_sidebars`（並確保前置元數據和 ID 引用正確）。
