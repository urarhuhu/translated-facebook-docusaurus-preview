...
本節與[樣式與佈局](./styling-layout.mdx)類似，但這次我們將實際編寫 React 程式碼而非調整樣式表。我們將探討 Docusaurus 的核心概念：**元件替換(Swizzling)**，這項技術允許進行**更深層的網站客製化**。

實務上，元件替換可透過兩種模式**替換主題元件為自訂實作**：
...
`docusaurus swizzle` 僅是協助自動化替換元件的工具。您亦可手動建立 `src/theme/SomeComponent.js` 文件，Docusaurus 會[自動解析](./advanced/client.mdx#theme-aliases)。此命令背後並無特殊魔法！
...
要彈出主題元件，請使用互動式元件替換 CLI 或搭配 `--eject` 參數：
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 將改用此 `<Footer>` 元件副本而非原始版本。您現在可完全重新實作 `<Footer>` 元件。
...
彈出[**不安全**](#what-is-safe-to-swizzle)元件可能導致複製大量內部程式碼，後續需自行維護。這會增加 Docusaurus 升級難度，若接收的 props 或使用的內部主題 API 發生變更，您需自行遷移客製化內容。

**請盡可能優先採用[包裝(wrapping)](#wrapping)**：需維護的程式量較少。

:::

:::tip[重新混寫]

為了在升級 Docusaurus 後保持彈出組件的更新，請重新執行彈出命令並使用 `git diff` 比較變更。建議在文件頂部添加簡短註解說明您所做的修改，以便在重新彈出後能更輕鬆地重新套用這些變更。

:::

### 封裝 {#wrapping}

封裝主題組件是指**創建一個包裝器**環繞原始主題組件，從而對其進行**功能增強**的過程。

要封裝主題組件，可使用交互式混寫 CLI 或搭配 `--wrap` 選項：
...
這將在您網站的 `src/theme` 目錄中創建封裝組件。Docusaurus 現在會使用 `<FooterWrapper>` 組件取代原始組件，您可以在原始組件周圍添加自定義內容。
...
封裝主題是**在現有組件周圍添加額外元素**的絕佳方式，無需[彈出](#ejecting)該組件。例如，您可以輕鬆在每篇部落格文章下方添加自定義評論系統：
...
## 哪些組件可安全混寫？ {#what-is-safe-to-swizzle}

> 能力越大，責任越大

某些主題組件屬於主題的**內部實現細節**。Docusaurus 允許您混寫這些組件，但**可能存在風險**。
...
對於每個主題組件，混寫 CLI 會標示主題作者聲明的**三種安全等級**：

- **安全**：此組件可安全混寫，其公開 API 被視為穩定，在主題**主版本**內不會發生破壞性變更
- **不安全**：此組件屬於主題實現細節，混寫不安全，可能在主題**次版本**升級時發生破壞性變更  
- **禁止**：混寫 CLI 會阻止您混寫此組件，因其設計上完全不允許混寫
...
不必過度**害怕混寫不安全組件**：只需注意可能會有**破壞性變更**，在次版本升級時可能需要手動更新您的自定義內容。

:::

:::note[回報使用情境]

如果您有**強烈需求必須混寫不安全組件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找使其安全的解決方案。

:::

## 應該混寫哪個組件？ {#which-component-should-i-swizzle}

要精確判斷該混寫哪個組件來達成預期效果並非總是顯而易見。提供大多數主題組件的 `@docusaurus/theme-classic` 約有[100個組件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！

:::tip

要列印所有 `@docusaurus/theme-classic` 組件的概覽：
...
1. **元件描述**。部分元件會提供簡短說明，這是快速找到合適元件的好方法。
2. **元件名稱**。官方主題元件皆採用語義化命名，您可從名稱推斷其功能。Swizzle CLI 支援輸入元件名稱片段來篩選選項。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高階元件開始**。元件會形成樹狀結構相互引用。每條路由都會關聯一個頂層元件（多數列於[內容外掛路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先swizzle此元件，再沿元件樹向下定位目標元件。找到正確元件後，請刪除其他swizzle產生的檔案以避免維護過多元件。
...
我們也希望了解您的客製化需求，歡迎[提交討論](https://github.com/facebook/docusaurus/discussions/5468)。
...
1. **使用CSS**。CSS規則與選擇器通常能滿足多數客製化需求，詳見[樣式與版面](./styling-layout.mdx)。
2. **使用翻譯功能**。翻譯本質上是自訂文字標籤的途徑。例如若您的網站預設語言是`en`，仍可執行`yarn write-translations -l en`後編輯生成的`code.json`，詳見[i18n教學](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**。若必須swizzle，請盡量只處理相關部分並減少自行維護的程式碼量。swizzle小型元件通常能降低升級時的**破壞性變更**風險。
...
`<Root>`元件會渲染在React樹的最頂層（高於主題`<Layout>`）且**永不卸載**，非常適合放置不該因導航而重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
...
可用此元件來渲染React Context providers。
