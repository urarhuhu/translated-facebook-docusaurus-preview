...
有關站點成為 PWA 的完整要求清單，請參考 [PWA 檢查清單](https://developers.google.com/web/progressive-web-apps/checklist)

## 應用安裝支援 {#app-installation-support}

如果瀏覽器支援，您應能將 Docusaurus 站點安裝為應用程式。

![應用安裝過程的螢幕錄影。瀏覽器地址欄中出現一個按鈕，點擊後顯示對話框詢問「安裝此應用程式？」。點擊「安裝」按鈕後，操作系統中會打開一個新應用程式，顯示 Docusaurus 首頁。](/img/pwa_install.gif)
...
>
> Workbox 通過簡化 API 並確保資源高效下載，大大減輕了預緩存的繁瑣工作。

預設情況下，當站點作為應用安裝時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項。

站點預緩存後，service worker 將為後續訪問提供緩存響應。當部署新構建版本並生成新 service worker 時，新版本會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗，要求用戶刷新頁面以獲取新內容。在用戶清除應用緩存或點擊彈窗中的 `reload` 按鈕前，service worker 將繼續提供舊內容。

:::warning

離線模式/預緩存需要提前下載站點所有靜態資源，可能消耗不必要的頻寬。對於某些類型的站點，啟用此功能可能不是好主意。
...
- `appInstalled`：針對已將網站安裝為應用的使用者啟用（並非 100% 可靠）
- `standalone`：針對以獨立應用模式運行的使用者啟用（通常是 PWA 安裝後的情況）
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入 `<head>` 標籤。技術上您可以透過此選項注入任何 head 標籤，但理想情況下用於使網站符合 PWA 規範的標籤。以下是使應用完全符合規範的標籤清單：
...
在 Docusaurus 應用之前新增一個進入點，以便註冊能在應用執行前完成。預設的 `registerSW.js` 檔案足以進行簡單的註冊。
...
當新的 Service Worker 等待安裝時，會渲染 `@theme/PwaReloadPopup` 元件並向使用者建議重新載入頁面。您可以[替換](../../swizzling.mdx)此元件並實作自己的 UI。該元件會接收一個 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時呼叫此函式。這將指示 Service Worker 安裝等待中的版本並重新載入頁面。

預設主題內建了重新載入彈出視窗的實作，採用 [Infima 警示框](https://infima.dev/docs/components/alert)樣式。
