...
有時您需要在網站發布公告。為此可添加公告欄——這是位於導覽列上方、非固定且可選擇關閉的面板。所有配置皆位於 `announcementBar` 物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文件網站託管於主站子目錄時特別有用——此情況下您可能不希望標誌連結至主站時開啟新分頁。
...
React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若某連結需在多個不同路徑下保持活動狀態（例如當同一側邊欄下有多個文件夾時），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
請注意，下拉選單基礎項目本身也是可點擊的連結，因此該項目可接收[純導航欄連結](#navbar-link)的任何屬性。
...
您可將導航欄項目連結至指定側邊欄的第一個文件連結（可能是文件連結或自動生成的類別索引），而無需硬編碼文件 ID。
...
若您使用版本化文件，此特殊導覽列項目類型會連結至當前瀏覽的文件版本（依據網址判斷），並在無匹配版本時自動回退至最新版本。
...
若啟用[搜尋功能](../../search.mdx)，搜尋欄預設會出現在導覽列最右側。
...
啟用此UI特性後，當使用者向下滾動頁面時導覽列會自動隱藏，向上滾動時重新顯示。
...
可設定靜態導覽列樣式而不影響主題切換功能。所選樣式將始終生效，不受使用者選擇的主題影響。

目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifma-color-primary`色值）。樣式預覽可參閱[Infima文件](https://infima.dev/docs/components/navbar/)。
...
預設使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。可從[可用主題清單](https://github.com/FormidableLabs/prism-react-renderer/tree/master/src/themes)選擇自訂主題，並為深色模式指定不同高亮主題。
...
若使用行高亮Markdown語法，可能需要為深色模式語法高亮主題指定不同的高亮背景色。詳見[行高亮說明文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
您可以透過`themeConfig.footer`添加標誌和版權聲明至頁尾。標誌可放置於[靜態資料夾](static-assets.mdx)中，其URL運作方式與導覽列標誌相同。
...
您可透過`themeConfig.footer.links`添加連結至頁尾。頁尾配置有兩種型態：**多欄式頁尾**與**簡易頁尾**。

多欄式頁尾連結需為每個欄位設定`title`及一組`FooterItem`清單。
...
用於存取色彩模式的React鉤子。此上下文包含設定亮色/暗色模式的函式，並暴露布林值變數顯示當前使用模式。
