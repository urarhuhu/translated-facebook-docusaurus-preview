...
專案初始化完成後，網站原始碼將包含可透過套件管理工具執行的 Docusaurus 指令：
...
以下為 Docusaurus CLI 指令及其用途清單：

### `docusaurus start [siteDir]` {#docusaurus-start-sitedir}

使用 [Webpack Dev Server](https://webpack.js.org/configuration/dev-server) 在本機建構並預覽網站。
...
請注意部分功能（例如錨點連結）在開發環境可能無法運作，正式環境將正常執行。

:::

:::info[網路開發模式]

當從遠端伺服器或虛擬機（如 GitHub Codespaces）轉發 3000 埠時，可指定 `0.0.0.0` 讓開發伺服器監聽本地 IP 位址。
...
2. 執行 `mkcert -install` 安裝憑證至信任儲存區，並重啟瀏覽器
...
針對 CSS 套件的進階壓縮，我們採用 [cssnano 進階預設值](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（搭配多個 PostCSS 外掛）與 [clean-css 第 2 級最佳化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若因此導致 CSS 異常，可設定環境變數 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 預設值](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 壓縮 CSS。**若遭遇 CSS 壓縮錯誤，請[提交問題報告](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)。**
