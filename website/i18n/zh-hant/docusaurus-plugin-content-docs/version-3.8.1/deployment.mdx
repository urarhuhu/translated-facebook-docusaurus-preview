...
將潛在敏感資訊放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是與 Node.js 環境的唯一接口（請參閱 [我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接訪問 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
沒有一個萬能的解決方案。您需要權衡您的需求和資源，然後做出選擇。

## 自架主機 {#self-hosting}

Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令進行自架主機部署。透過 `--port` 參數變更連接埠，或使用 `--host` 變更主機位址。
...
以下章節將介紹常見託管服務供應商及其針對 Docusaurus 站點的最佳化部署設定。Docusaurus 與這些服務無商業合作關係，此處資訊僅供參考。部分內容由第三方提供，最新API變動可能未即時反映。若發現過時內容，歡迎提交PR修正。

由於我們僅能盡力維護現有內容，目前已停止接受新增託管方案的PR。您可將部署指南發表於外部平台（如個人部落格或服務商官網），並申請將連結收錄至本文件。

:::

## 部署至 Netlify {#deploying-to-netlify}

要將 Docusaurus 站點部署至 [Netlify](https://www.netlify.com/)，請先確認以下設定正確配置：
...
接著前往 [Netlify 建立新站點](https://app.netlify.com/start)。

設定站點時，請依下列規範指定建置指令與發佈目錄：

- 建置指令：`npm run build`
- 發佈目錄：`build`

若未預先設定這些建置選項，仍可於站點建立後至「Site settings」→「Build & deploy」進行調整。

完成上述設定後，站點將自動部署並在合併至預設部署分支（通常為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 站點將 `docs` 目錄置於 `website` 外層（多見於 Docusaurus v1 舊版結構）：
...
若選擇將 `website` 目錄設為 Netlify 的基礎目錄，需配置 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/) 以避免更新 `docs` 時未觸發建置：
...
建議關閉 Netlify 的「Post Processing > Asset Optimization > Pretty Urls」設定，以避免網址強制小寫化、非必要重新導向及404錯誤。

**請特別注意**：全局選項「Disable asset optimization」實際無法正確關閉「Pretty URLs」設定，務必手動獨立取消勾選該選項。
...
將 Docusaurus 專案部署至 [Vercel](https://vercel.com/) 可獲得[多重優勢](https://vercel.com/)，包含效能優化與操作簡便性。

要透過 [Vercel Git 整合功能](https://vercel.com/docs/concepts/git) 部署專案，請先確保程式碼已推送至 Git 儲存庫。

透過 [匯入流程](https://vercel.com/import/git) 將專案導入 Vercel。在導入過程中，您會發現所有相關選項已預先配置完成；不過，您仍可選擇變更任何這些 [選項](https://vercel.com/docs/build-step#build-&-development-settings)。

專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）的變更則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務的建置輸出的分支。以下教學中將分別稱其為「來源」與「部署」分支。
...
GitHub Pages 預設會為 Docusaurus URL 加上尾部斜線。建議設定明確的 `trailingSlash` 配置（`true` 或 `false`，非 `undefined`）。
...
GitHub Pages 預設會透過 [Jekyll](https://jekyllrb.com/) 處理發布檔案。由於 Jekyll 會捨棄所有以 `_` 開頭的檔案，建議您在 `static` 目錄中加入名為 `.nojekyll` 的空檔案來停用 Jekyll。
...
1. 當有新的拉取請求提交到 `main` 分支時，會有一個動作確保網站能成功建置，但不會實際部署。此工作將稱為 `test-deploy`。
2. 當拉取請求合併到 `main` 分支或有人直接推送到 `main` 分支時，將會建置並部署到 GitHub Pages。此工作將稱為 `deploy`。

以下是使用 GitHub Actions 部署文件的兩種方法。根據部署儲存庫的位置，選擇下方相關的標籤：
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中總是會顯示為已跳過，這無法反映實際狀態，也對審查流程沒有幫助。因此，我們建議將它們分開管理為不同的工作流程。
...
跨儲存庫發布的設定較為困難，因為您需要有權限檢查才能推送到另一個儲存庫。我們將使用 SSH 進行身份驗證。
...
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私鑰，並在原始碼儲存庫中建立名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令列無法使用，請直接複製檔案內容。儲存您的秘密。
...
持續整合 (CI) 服務通常用於在原始碼控制中提交新變更時執行例行任務。這些任務可以是執行單元測試和整合測試、自動化建置、將套件發佈到 npm，以及將變更部署到您的網站等操作的任意組合。要自動化網站的部署，您只需在網站更新時呼叫 `yarn deploy` 腳本即可。以下章節將說明如何使用熱門的持續整合服務供應商 [Travis CI](https://travis-ci.com/) 來實現此目的。

1. 前往 https://github.com/settings/tokens 並生成一個新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予其 `repo` 權限範圍以確保其具備所需權限。
...
4. 建立一個名為 `GH_TOKEN` 的新環境變數，其值為您新生成的權杖，接著建立 `GH_EMAIL`（您的電子郵件地址）和 `GH_NAME`（您的 GitHub 使用者名稱）。
5. 在儲存庫的根目錄中建立一個包含以下內容的 `.travis.yml` 檔案：
...
現在，每當有新的提交進入 `main` 分支時，Travis CI 將執行您的測試套件，如果一切通過，您的網站將透過 `yarn deploy` 腳本進行部署。

### 使用 Buddy 觸發部署 {#triggering-deployment-with-buddy}

[Buddy](https://buddy.works/) 是一款易於使用的 CI/CD 工具，可讓您將入口網站自動部署到不同環境，包括 GitHub Pages。

按照以下步驟建立一個管道，以便在您將變更推送到所選專案分支時自動部署網站的新版本：
...
3. 選擇 GitHub 作為 git 代管服務提供商，並選取包含網站程式碼的儲存庫。
...
5. 點擊儲存庫，點擊啟用儲存庫，並新增名為 `git_deploy_private_key` 的密碼，其值為您剛生成的私鑰。
...
[Koyeb](https://www.koyeb.com) 是一個對開發者友善的無伺服器平台，可將應用程式部署至全球。該平台讓您能無縫運行 Docker 容器、網頁應用程式和 API，並提供基於 Git 的部署、原生自動擴展、全球邊緣網路，以及內建的服務網格與服務發現功能。請參閱 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 以開始使用。
...
2. 建立專案。
   ...
   - 點擊 **下一步**。
3. 建立新環境。
   - 點擊 **建立環境** 並為其命名（例如 staging、production）。
4. 新增應用程式。
   - 點擊 **建立應用程式**，為其命名並選擇您的 GitHub 或 GitLab 儲存庫，其中包含您的 Docusaurus 應用程式。
   - 定義主分支名稱和應用程式的根路徑。
   - 點擊 **建立**。應用程式建立後：
   - 導航至您的應用程式 **設定**
   - 選擇 **連接埠**
...
[Hostman](https://hostman.com/) 允許您免費託管靜態網站。Hostman 自動化了一切，您只需連接您的儲存庫並遵循這些簡單步驟：
...
4. 部署。
   ...
   - 開始後，您將進入部署日誌。如果程式碼有任何問題，日誌中會顯示警告或錯誤訊息，指出問題原因。通常，日誌包含您所需的所有除錯資料。

   - 部署完成後，您將收到電子郵件通知，並看到日誌條目。大功告成！您的專案已上線並準備就緒。

## 部署至 Surge {#deploying-to-surge}

Surge 是一個[靜態網站託管平台](https://surge.sh/help/getting-started-with-surge)，您可以用它從命令列在幾秒內部署 Docusaurus 專案。將專案部署至 Surge 既簡單又免費（包括自訂網域和 SSL 憑證）。
...
1. 首先，使用 npm 安裝 Surge，執行以下指令：
   ...
   npm install -g surge
   ```
2. 在專案根目錄中，執行以下指令以建置專案的靜態檔案供生產環境使用：
   ```bash npm2yarn
   npm run build
   ```
3. 接著，在專案根目錄中執行以下指令：
...
首次使用 Surge 的用戶會從命令列被提示建立帳號（僅此一次）。

確認您要發佈的網站位於 `build` 目錄中。系統會隨機產生一個 `*.surge.sh` 子網域（可編輯）。
...
透過以下指令將您的網域儲存在 CNAME 檔案中，以供未來部署使用：
...
您可以將 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個針對靜態網站、單頁應用程式 (SPA) 和無伺服器功能的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項能直接從程式碼儲存庫自動建置並部署全端網頁應用程式至 Azure 的服務，簡化了 CI/CD 的開發者體驗。Static Web Apps 將網頁應用的靜態資源與動態（API）端點分離。靜態資源透過全球分發的內容伺服器提供，讓客戶端能從鄰近伺服器更快取得檔案。動態 API 則採用無伺服器架構，透過事件驅動的函式基礎方法進行擴展，更具成本效益且能依需求擴展。依照[此逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，幾分鐘內即可開始使用。

## 部署至 Kinsta {#deploying-to-kinsta}

[Kinsta 靜態網站託管](https://kinsta.com/static-site-hosting)讓您免費部署最多 100 個靜態網站，包含自訂網域與 SSL、每月 100 GB 頻寬，以及 260+ 個 Cloudflare CDN 節點位置。

依照我們的 [Docusaurus on Kinsta](https://kinsta.com/docs/docusaurus-example/) 文章指引，只需點擊幾下即可開始使用。