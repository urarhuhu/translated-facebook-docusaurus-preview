...
預設顯示第一個標籤頁，若要覆寫此行為，可透過在標籤項目添加 `default` 屬性指定預設標籤。亦可設定 `Tabs` 元件的 `defaultValue` 屬性為指定標籤的標籤值。例如上方案例中，無論是為 `value="apple"` 標籤設定 `default` 屬性，或是為 `Tabs` 設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若為 `Tabs` 提供的 `defaultValue` 指向不存在的值，Docusaurus 會拋出錯誤。若希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤選擇能相互同步。例如，您可能想為 Windows 用戶與 macOS 用戶提供不同的操作指示，並希望透過單擊切換所有作業系統專用的指示標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此設定會將選擇持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不必相同。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，則缺少該值的標籤群組不會變更其標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux，上方的標籤群組不會變更。
...
頁面載入時，會優先還原標籤的查詢字串選擇（其次才是透過 `localStorage` 儲存的 `groupId` 選擇）。
