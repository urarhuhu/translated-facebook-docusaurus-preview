...
本指南將依序介紹：從[如何配置個別頁面](./docs-create-doc.mdx)開始，到[如何建立單一或多個側邊欄](./sidebar/index.mdx)，再到[如何建立與管理版本](./versioning.mdx)，最後說明[如何使用多個文件插件實例](./docs-multi-instance.mdx)。

## 純文件模式 {#docs-only-mode}

新初始化的 Docusaurus 網站具有以下結構：
...
所有文件將透過子路由 `docs/` 提供。但若**您的網站僅包含文件**，或希望將文件置於根目錄以提升優先級該怎麼辦？
...
請注意，您**不一定需要放棄使用部落格**或其他插件；`routeBasePath: '/'` 的作用僅是將文件服務路徑從 `https://example.com/docs/some-doc` 改為網站根目錄 `https://example.com/some-doc`。若啟用部落格功能，仍可透過 `blog/` 子路由存取。

別忘了透過添加前置元數據在根目錄 (`https://example.com/`) 建立頁面：
