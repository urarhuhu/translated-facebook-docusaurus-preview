...
在升級之前，我們建議 [**為 Docusaurus v3 準備您的網站**](/blog/preparing-your-site-for-docusaurus-v3)。有一些變更您可以在**Docusaurus v2 下逐步處理**。這樣做有助於減少最終升級至 Docusaurus v3 所需的工作。
...
請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的發布說明，並瀏覽 pull-requests 以獲取其他有用信息以及此處提到的每個變更背後的動機。
...
在您的網站上執行 [`npx docusaurus-mdx-checker`](https://github.com/slorber/docusaurus-mdx-checker)，以獲取現在在 Docusaurus v3 下無法編譯的文件列表。
...
升級 MDX 伴隨著 [MDX v2](https://mdxjs.com/blog/v2/) 和 [MDX v3](https://mdxjs.com/blog/v3/) 發布部落格中記載的所有重大變更。大多數重大變更來自 MDX v2，而 MDX v3 是一個相對較小的版本。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/) 中有一個關於如何[更新 MDX 文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的章節，這對我們特別相關。同時請確保閱讀[MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，這可以幫助您解讀常見的 MDX 錯誤訊息。

請確保也閱讀我們更新的 [**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx) 文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場是您的新好朋友。它可以幫助您理解您的內容如何**被編譯為 React 元件**，並在隔離環境中排查編譯或渲染問題。
...
使用此 CLI 來評估需要多少工作才能使您的內容與 MDX v3 兼容。
...
- `MDXComponent` 對 `h1` 的映射僅適用於 `# 標題` 而不適用於 `<h1>標題</h1>`
...
如果該內容尚未使用 Markdown 語法，您也可以將其用 `{` 和 `}` 包裹以避免額外的 `<p>` 標籤。
...
Docusaurus v3 現在使用 [Markdown Directives](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）作為一種通用方式來支持 admonitions 和其他即將推出的 Docusaurus 功能。
...
日文和中文受此影響最大，但其他一些語言（如泰文和高棉文）也可能受到影響，例如當您嘗試強調行內程式碼或連結時。使用空格分隔單詞的語言受影響程度較小。
...
如果您希望繼續使用 CommonJS 模組，可以使用動態導入作為變通方案，這樣可以在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用非同步函數](/docs/configuration#syntax-to-declare-docusaurus-config)，讓您可以這樣做：
...
如果您創建了自定義的 Remark 或 Rehype 插件，由於新的 AST 結構，您可能需要重構這些插件，甚至完全重寫它們。我們創建了一個[專屬支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來幫助插件開發者升級他們的程式碼。

:::

### 程式碼格式化工具

目前最常見的格式化工具 Prettier 僅支援舊版 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可以在程式碼不相容的部分前添加 `{/* prettier-ignore */}` 來讓 Prettier 正常工作。
...
同時，請配置您的持續整合(CI)、內容傳遞網路(CDN)或主機服務以使用此新版本的 Node.js。
...
React 18 自帶一些破壞性變更，根據您為網站編寫的自訂 React 程式碼量，處理難度會有所不同。官方主題和外掛程式已相容於 React 18。

:::info[如何升級]

請閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，檢查您自己的 React 程式碼以確認哪些元件可能受此升級影響。
...
這些功能在 Docusaurus 中的支援尚屬實驗性質。未來我們可能需要調整整合方式，這可能導致運行時行為發生變化。
...
這是包含破壞性變更的新主版本，我們無法保證完全向下相容。[`prism-react-renderer` v2 版本說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)不夠詳盡，但 Docusaurus 使用者需注意以下 3 項主要變更。
...
但請注意這是重大版本更新，我們無法保證完全向下相容。若遇到問題請查閱 [v3](https://github.com/FormidableLabs/react-live/releases/tag/v3.0.0) 和 [v4](https://github.com/FormidableLabs/react-live/releases/tag/v4.0.0) 變更日誌。
...
多數 Docusaurus 用戶無需操作。使用表情符號快捷碼的用戶請查閱 [變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md) 並確認表情符號渲染效果符合預期。

> **重大變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 升級至 v2。此變更新增支援多種新表情符號，並移除了 GitHub 已不再支援的舊快捷碼。
...
但請注意這是重大版本更新，我們無法保證完全向下相容。若遇到問題請查閱 [v10](https://github.com/mermaid-js/mermaid/releases/tag/v10.0.0) 變更日誌。
...
官方 Docusaurus TypeScript 配置已從外部套件 [`@tsconfig/docusaurus`](https://www.npmjs.com/package/@tsconfig/docusaurus) 遷移至我們新的 monorepo 套件 [`@docusaurus/tsconfig`](https://www.npmjs.com/package/@docusaurus/tsconfig)。

此新套件版本將與其他 Docusaurus 核心套件同步更新，用於確保 TypeScript 的向下相容性與重大版本升級的變更管理。
...
Docusaurus v3 將內部配置載入函式庫從 [`import-fresh`](https://github.com/sindresorhus/import-fresh) 更換為 [`jiti`](https://github.com/unjs/jiti)。此載入器負責處理如 `docusaurus.config.js`、`sidebars.js` 及 Docusaurus 插件等檔案的載入。

:::info[如何升級]

理論上您無需任何操作，現有配置文件應能繼續正常運作。

但這是一次核心依賴庫的重大更換，可能出現細微的行為變化。

:::

### 警告提示框注意事項

由於歷史原因，我們曾支持未公開的紅色`:::warning`警告提示框。
...
但其顏色與圖標始終存在錯誤。Docusaurus v3正式重新引入`:::warning`提示框，修正顏色與圖標並納入文檔。
...
若您曾使用未公開的`:::warning`提示框，請逐一確認黃色是否適用。如需保持紅色，請改用`:::danger`。

Docusaurus v3同時[棄用`:::caution`](https://github.com/facebook/docusaurus/pull/9308)提示框。請將`:::caution`(黃色)重構為`:::warning`(黃色)或`:::danger`(紅色)。

如需保留"caution"標題，可重構為`:::warning[caution]`(黃色)。

:::

### 版本化側邊欄

此變更僅影響**在v2.0.0-beta.10版本前**(2021年12月)啟用文檔版本化的早期用戶。

創建v1.0.0版本時，側邊欄文件包含`version-v1.0.0/`前綴，[Docusaurus v3已不再支持](https://github.com/facebook/docusaurus/pull/9310)。
...
`@docusaurus/plugin-content-blog`現在默認將RSS饋送限制為最近20條內容。對於大型部落格，這能避免饋送文件過度膨脹。

:::info[如何升級]

若不認同此默認行為，可通過新增的`limit: false`選項恢復"無限制"模式：
...
對於替換過文檔相關主題組件(如`@theme/DocPage`)的用戶，這些組件已進行重大重構以提升可定制性。

技術上**這不屬於破壞性變更**，因這些組件**標記為不建議替換**。但許多站點仍替換了文檔組件，需注意自定義內容可能失效。

:::info[如何升級]

刪除所有替換組件，重新替換後在新組件基礎上重新應用定制。

或參考[PR說明](https://github.com/facebook/docusaurus/pull/7966)理解新組件樹結構，嘗試手動遷移現有組件。
...
當您在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，我們建議使用 `.mdx` 副檔名。這在語意上更為正確，並能提升與外部工具（IDE、格式化工具、linter 等）的相容性。

在未來版本的 Docusaurus 中，`.md` 檔案將被解析為標準的 [CommonMark](https://commonmark.org/)，該格式不支援這些功能。在 Docusaurus v3 中，`.md` 檔案仍會被編譯為 MDX 檔案，但未來將可[選擇啟用 CommonMark](https://github.com/facebook/docusaurus/issues/3018)。
...
Docusaurus 現在透過 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（由 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實作警告框，而指定指令標籤需使用方括號：
...
我們建議暫時保持此兼容性選項開啟，直到我們提供與新版 MDX 相容的新語法為止。

## 疑難排解

若遇到升級問題，建議優先嘗試以下步驟：

- 確認所有文檔都能在 [MDX 遊樂場](https://mdxjs.com/playground/) 或透過 [`npx docusaurus-mdx-checker`](https://github.com/slorber/docusaurus-mdx-checker) 成功編譯
- 刪除 `node_modules` 和 `package-lock.json` 後重新執行 `npm install`
- 執行 `docusaurus clear` 清除快取
- 移除可能尚未支援 Docusaurus v3 的第三方插件
- 刪除所有自行客製化的組件
...
- 提供**最簡化**且可立即執行的重現範例，建議使用 [docusaurus.new](https://docusaurus.new) 建立
- 若網站可建構成功，請提供展示問題的線上部署網址
- 清楚描述問題細節，避免僅籠統表示「無法運作」
- 附上所有相關資料：程式碼片段、儲存庫網址、Git 分支連結、完整錯誤堆疊、螢幕截圖與影片
- 以清晰簡潔的方式呈現請求，展現您已盡力自助的誠意

您亦可考慮付費委託 [Docusaurus 服務供應商](https://github.com/facebook/docusaurus/discussions/9281) 代為升級。若專案屬開源性質，也可在社群尋求 [免費友善協助](https://github.com/facebook/docusaurus/discussions/9283)。