...
**不應提交**此權杖，建議為公司建立專用的 **Crowdin 帳號**而非使用個人帳戶。
...
在 `website/i18n/en` 中為預設語言產生 JSON 翻譯檔案：
...
在 `https://crowdin.com/project/<MY_PROJECT_NAME>` 點選法文目標語言。
...
![Crowdin 隱藏字串介面](/img/crowdin/crowdin-hide-string.png)
...
先使用**[預先翻譯功能](https://support.crowdin.com/pre-translation-via-machine/)**處理網站內容，再**手動修正預先翻譯的錯誤**（需先在設定中啟用全域翻譯記憶庫）。

建議先使用 `Hide String` 功能，因 Crowdin 的預先翻譯機制可能過度樂觀地處理不該翻譯的內容。
...
確認網站現已成功翻譯為法語版本，可透過 [`http://localhost:3000/fr/`](http://localhost:3000/fr/) 存取。

### 透過 CI 自動化流程 {#automate-with-ci}

我們將設定 CI 在建置時**自動從 Crowdin 下載翻譯檔案**，並將其排除在 Git 版本控制外。

將 `website/i18n` 加入 `.gitignore`。

在 CI 環境中設定 `CROWDIN_PERSONAL_TOKEN` 環境變數。

建立 npm 腳本來 `sync` Crowdin 流程（提取原始檔、上傳原始檔、下載翻譯）：
...
維持部署預覽速度：不需下載翻譯檔案，針對功能分支使用 `npm run build -- --locale en` 指令。

:::

:::warning

Crowdin 對多個並行上傳/下載支援不佳：建議僅在正式部署時包含翻譯內容，保持部署預覽版本為未翻譯狀態。
...
Crowdin **官方不支援 MDX 格式**，但已新增 **`.mdx` 副檔名支援**，會將此類檔案視為 Markdown 格式（而非純文字）。

#### MDX 常見問題 {#mdx-problems}

Crowdin 會將 JSX 語法誤判為嵌入式 HTML，可能在下載翻譯時破壞 JSX 標記結構，導致網站因無效 JSX 而建置失敗。

使用簡單字串屬性的 JSX 片段（如 `<Username name="Sebastien"/>`）可正常運作；但使用物件/陣列屬性的複雜片段（如 `<User person={{name: "Sebastien"}}/>`）則更容易因語法不符合 HTML 格式而失敗。

#### MDX 解決方案 {#mdx-solutions}

我們建議將複雜的嵌入式 JSX 程式碼提取為獨立的元件。我們還新增了 `mdx-code-block` 的逃脫語法：
...
- 不幸的是會退出 MDX 工具鏈（如 IDE 語法高亮、Prettier 等）

### 文件版本控制 {#docs-versioning}

為 `website/versioned_docs` 資料夾配置翻譯檔案。

當建立新版本時，原始字串通常與當前版本 (`website/docs`) 非常相似，而您不希望一再重複翻譯新版本的文件。
...
有時，您會在 Git 上**刪除或重新命名原始檔案**，而 Crowdin 會顯示 CLI 警告：
...
- Git -> Crowdin 同步是手動的（您需要按按鈕）
- Crowdin 用於匹配現有 Markdown 翻譯與原始 Markdown 的啟發式方法並非 100% 可靠，您必須在每次從 Git 同步後在 Crowdin UI 上驗證結果
- 兩個用戶同時在 Git 和 Crowdin 上編輯可能導致翻譯遺失
- 需要在儲存庫根目錄放置 `crowdin.yml` 檔案
...
不幸的是，由於技術原因，它目前無法運作，但我們有希望解決這個問題。

Crowdin 會將 Markdown 字串替換為技術 ID，如 `crowdin:id12345`，但這種替換過於激進，包括隱藏字串，並會破壞 front matter、admonitions 和 JSX...
