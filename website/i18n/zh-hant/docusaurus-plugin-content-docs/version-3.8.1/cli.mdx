...
當網站初始化完成後，專案源碼中將包含可透過套件管理器調用的 Docusaurus 腳本：
...
當從遠端伺服器或虛擬機（如 GitHub Codespaces）轉發 3000 端口時，可透過 `0.0.0.0` 參數使開發伺服器監聽本地 IP 位址。
...
針對 CSS 套件的進階壓縮，我們採用 [cssnano 進階預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（搭配多個 PostCSS 插件）與 [clean-css 第二級優化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若此進階壓縮導致 CSS 異常，可透過環境變數 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 默認預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 進行壓縮。**若遭遇 CSS 壓縮錯誤，請[提交問題報告](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)。**
