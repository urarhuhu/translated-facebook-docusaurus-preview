...
用它來包裹可能拋出錯誤的組件，並在發生錯誤時顯示後備內容，而不是導致整個應用程序崩潰。
...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含 2 個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個函數 (`() => void`) 回調，用於重置組件中的錯誤並嘗試重新渲染。如果未提供，則將渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹站點的錯誤邊界，位於佈局之上。

:::warning

`fallback` 屬性是一個回調函數，**而不是一個 React 函數組件**。您不能在此回調中使用 React hooks。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能功能稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經被獲取。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後使用 `onMouseOver` 事件在用戶可能導航到請求的資源時觸發高優先級請求。
...
相比普通的 `<a>` 標籤，更推薦使用此組件，因為 Docusaurus 做了許多優化（例如檢測錯誤路徑、預取、應用基礎 URL 等）。

:::

### `<Redirect/>` {#redirect}

渲染 `<Redirect>` 將導航到新位置。新位置將像服務器端重定向（HTTP 3xx）那樣覆蓋歷史堆棧中的當前位置。您可以參考 [React Router 的 Redirect 文檔](https://reacttraining.com/react-router/web/api/Redirect) 獲取更多關於可用屬性的信息。
...
- `children`: 渲染函式屬性，返回僅在瀏覽器中運行的 JSX。在 Node.js 中不會執行。
...
當[本地化您的網站](./i18n/i18n-introduction.mdx)時，`<Translate/>` 元件將為 React 元件（例如首頁）提供**翻譯支援**。`<Translate>` 元件支援[插值功能](#interpolate)。
...
`<Translate>` 元件支援插值。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 來實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
用於存取 Docusaurus 上下文的 React 鉤子。上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
客戶端首次渲染輸出（瀏覽器端）**必須與**伺服器端渲染輸出（Node.js 端）**完全一致**。違反此規則可能導致不可預期的水合行為，詳見 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/)。
...
存取特定插件創建的全域資料。根據插件名稱返回該插件所有實例的資料（按插件 ID 分類）。
