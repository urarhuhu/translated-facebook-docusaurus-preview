...
預設情況下，建議將這些資源放在 `static` 資料夾中。您放入**該目錄的每個檔案**都會被複製到生成的 `build` 資料夾的根目錄中，並保留目錄層級結構。例如，如果您在 static 資料夾中添加名為 `sun.jpg` 的檔案，它將被複製到 `build/sun.jpg`。
...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並不理想，因為更改網站 `baseUrl` 會**破壞這些連結**。對於圖片 `<img src="/img/docusaurus.png" />` 在 `https://example.com/test` 提供時，瀏覽器會嘗試從 URL 根目錄解析它，即 `https://example.com/img/docusaurus.png`，這將失敗，因為它實際上是在 `https://example.com/test/img/docusaurus.png` 提供。
...
- 預設情況下，`static` 資料夾中的檔案都不會經過後處理、雜湊或最小化。
  - 但如先前示範，我們通常能將其轉換為 `require` 調用以進行處理。這有利於積極緩存並提升使用者體驗。
- 若透過硬編碼的絕對路徑引用缺失檔案，編譯時不會被偵測到，將導致 404 錯誤。
- GitHub Pages 預設會使用 [Jekyll](https://jekyllrb.com/) 處理發佈檔案。由於 Jekyll 會忽略以 `_` 開頭的檔案，若使用 GitHub Pages 託管，建議在 `static` 目錄添加空檔案 `.nojekyll` 來停用 Jekyll。