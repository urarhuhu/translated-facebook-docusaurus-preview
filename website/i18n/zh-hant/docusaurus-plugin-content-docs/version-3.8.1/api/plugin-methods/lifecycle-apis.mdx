...
宣告式回呼函式，用於建立可稍後作為 props 提供給路由的靜態資料（通常為 JSON 或字串）。接收檔案名稱與要儲存的資料，並回傳實際資料檔案的路徑。
...
此函式允許建立一些可從任何頁面（包括其他插件建立的頁面及主題佈局）讀取的全域插件資料。
...
全域資料是...全域的：其大小會影響網站所有頁面的載入時間，請盡量保持精簡。在可能的情況下，優先使用 `createData` 和頁面特定資料。
...
修改內部 webpack 配置。若返回值為 JavaScript 物件，將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併至最終配置；若為函式，則會呼叫該函式並接收 `config` 作為第一個參數、`isServer` 標記作為第二個參數。
...
`configureWebpack` 會根據客戶端/伺服器端建置傳入生成的 `config`。可將其視為待合併的基礎配置。
...
`configureWebpack` 會與外掛載入的內容一起被呼叫。
