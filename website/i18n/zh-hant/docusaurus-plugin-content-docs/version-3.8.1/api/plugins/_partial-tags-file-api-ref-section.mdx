...
使用 [`tags` 插件選項](#tags) 來設定 YAML 標籤檔案的路徑。

依照慣例，插件會在您的內容資料夾根目錄中尋找 `tags.yml` 檔案。

此檔案可包含預定義的標籤清單。您可透過 [`tags` front matter](#markdown-front-matter) 在 Markdown 檔案中引用這些標籤的鍵值。

:::tip[保持標籤一致性]

使用標籤檔案可確保標籤用法在插件內容集中保持一致。使用 [`onInlineTags: 'throw'`](#onInlineTags) 插件選項來強制執行此一致性，並防止使用臨時宣告的行內標籤。

:::

### 類型 {#tags-file-types}

提供的標籤檔案之 YAML 內容應符合以下結構：
