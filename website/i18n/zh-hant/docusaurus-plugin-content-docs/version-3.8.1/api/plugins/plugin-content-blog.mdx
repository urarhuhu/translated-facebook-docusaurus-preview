...
允許透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化部落格 XML 訂閱源的瀏覽器渲染樣式。

- 設為 `true` 使用內建 `.xsl` 和 `.css` 檔案樣式化訂閱源
...
Markdown 文件可使用以下以 `---` 分隔的 [前言](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 元資料欄位。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。可透過 Markdown 文件的 [`authors` 前言](#markdown-front-matter) 引用這些作者鍵值。
