...
Docusaurus 的預設頁面插件。經典模板預設已配置此插件。該插件提供[建立頁面](guides/creating-pages.mdx)功能。
...
Markdown 頁面可使用以下 Markdown [前置資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)元資料欄位，需以 `---` 分隔線包圍。
