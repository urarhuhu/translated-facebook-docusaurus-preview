...
這會啟用 `@docusaurus` eslint 插件並使用 `recommended` 配置。請參閱下方的[支援規則](#supported-rules)以查看此配置會啟用的規則清單。

### 手動配置

若需要更細緻的控制，你也可以手動啟用插件並直接設定你想使用的規則：
...
- 推薦：適用於大多數 Docusaurus 網站的推薦規則集，應從此處延伸。
- 全部：啟用**所有**規則。這會在次要版本之間變更，因此若你想避免意外的破壞性變更，不應使用此配置。
