...
安裝 Docusaurus 最簡單的方法是使用命令行工具，它能幫助您快速建立一個 Docusaurus 網站的基本骨架。您可以在新的空倉庫或現有倉庫中的任何位置執行此命令，它將創建一個包含骨架文件的新目錄。

```bash
npx create-docusaurus@latest my-website classic
```

我們推薦使用 `classic` 模板，以便快速上手，它包含了 Docusaurus 1 中的功能。`classic` 模板包含 `@docusaurus/preset-classic`，其中提供了標準文檔、博客、自定義頁面和 CSS 框架（支持暗黑模式）。使用經典模板可以極快地啟動並運行，待您對 Docusaurus 更加熟悉後再進行自定義。

您也可以通過傳遞 `--typescript` 標誌使用模板的 TypeScript 變體。詳見 [TypeScript 支持](./typescript-support.mdx) 以獲取更多信息。
...
如果您正在為 Meta 開源項目設置新的 Docusaurus 網站，請在內部倉庫中運行此命令，它會提供一些有用的 Meta 特定默認設置：
...
執行 `npx create-docusaurus@latest --help`，或查看其 [API 文檔](./api/misc/create-docusaurus.mdx) 以獲取所有可用標誌的更多信息。

## 專案結構 {#project-structure}

假設您選擇了經典模板並將網站命名為 `my-website`，您將在新建的 `my-website/` 目錄下看到以下生成的文件：
...
- `/src/` - 非文件類檔案（如頁面或自訂 React 元件）。雖非強制要求，但將非文件檔案集中存放便於後續進行程式碼檢查或處理
...
- `/sidebars.js` - 用於指定文件側邊欄的排序結構

### 單體式倉庫 (Monorepos) {#monorepos}
...
若使用 Netlify 或 Vercel 等託管服務，需將站點的「基礎目錄」變更為 Docusaurus 根目錄位置（本例為 `./website`）。更多忽略指令設定詳情請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。
...
建置完成後，靜態內容將生成於 `/build` 目錄，可部署至 [GitHub Pages](https://pages.github.com/)、[Vercel](https://vercel.com/) 或 [Netlify](https://www.netlify.com/) 等靜態託管服務。詳情請參閱[部署文件](deployment.mdx)。
