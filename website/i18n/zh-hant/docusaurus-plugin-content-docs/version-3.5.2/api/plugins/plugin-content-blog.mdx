...
若使用 `@docusaurus/preset-classic` 預設套件，無需額外安裝此外掛作為依賴。
...
- 設為 `true` 使用外掛內建的 `.xsl` 與 `.css` 檔案樣式化訂閱源
- 設為假值 (`undefined | null | false`) 停用此功能
- 設為 `string` 提供相對於部落格內容目錄的自訂 `.xsl` 檔案路徑。依慣例需提供同名 `.css` 檔案。
...
Markdown 文件可使用以下 Markdown [前置元資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 欄位，需以 `---` 分隔線包覆。
...
使用 [`authors` 外掛選項](#authors) 配置 YAML 作者檔案路徑。

依慣例，外掛會在部落格內容目錄根層級尋找 `authors.yml` 檔案。

此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。可透過 Markdown 檔案中的 [`authors` 前置元資料](#markdown-front-matter) 引用這些作者。
