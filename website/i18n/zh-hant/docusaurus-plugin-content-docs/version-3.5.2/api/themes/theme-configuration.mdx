...
當需要在網站發布公告時，可添加公告欄功能。這是位於導航欄上方、非固定且可選擇性關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可放置於[靜態資源目錄](static-assets.mdx)中。標誌URL預設設為網站根目錄。雖然可自訂標誌URL，但若為外部連結將會在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文檔網站作為主網站子目錄時特別有用，此情況下通常不需要標誌連結在主網站開啟新分頁。

為增強深色模式支援，亦可為此模式設定不同標誌。
...
您可以透過 `themeConfig.navbar.items` 添加項目到導航欄。
...
根據 `type` 欄位的不同，這些項目可以有不同的行為。以下小節將介紹所有可用的導航欄項目類型。

#### 導航欄連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應自動為連結套用活動連結樣式，但在邊緣情況下您可以使用 `activeBasePath`。當連結需要在多個不同路徑下保持活動狀態時（例如當您在同一側邊欄下有多個文檔文件夾），可以使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案，且優先級高於它——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配。
...
除了上述欄位外，您還可以指定其他任意屬性，這些屬性將套用到 HTML 連結上。
...
類型為 `dropdown` 的導航欄項目具有額外的 `items` 欄位，這是一個導航欄項目的內部陣列。
...
- [帶有自訂 HTML 的導航欄](#navbar-with-custom-html)
...
如果您想連結到特定文檔，此特殊導航欄項目類型將渲染指向提供 `docId` 的文檔連結。只要您瀏覽同一側邊欄的文檔，它就會獲得 `navbar__link--active` 類別。
...
若您使用具版本控制的文檔，此特殊導航欄項目類型將渲染一個下拉選單，顯示您站點所有可用版本。

使用者可在不同版本間切換，同時停留在同一份文檔上（只要該文檔ID在不同版本間保持一致）。
...
若您使用具版本控制的文檔，此特殊導航欄項目類型會連結至當前瀏覽的文檔版本（根據URL判斷），並自動回退至最新版本。
...
若您使用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導航欄項目類型會渲染一個下拉選單，顯示您站點所有可用語言選項。
...
您亦可使用此導航欄項目類型，在導航欄項目中渲染自訂HTML標記。
...
您可以啟用此精緻的UI功能：當使用者向下滾動頁面時自動隱藏導航欄，向上滾動時再次顯示。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現代碼區塊語法高亮。所有配置均位於`prism`物件中。
...
如果您使用行高亮 Markdown 語法，可能需要為深色模式語法高亮主題指定不同的高亮背景色。請參考[文件指南](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 預設語言 {#default-language}

如果程式碼區塊開頭的三重反引號（即 ```）後未指定語言，您可以設定預設語言。請注意，必須傳遞有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
您可以透過 `themeConfig.footer` 在頁尾新增標誌和版權聲明。標誌可放置在[靜態資料夾](static-assets.mdx)中。標誌 URL 的運作方式與導覽列標誌相同。
...
您可以透過 `themeConfig.footer.links` 在頁尾新增連結。頁尾設定有兩種型態：**多欄頁尾**和**簡易頁尾**。
