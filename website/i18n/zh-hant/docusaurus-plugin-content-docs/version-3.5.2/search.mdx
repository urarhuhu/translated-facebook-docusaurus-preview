...
該服務對所有開發者文件或技術部落格**免費**：請務必閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請 DocSearch 計畫](https://docsearch.algolia.com/apply)。
...
申請核准並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 加入專案。可透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取作業。索引在部署後立即可用，因此通常無需手動配置。
...
Docusaurus 自身的 `@docusaurus/preset-classic` 支援 Algolia DocSearch 整合。若使用經典預設配置，則無需額外安裝。
...
若搜尋功能在重大變更後失效，請使用 Algolia 控制台 **手動觸發重新爬取**。
...
為解決此問題，情境化搜尋會動態識別您正在瀏覽的文件版本與語言，並自動建立對應的搜尋篩選條件。

- 在 `/en/docs/v1/myDoc` 頁面，搜尋結果僅包含 **英語** 的 **v1** 文件（及其他未版本化的頁面）
- 在 `/fr/docs/v2/myDoc` 頁面，搜尋結果僅包含 **法語** 的 **v2** 文件（及其他未版本化的頁面）
...
若需直接修改 Algolia 搜尋的 React 元件，可對 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 元件進行 [替換](swizzling.mdx)：
...
- [docusaurus-theme-search-typesense](https://github.com/typesense/docusaurus-theme-search-typesense) - 搜尋列 UI 元件，可整合至網站
...
若搜尋索引規模較小，可讓使用者在瀏覽網站時下載至其瀏覽器，此時適合採用本地搜尋外掛。

您可在[社群資源清單](https://docusaurus.io/community/resources#search)中找到社群支援的本地搜尋外掛。

## 👥 使用自訂搜尋 {#using-your-own-search}

若要使用自訂搜尋，請對 `@docusaurus/theme-classic` 中的 `SearchBar` 元件進行 swizzle 操作

```bash npm2yarn
npm run swizzle @docusaurus/theme-classic SearchBar
```

這會在專案資料夾中建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯該元件後，您將看到 Docusaurus 已改用您的自訂 `SearchBar` 元件。
