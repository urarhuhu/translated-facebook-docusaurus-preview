...
地區名稱用於翻譯檔案的存放位置，以及翻譯後語言環境的基礎 URL。當建置所有語言環境時，只有預設語言環境的名稱會從基礎 URL 中省略。
...
這對於在您的伺服器上實作自動語言環境偵測非常有用。例如，您可以使用此參數將使用者偏好的語言環境儲存在 cookie 中。
...
JSX 元素是_物件_而非字串。在預期字串的上下文中使用（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會[強制轉換為字串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，導致返回 `"[object Object]"`。雖然我們鼓勵將 `<Translate>` 作為 JSX 子元素使用，但請僅在確實可行的情境下使用元素形式。
...
`docusaurus write-translations` 指令會靜態分析您站點中使用的所有 React 程式碼文件，提取這些 API 的調用，並將其聚合到 `code.json` 文件中。翻譯文件將儲存為從 ID 到翻譯訊息物件（包含已翻譯標籤和標籤描述）的映射。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定預設未翻譯訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯。

:::warning[文字標籤必須是靜態的]

`docusaurus write-translations` 指令僅對您的程式碼進行**靜態分析**，不會實際運行您的站點。因此，動態訊息無法被提取，因為訊息是_表達式_而非_字串_：
...
此代碼在運行時仍能正確運作。然而，未來我們可能提供「無運行時」機制，透過 Babel 轉換將翻譯直接內嵌到 React 代碼中，而非在運行時調用 API。因此，為確保未來相容性，您應始終偏好靜態可分析的訊息。例如，我們可將上述代碼重構為：
...
該命令會提取並初始化需要翻譯的 JSON 翻譯文件。根目錄下的 `code.json` 文件包含從原始碼中提取的所有翻譯 API 呼叫，這些可能是您編寫的或由主題提供的，其中部分可能已經預設翻譯完成。
...
靜態託管服務通常會將 `/unknown/url` 重新導向至 `/404.html`，這會導致 **英文的 404 頁面** 被顯示。
...
也可以將每個語言版本部署為獨立的子網域，並在 CDN 層級將這些子網域整合到一個統一的網域下：
