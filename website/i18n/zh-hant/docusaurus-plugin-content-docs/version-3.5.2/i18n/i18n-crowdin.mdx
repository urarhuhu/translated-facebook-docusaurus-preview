...
Docusaurus 的 i18n 系統**與任何翻譯軟體解耦**。
...
- 使用 Crowdin **翻譯內容**
- **從 Crowdin 下載翻譯**（本地化的翻譯文件）

Crowdin 提供一個 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳來源文件**和**下載翻譯**，讓您可以自動化翻譯流程。

[`crowdin.yml` 設定文件](https://support.crowdin.com/configuration-file/) 對 Docusaurus 很方便，並允許**將本地化的翻譯文件下載到預期的位置**（在 `i18n/[locale]/..` 中）。

閱讀 **[官方文件](https://support.crowdin.com/)** 以了解更多關於進階功能和不同翻譯工作流程的資訊。

## Crowdin 教學 {#crowdin-tutorial}

這是一個使用 Crowdin 將新初始化的英文 Docusaurus 網站翻譯成法文的逐步指南，假設您已經遵循了 [i18n 教學](./i18n-tutorial.mdx)。
...
### 創建 Crowdin 專案 {#create-a-crowdin-project}

在 [Crowdin](https://crowdin.com/) 上註冊，並創建一個專案。

使用英文作為來源語言，法文作為目標語言。

![創建一個 Crowdin 專案，以英文為來源語言，法文為目標語言](/img/crowdin/crowdin-create-project.png)

您的專案已創建，但目前是空的。我們將在接下來的步驟中上傳要翻譯的文件。

### 創建 Crowdin 設定 {#create-the-crowdin-configuration}
...
將 Crowdin CLI 作為 npm 套件安裝到 Docusaurus 專案：
...
**[預先翻譯](https://support.crowdin.com/pre-translate-via-machine/)**您的網站，並**手動修正預先翻譯的錯誤**（請先在設定中啟用全域翻譯記憶庫）。
...
Crowdin 不支援多個同時上傳/下載：建議僅在正式部署中包含翻譯，並保持部署預覽未翻譯狀態。
...
Crowdin 會將 JSX 語法視為嵌入式 HTML，並可能在下載翻譯時破壞 JSX 標記，導致網站因無效 JSX 而無法建置。
...
我們建議將複雜的嵌入式 JSX 程式碼提取為獨立的獨立元件。我們還添加了 `mdx-code-block` 逃逸語法：
...
當建立新版本時，來源字串通常與當前版本（`website/docs`）非常相似，您不希望一次又一次地翻譯新版本的文件。

Crowdin 提供了 `Duplicate Strings` 設定。

![Crowdin 重複字串選項設定](/img/crowdin/crowdin-settings-duplicate-strings.png)

我們建議使用 `Hide`，但理想的設定取決於您的版本差異程度。
...
如果您有一個 `id=ios` 的文件插件實例，您還需要配置這些來源檔案
...
有時，您會在 Git 上**刪除或重新命名來源檔案**，Crowdin 將顯示 CLI 警告：

![Crowdin CLI：下載翻譯警告](/img/crowdin/crowdin-download-translations-warning.png)

當您的來源被重構時，您應該使用 Crowdin UI **手動更新您的 Crowdin 檔案**：

![Crowdin UI：重新命名檔案](/img/crowdin/crowdin-files-rename.png)

### VCS (Git) 整合 {#vcs-git-integrations}

Crowdin 提供多種 VCS 整合，適用於 [GitHub](https://support.crowdin.com/github-integration/)、GitLab、Bitbucket。
...
原本希望能同時在 Git 和 Crowdin 上編輯翻譯，並在兩個系統之間實現**雙向同步**。

實際上，**它的運作並不太可靠**，原因如下：

- Crowdin -> Git 同步運作良好（透過 pull request）
- Git -> Crowdin 同步是手動的（您必須按下按鈕）
- Crowdin 用於匹配現有 Markdown 翻譯與現有 Markdown 來源的啟發式方法並非 100% 可靠，您必須在每次從 Git 同步後在 Crowdin UI 上驗證結果
...
不幸的是，由於技術原因，它目前無法運作，但我們有希望可以解決。

Crowdin 將 Markdown 字串替換為技術 ID，例如 `crowdin:id12345`，但替換過於激進，包括隱藏字串，並會破壞 front matter、admonitions、JSX...
...
您可能更傾向讓編輯按鈕連結至 Crowdin 介面，可透過 `editUrl` 函數根據不同語系自訂編輯網址。
...
Crowdin 近期對 Markdown 檔案格式進行重大變更，連結的處理方式與以往不同。過去連結被視為標籤，現在則顯示為純文字。由於這些變更，純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，導致翻譯錯誤（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被翻譯為 `Check out [my wonderful page](/my-wonderful-page)`，這會破壞 Docusaurus 的國際化工作流程，因為頁面名稱不應被翻譯）。

截至 2023 年 12 月 7 日，Crowdin 無意變更連結的處理邏輯，因此若您計劃搭配使用 Crowdin 與機器翻譯，請務必注意此問題。