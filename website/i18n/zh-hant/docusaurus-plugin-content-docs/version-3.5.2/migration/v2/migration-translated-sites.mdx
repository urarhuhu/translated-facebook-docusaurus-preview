...
MDX 將 Markdown 檔案編譯為 React 組件，比 Docusaurus v1 的解析器更嚴格，並會在出現錯誤時使構建失敗，而不是渲染一些不良內容。
...
為了避免任何破壞您 v1 生產網站的風險，一種可能的策略是複製原始的 v1 Crowdin 專案。
...
你需要修復所有錯誤直到網站能成功建置。可以透過修改本地翻譯的MD檔案，並使用`docusaurus build --locale fr`一次修復一個語系的網站。

我們無法提供修復這些錯誤的終極指南，但常見錯誤原因包括：

- 未在Crowdin將足夠字串標記為「隱藏字串」，導致預翻譯嘗試翻譯這些字串。
...
你可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並限制僅預翻譯部分語言/檔案。
...
Crowdin的Markdown解析器會隨時間演進，每個Crowdin專案的解析器版本可能不同，這會導致預翻譯無法完整預翻譯所有字串。
...
Crowdin有「上傳翻譯」功能，但根據我們的經驗，對Markdown的效果並不理想
...
如此一來，你無需建立新Crowdin專案、轉移翻譯記憶檔、套用預翻譯並嘗試修復預翻譯錯誤。

你可以為Docusaurus v2建立Crowdin分支，上傳v2源文件，並在準備完成後將分支合併至主線。
