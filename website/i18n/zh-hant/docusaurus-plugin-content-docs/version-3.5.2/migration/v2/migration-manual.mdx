...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)完成後執行，以補足缺失部分或除錯遷移 CLI 輸出的問題。
...
CLI 指令現更名為 `docusaurus <command>`（取代原先的 `docusaurus-command`）。

您的 `package.json` 中 `"scripts"` 區段應更新如下：
...
Docusaurus 2 已將其移至 `website/build`。請確保更新部署配置以從正確的 `build` 目錄讀取生成檔案。
...
D1 站台可能已有 README 檔案。您可修改其內容以反映 D2 變更，或複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集為插件組合包，為保持向後相容性，我們建立了 `@docusaurus/preset-classic` 預設集，其中包含 Docusaurus 1 的大部分核心插件。

將以下預設集配置加入您的 `docusaurus.config.js`：
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完備，後續會更新此處說明。要覆蓋 Infima 的 CSS 變數，請創建自己的 CSS 文件（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 為您選擇的主色調找出合適的漸變色。

或者使用以下工具生成網站所需的漸變色，並將變數複製到 `src/css/custom.css` 中。
...
網站元信息（如資源、SEO、版權信息）現在由主題處理。要自定義這些內容，請在 `docusaurus.config.js` 中使用 `themeConfig` 欄位：
...
可聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們能協助更新配置並觸發網站重新爬取以恢復搜索功能（否則您需要等待最多 24 小時直到下次預定爬取）
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 檔案，並填入您的自訂網域。執行建置命令時，`static` 資料夾中的檔案將被複製到 `build` 資料夾的根目錄下。
...
- `defaultVersionShown` - 版本控制功能尚未移植。如果您使用版本控制，目前無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
舊版本不允許嵌套側邊欄分類，且分類僅能包含文件 ID。但 v2 版本支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不限於文件）。
...
v2 版本中需在每個頁面套用主題佈局元件。Layout 元件會接收 metadata props。
...
Docusaurus 2 採用 [MDX](https://mdxjs.com/) 語法，需更新現有文件中可能失效的語法。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須是有效的 JSX。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若您的 front matter 包含特殊字符（如 `:`），現在需要加上引號：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。
...
GitHub Pages 使用的 `CNAME` 檔案不再自動產生，若使用自訂網域，請確保已建立 `/static/CNAME` 檔案。

部落格 RSS 訂閱位址現改為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以維持使用者訂閱有效性。
