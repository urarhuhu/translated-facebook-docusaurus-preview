...
此組件支持鏈接到內部頁面，以及一個稱為預加載的強大性能功能。預加載用於預取資源，以便用戶通過此組件導航時資源已經獲取。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時獲取低優先級請求，然後使用 `onMouseOver` 事件在用戶可能導航到請求的資源時觸發高優先級請求。
...
`<BrowserOnly>` 元件允許在 React 應用程式水合(hydrate)後，僅在瀏覽器中渲染 React 元件。
...
- `children`: 渲染函數 prop，返回僅在瀏覽器中執行的 JSX。在 Node.js 中不會執行
...
用於存取 Docusaurus 上下文的 React Hook。此上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
客戶端首次渲染輸出（瀏覽器中）**必須完全匹配**伺服器端渲染輸出（Node.js）。違反此規則可能導致非預期的水合行為，詳見 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/)。
...
存取特定插件建立的全域資料。根據插件名稱，返回該插件所有實例的資料（按插件 ID 分類）。
...
React 渲染邏輯請改用 [`useIsBrowser()`](#useIsBrowser) 或 [`<BrowserOnly>`](#browseronly)。
