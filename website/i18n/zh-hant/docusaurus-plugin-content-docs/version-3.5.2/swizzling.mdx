...
若要彈出主題元件，請使用互動式 swizzle CLI 或搭配 `--eject` 參數：
...
**建議盡可能採用[包裝模式](#wrapping)**：需維護的程式碼量較少。

:::

:::tip[重新混寫]

在 Docusaurus 升級後，若要保持抽離的元件處於最新狀態，請重新執行抽離命令並使用 `git diff` 比較變更。建議在檔案頂部寫下簡短註解說明您所做的修改，以便在重新抽離後能更輕鬆地重新套用您的變更。

:::

### 封裝 {#wrapping}

封裝主題元件是指**建立一個包裝器**圍繞原始主題元件，您可以對其進行**增強**。
...
這將在您網站的 `src/theme` 目錄中建立一個封裝器。Docusaurus 現在將使用 `<FooterWrapper>` 元件而非原始元件。您現在可以在原始元件周圍添加自訂內容。
...
封裝主題是一種**在現有元件周圍添加額外元件**的絕佳方式，而無需[抽離](#ejecting)它。例如，您可以輕鬆在每篇部落格文章下方添加自訂評論系統：
...
- **不安全**：此元件是主題實作細節，不建議混寫，且在主題**次要版本**內可能發生破壞性變更
...
不必過於**害怕混寫不安全元件**：只需記住**破壞性變更**可能發生，且您可能需要在次要版本升級時手動更新您的自訂內容。
...
要達成預期效果，有時並不清楚該混寫哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 約有[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
2. **元件名稱**：官方主題元件採用語義化命名，通常可從名稱推斷功能。Swizzle CLI 支援輸入元件名稱片段來篩選選項。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高階元件開始**：元件會形成樹狀結構，某些元件會導入其他元件。每條路由都會關聯一個頂層元件（多數列於[內容外掛的路由系統](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。可先從此元件開始 swizzle，再向下追蹤元件樹找到目標元件。找到正確元件後，請刪除其他已 swizzle 的檔案以避免維護過多元件。
...
Swizzle 本質上意味著您需要維護與 Docusaurus 內部 API 互動的 React 程式碼。在客製化網站時，請優先考慮以下替代方案：

1. **使用 CSS**：CSS 規則和選擇器通常能實現相當程度的客製化。詳見[樣式與佈局](./styling-layout.mdx)。
2. **使用翻譯功能**：雖然看似意外，但翻譯本質上也是一種文字標籤客製化方式。例如若網站預設語言為 `en`，仍可執行 `yarn write-translations -l en` 後編輯生成的 `code.json`。詳見[國際化教學](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**。若必須 swizzle，請僅針對相關部分進行，並盡量減少自行維護的程式碼量。Swizzle 小型元件通常能降低升級時的**破壞性變更**風險。

相較於[完整替換(ejecting)](#ejecting)，[封裝(wrapping)](#wrapping)是更安全的選擇。

:::

## 使用 `<Root>` 封裝您的網站 {#wrapper-your-site-with-root}

`<Root>` 元件會渲染在 React 樹的最頂層（高於主題的 `<Layout>`），且**永遠不會卸載**。非常適合用於添加不應在導航時重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
