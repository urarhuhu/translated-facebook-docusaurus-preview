...
最流行的格式化工具 Prettier [僅支援舊版 MDX v1](https://github.com/prettier/prettier/issues/12209)。若遇到非預期的格式化結果，您可以在問題區域前添加 `{/* prettier-ignore */}`，或將 `*.mdx` 加入 `.prettierignore` 中，直到 Prettier 完整支援 MDX v3。[MDX 的主要作者之一推薦使用 `remark-cli` 搭配 `remark-mdx`](https://github.com/orgs/mdx-js/discussions/2067)。

:::

### 匯出元件 {#exporting-components}

要在 MDX 文件中定義自訂元件，您必須將其匯出：只有以 `export` 開頭的段落才會被解析為元件而非普通文字。
...
雖然在 Markdown 中宣告元件對於簡單案例非常方便，但由於編輯器支援有限、解析錯誤風險和低可重用性，維護起來會變得困難。當元件涉及複雜的 JS 邏輯時，請使用獨立的 `.js` 文件：
...
若在多個文件中使用相同元件，您無需在每個文件中匯入它——考慮將其加入全域作用域。[參見下方](#mdx-component-scope)
...
若要註冊額外標籤名稱（如上述 `<Highlight>` 標籤），建議[封裝 `@theme/MDXComponents`](../../swizzling.mdx#wrapping)，如此便無需維護所有現有映射。由於替換 CLI 尚不支援封裝非元件檔案，您需手動建立封裝器：
...
從 MDX v3+ 開始（Docusaurus v3+），小寫標籤名稱一律會渲染為原生 html 元素，且不會使用您提供的任何元件映射。
...
若未用 `MDXContent` 封裝匯入的 MDX，全域作用域將無法使用。
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)大多相容於[CommonMark](https://commonmark.org/)，但更為嚴格，因為您的 `.mdx` 檔案可使用 JSX 並會被編譯為真實 React 元件（參閱[遊樂場](https://mdxjs.com/playground/)）。
...
此功能為**實驗性**，目前有若干[限制](https://github.com/facebook/docusaurus/issues/9092)。
...
- `contentTitle`：Markdown 標題，即 Markdown 文本中的第一個 `h1` 標題。如果沒有（例如，前端元數據中指定的標題），則為 `undefined`。
