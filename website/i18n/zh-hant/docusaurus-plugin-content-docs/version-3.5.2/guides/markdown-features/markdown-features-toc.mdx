...
自動生成的標題 ID 在單一頁面中能保證唯一性，但若使用自訂 ID，請確保每個 ID 在頁面中只出現一次，否則會導致 HTML 語義錯誤，且其中一個標題將無法被連結。

:::

## 目錄標題層級 {#table-of-contents-heading-level}

每份 Markdown 文件右上角會顯示目錄。預設僅顯示 h2 和 h3 級別的標題，這通常足以呈現頁面結構。如需調整顯示的標題層級範圍，您可以針對單一頁面或全局進行設定。
...
若要為所有頁面設定標題層級，請使用 [`themeConfig.tableOfContents`](../../api/themes/theme-configuration.mdx#table-of-contents) 配置項。
...
`themeConfig` 選項會影響網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），而 front matter 選項僅影響右上角的目錄。如需自訂每個 `<TOCInline />` 組件，需使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性。
...
MDX 文件中可透過 `toc` 變量獲取所有標題資訊。預設僅顯示 `h2` 和 `h3` 級別標題。您可透過為個別 `TOCInline` 組件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示層級。
...
請注意 `toc` 是扁平結構，因此您可以輕鬆過濾不需要的節點或插入額外節點，重新構建目錄樹。
...
目錄是透過 [Remark 插件](./markdown-features-plugins.mdx) 解析 Markdown 原始碼生成的。在某些邊緣情況下，可能會產生誤判或漏判。
...
非 Markdown 標題不會出現在目錄中。這可以巧妙運用來解決上述問題。
