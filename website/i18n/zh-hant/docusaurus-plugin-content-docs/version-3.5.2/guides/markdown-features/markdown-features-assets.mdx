...
有時您會希望直接從 Markdown 文件連結靜態資源（如 docx 文件、圖片等），此時將資源與使用它的 Markdown 文件放在同一目錄下會非常方便。
...
您可以用三種方式顯示圖片：Markdown 語法、CommonJS require 或 ES 導入語法。
...
如果您使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為文件路徑並自動轉換為 `require()` 調用。除非您使用 JSX 語法（此時需自行處理），否則無需在 Markdown 中使用 `require()`。
...
這在您希望通過 CSS 修改 SVG 部分內容時非常有用。例如，您可以根據當前主題更改 SVG 的某種顏色。
...
Docusaurus 支持主題化圖片：內置於主題中的 `ThemedImage` 組件可讓您根據當前主題切換圖片來源。
...
要使用路徑片段（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切換圖片可見性，請將以下代碼添加到自定義 CSS 中（如果不希望與 GitHub 耦合，也可使用自定義後綴）：
...
如果 Markdown 連結或圖片使用絕對路徑，該路徑將被視為文件路徑，並從靜態目錄解析。例如，若您配置[靜態目錄](../../static-assets.mdx)為 `['public', 'static']`，則對於以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。連結將被轉換為 `require()` 調用而非保留為 URL。這種處理方式有兩個主要優勢：
