...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[ admonition 語法](./markdown-features-admonitions.mdx)也是透過 Remark 插件生成，您可依此模式實現自訂需求。
...
以下說明並非建立外掛程式的完整指南，僅為示範如何使其在 Docusaurus 中運作。如需更深入的運作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 官方文件。

:::

舉例來說，我們建立一個外掛程式，為每個 `h2` 標題加上 `第 X 節. ` 前綴。首先，在任何位置建立外掛程式原始檔（亦可發佈為獨立 npm 套件並依前述方式安裝）。我們將外掛程式置於 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是接收 `options` 並返回操作 AST 的 `transformer` 函式。
...
例如我們的 `transformImage` 外掛程式便利用此參數，將相對圖片參照轉換為 `require()` 呼叫。
