...
靜態資源是指直接複製到建置輸出目錄的非程式碼文件，包括圖片、樣式表、網站圖示、字型等。

預設建議將這些資源放置在 `static` 資料夾中。您放入該目錄的每個文件都會按照原有目錄結構被複製到生成的 `build` 資料夾根目錄下。例如，若您添加名為 `sun.jpg` 的文件至 static 資料夾，它將被複製為 `build/sun.jpg`。
...
現在，`public` 和 `static` 目錄中的所有文件都將被複製到建置輸出中。

## 引用靜態資源 {#referencing-your-static-asset}

### 在 JSX 中 {#in-jsx}

在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並非理想做法，因為變更網站 `baseUrl` 會導致這些連結失效。例如圖片 `<img src="/img/docusaurus.png" />` 在 `https://example.com/test` 下提供時，瀏覽器會嘗試從 URL 根目錄解析，即 `https://example.com/img/docusaurus.png`，這將失敗，因為實際提供路徑應為 `https://example.com/test/img/docusaurus.png`。
...
您也可以導入 SVG 文件：它們將被轉換為 React 元件。
...
關鍵結論：切勿硬編碼您的基礎 URL！基礎 URL 被視為實作細節，應可輕鬆變更。所有路徑（即使它們看似 URL 段）實際都是文件路徑。

若您更習慣 URL 段思維模型，可遵循以下經驗法則：

- 撰寫 JSX 時，假設您的基礎 URL 為 `/test/`，這樣您就不會使用 `src="/img/thumbnail.png"` 這類絕對 URL 路徑，而是透過 `require` 引用資源。
- 撰寫 Markdown 或 CSS 時，假設基礎 URL 為 `/`，這樣您始終使用不含基礎 URL 的絕對路徑。
...
- 預設情況下，`static` 資料夾中的檔案都不會經過後處理、雜湊或最小化。
  - 但如前述範例所示，我們通常能將其轉換為 `require` 調用以進行處理。這有利於實現積極緩存並提升使用者體驗。
