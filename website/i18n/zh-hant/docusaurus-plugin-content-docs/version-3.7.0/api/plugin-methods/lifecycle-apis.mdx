...
修改內部 webpack 配置。若返回值為 JavaScript 物件，將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併至最終配置；若為函式，則會呼叫該函式並接收 `config` 作為第一參數、`isServer` 旗標作為第二參數。

:::warning

`configureWebpack` API 未來將改為接收物件形式 (`configureWebpack({config, isServer, utils, content})`)
