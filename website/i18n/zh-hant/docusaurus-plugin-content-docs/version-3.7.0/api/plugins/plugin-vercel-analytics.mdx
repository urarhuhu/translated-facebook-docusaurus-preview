...
[Vercel Analytics](https://vercel.com/docs/analytics) 提供全面的網站訪客洞察，追蹤熱門頁面、流量來源以及地理位置、作業系統和瀏覽器等人口統計資料。

:::warning[僅限正式環境]

此插件在開發環境中始終處於非活躍狀態，**僅在正式環境** (`docusaurus build`) 中啟用，以避免污染分析統計數據。
