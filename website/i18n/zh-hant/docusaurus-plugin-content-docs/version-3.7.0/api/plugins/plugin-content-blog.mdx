...
Markdown 文件可使用以下 [前置元數據](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 欄位，需以 `---` 分隔線包夾。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。透過 Markdown 文件中的 [`authors` 前置元數據](#markdown-front-matter) 可引用這些作者的鍵值。
