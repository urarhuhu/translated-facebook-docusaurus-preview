...
此檔案可包含預定義標籤列表。您可以在 Markdown 檔案中透過 [`tags` 前置資料](#markdown-front-matter) 來引用這些標籤的鍵值。

:::tip[保持標籤一致性]

使用標籤檔案可確保標籤在插件內容集中的使用保持一致。使用 [`onInlineTags: 'throw'`](#onInlineTags) 插件選項來強制執行此一致性，並防止使用臨時宣告的行內標籤。
