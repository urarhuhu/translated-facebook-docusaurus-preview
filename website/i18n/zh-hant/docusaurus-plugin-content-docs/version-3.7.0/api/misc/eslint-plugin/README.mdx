...
[ESLint](https://eslint.org/) 是一個靜態分析程式碼的工具，能透過編輯器提示與命令列回報問題或建議最佳實踐。Docusaurus 提供 ESLint 插件來強制執行 Docusaurus 的最佳實踐。
...
這會啟用 `@docusaurus` eslint 插件並採用 `recommended` 配置。下方[支援的規則](#supported-rules)列出了此配置會啟用的規則清單。
...
- 推薦配置：適用於大多數 Docusaurus 網站的推薦規則集，應作為基礎擴展。
- 完整配置：啟用**所有**規則。此配置會隨次要版本變更，若想避免意外重大變更應避免使用。
