...
預設值通常運作良好。不幸的是，每個靜態託管提供者都有**不同的行為**，將相同的網站部署到不同的主機可能會導致不同的結果。根據您的主機，更改此配置可能會有所幫助。
...
沒有一個萬能的解決方案。您需要根據需求和資源來權衡後做出選擇。

## 自行託管 {#self-hosting}

Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令進行自行託管。透過 `--port` 參數變更連接埠，並使用 `--host` 參數變更主機位址。
...
相較於靜態託管服務商/CDN，此並非最佳選擇。

:::

:::warning

以下章節將介紹常見託管服務商及其高效部署 Docusaurus 網站的配置方式。Docusaurus 與這些服務無商業合作關係，此處資訊僅供參考。部分說明由第三方提供，最新API變動可能未即時更新。若發現內容過時，歡迎提交PR修正。

由於我們僅能盡力維護現有內容，目前已停止接受新增託管選項的PR。您可將部署指南發布於其他平台（如個人部落格或服務商官網），並申請將連結收錄至本文件。
...
站點設定時，請依下列規格指定建置指令與發佈目錄：

- 建置指令：`npm run build`
- 發佈目錄：`build`

若未預先設定建置選項，仍可於站點建立後至「Site settings」→「Build & deploy」進行調整。

完成上述配置後，網站將自動部署，並在程式碼合併至預設部署分支（通常為 `main`）時觸發重新部署。
...
建議停用 Netlify 設定項 `Post Processing > Asset Optimization > Pretty Urls` 以避免網址強制小寫、非必要重新導向及404錯誤。

**請特別注意**：全局選項 `Disable asset optimization` 實際無法正確關閉 `Pretty URLs` 功能，務必手動獨立取消勾選該項目。

若需保留 Netlify 的 `Pretty Urls` 功能，請相應調整 Docusaurus 的 `trailingSlash` 配置。
...
透過 [導入流程](https://vercel.com/import/git) 將專案導入 Vercel。在導入過程中，您會發現所有相關選項都已預先配置好；不過，您仍可選擇變更任何這些 [選項](https://vercel.com/docs/build-step#build-&-development-settings)。

專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）所做的任何變更，都會觸發 [生產部署](https://vercel.com/docs/platform/deployments#production)。

## 部署至 GitHub Pages {#deploying-to-github-pages}

Docusaurus 提供了一個簡單的方法來發布到 [GitHub Pages](https://pages.github.com/)，這項服務免費提供給每個 GitHub 儲存庫使用。

### 概覽 {#github-pages-overview}

通常，發布流程會涉及兩個儲存庫（至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務的建置輸出的分支。在接下來的教學中，它們將分別被稱為 **「來源」** 和 **「部署」**。

每個 GitHub 儲存庫都與一項 GitHub Pages 服務相關聯。如果部署儲存庫名為 `my-org/my-project`（其中 `my-org` 是組織名稱或用戶名），部署後的網站將出現在 `https://my-org.github.io/my-project/`。如果部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），網站將出現在 `https://my-org.github.io/`。

:::info

如果您想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄中的任何內容都會被複製到 `build` 目錄的根目錄以進行部署。使用自訂網域時，您應該可以從 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設定為您的自訂網域。

更多詳細資訊，請參閱 GitHub Pages 的文件 [用戶、組織和專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常是 `master` 或 `main`）或 `gh-pages` 分支，以及從根目錄或 `/docs` 資料夾中擷取準備好部署的檔案（`docusaurus build` 的輸出）。您可以透過儲存庫中的 `Settings > Pages` 來配置這一點。這個分支將被稱為「部署分支」。

我們提供了一個 `docusaurus deploy` 指令，幫助您透過一個指令將網站從來源分支部署到部署分支：克隆、建置和提交。
...
這些欄位也有其對應的環境變數，且具有更高的優先級：`ORGANIZATION_NAME`、`PROJECT_NAME` 和 `DEPLOYMENT_BRANCH`。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 執行發布的檔案。由於 Jekyll 會丟棄任何以 `_` 開頭的檔案，建議您透過在 `static` 目錄中添加一個名為 `.nojekyll` 的空檔案來停用 Jekyll。
...
1. 當有新的拉取請求提交至 `main` 分支時，會有一個動作確保網站能成功建置，但不會實際部署。此工作稱為 `test-deploy`。
2. 當拉取請求合併至 `main` 分支或有人直接推送至 `main` 分支時，將會建置並部署至 GitHub Pages。此工作稱為 `deploy`。

以下是兩種使用 GitHub Actions 部署文件的方法。根據您的部署儲存庫位置，請選擇下方相關的標籤：

- 原始碼儲存庫與部署儲存庫為**同一個**儲存庫。
- 部署儲存庫是一個**遠端**儲存庫，與原始碼儲存庫不同。此情境的說明假設 [發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 為 `gh-pages` 分支。
...
跨儲存庫發佈的設定較為複雜，因為您需要具備權限才能推送至另一個儲存庫。我們將使用 SSH 進行身份驗證。
...
2. 預設情況下，您的公鑰應已建立於 `~/.ssh/id_rsa.pub`；否則，請使用上一步驟中提供的名稱將金鑰新增至 [GitHub 部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/)。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其貼上為部署儲存庫中的 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys)。如果命令列無法運作，請直接複製檔案內容。在儲存部署金鑰前，勾選 `允許寫入存取` 的選項。
...
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私鑰，並在來源儲存庫中貼上一個名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令列無法運作，請直接複製檔案內容。儲存您的秘密。
6. 在 `.github/workflows/` 目錄中建立您的 [文件工作流程](https://docs.github.com/en/actions/use-cases-and-examples/creating-an-example-workflow)。在此範例中，檔案名為 `deploy.yml`。

此時，您應該具備：

- 來源儲存庫已設定 GitHub 工作流程，並將私鑰 SSH 金鑰設為 GitHub 秘密，以及
- 部署儲存庫已在 GitHub 部署金鑰中設定公鑰 SSH 金鑰。
...
持續整合（CI）服務通常用於在每次新提交檢入原始碼控制時執行例行任務。這些任務可以是執行單元測試和整合測試、自動化建置、將套件發佈到 npm，以及將變更部署到您的網站。要自動化網站的部署，您只需在網站更新時呼叫 `yarn deploy` 指令碼即可。以下部分介紹如何使用 [Travis CI](https://travis-ci.com/)（一個流行的持續整合服務提供商）來實現這一點。

1. 前往 https://github.com/settings/tokens 並生成一個新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。在建立權杖時，授予其 `repo` 範圍，以確保其擁有必要的權限。
...
4. 建立一個名為 `GH_TOKEN` 的新環境變數，其值為您新生成的權杖，然後新增 `GH_EMAIL`（您的電子郵件地址）和 `GH_NAME`（您的 GitHub 使用者名稱）。
...
現在，每當有新提交進入 `main` 分支時，Travis CI 將執行您的測試套件，如果一切順利，您的網站將透過 `yarn deploy` 指令碼進行部署。
...
按照以下步驟建立一個管道，以便在您將變更推送到專案的選定分支時自動部署新版本的網站：
...
現在，每當您推送新標籤至 GitHub 時，此觸發將啟動 drone CI 作業以發佈您的網站。
...
3. 設定建置選項。
   ...
   - 如有需要，可以修改建置指令。您可以輸入多個指令，以 `&&` 分隔。

4. 部署。

   - 點擊 **部署** 以開始建置流程。

   - 開始後，您將進入部署日誌。如果程式碼有任何問題，日誌中會顯示警告或錯誤訊息，說明問題原因。通常日誌會包含所有您需要的除錯資料。
...
您可以將 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個針對靜態網站、單頁應用程式 (SPA) 和無伺服器函數的部署平台。詳細說明請參閱此 [指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項服務，可直接從代碼存儲庫自動構建並部署全端 Web 應用至 Azure，簡化了 CI/CD 的開發者體驗。Static Web Apps 將 Web 應用程序的靜態資源與其動態（API）端點分開。靜態資源從全球分佈的內容服務器提供，使客戶端能夠透過附近的服務器更快地檢索文件。動態 API 使用基於無服務器架構的事件驅動函數方法進行擴展，這種方法更具成本效益並可根據需求擴展。只需幾分鐘即可開始，請遵循[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)。
