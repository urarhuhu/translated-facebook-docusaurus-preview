...
- **模組化**：允許插件作者提供國際化支援
- **低運行時開銷**：文件主要是靜態的，不需要繁重的 JS 函式庫或 polyfill
...
- **良好的 SEO 預設值**：我們為您設置了有用的 SEO 標頭，如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
...
部分插件會從外部資料檔案讀取內容，這些檔案需整體本地化。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/`路徑下建立副本進行翻譯。

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需建立在正確的檔案系統路徑下。

每個語系與插件都有專屬的`i18n`子資料夾：
...
為簡易法文版Docusaurus網站進行翻譯後，目錄結構如下：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應資料夾下管理自有翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤。

每個內容插件或主題的**翻譯檔案存放位置定義不同**：
