...
若每個文檔實例體量龐大，建議建立兩個獨立的 Docusaurus 站點。

當有人編輯 iOS 文檔時，重建包含未變更的完整 Android 文檔是否真的必要？

:::

### 版本化與非版本化文檔 {#versioned-and-unversioned-doc}

有時您希望部分文檔進行版本化，而其他「全局性」文檔則無需版本化。

我們在 Docusaurus 官方網站即採用此模式：

- [/docs/\*](/docs) 章節為版本化文檔
- [/community/\*](/community/support) 章節為非版本化文檔
...
我們將 `product` 實例視為最重要實例，並通過不分配 ID 使其成為「預設」實例。
...
每個插件實例擁有獨立的 CLI 命令來標記新版本。執行以下命令即可查看：
