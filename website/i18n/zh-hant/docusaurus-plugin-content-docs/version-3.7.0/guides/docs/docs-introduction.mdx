...
請注意，您**並非必須放棄使用部落格**或其他插件；`routeBasePath: '/'` 的作用僅是將文件服務路徑從 `https://example.com/docs/some-doc` 變更為網站根目錄 `https://example.com/some-doc`。若啟用部落格功能，仍可透過 `blog/` 子路由存取。
...
Docusaurus 也提供「純部落格模式」供僅需使用部落格功能的用戶。您可採用上述相同方法，具體設定請參閱[純部落格模式](../../blog.mdx#blog-only-mode)。
