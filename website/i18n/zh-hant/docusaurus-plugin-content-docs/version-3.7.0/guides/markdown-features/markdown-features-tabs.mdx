...
您可能希望同類型的標籤頁選擇能相互同步。例如，針對 Windows 與 macOS 使用者提供不同操作說明時，希望透過單次點擊即可切換所有作業系統專用說明標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此設定會將選擇狀態持久化至 `localStorage`，且所有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
相同 `groupId` 的標籤群組，其可用值不必完全相同。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，該標籤群組將不會切換標籤頁。從以下範例可見，當選擇 Linux 時，上方標籤群組不會發生變化。
...
您還可透過 `attributes` 欄位獨立自訂每個標籤標題。額外屬性可透過 `Tabs` 的 `values` 屬性傳遞，或如同宣告 `label` 的方式透過各 `TabItem` 的屬性傳遞。
...
`className` 會與其他預設的類別名稱合併使用。你也可以使用自訂的 `data-value` 欄位（`{'data-value': 'apple'}`）搭配 CSS 屬性選擇器：
...
可以將選取的標籤狀態持久化到 URL 的搜尋參數中。這讓你能分享一個預先選定標籤的頁面連結——例如從你的 Android 應用程式連結到已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動到標籤位置。
...
當點擊標籤時，URL 末端會加入搜尋參數：例如 `?current-os=android` 或 `?current-os=ios`。
...
頁面載入時，會優先從 URL 查詢字串恢復標籤選擇狀態（而非從 `localStorage` 讀取 `groupId` 的記錄）。
