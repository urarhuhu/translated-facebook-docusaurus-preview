...
若使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為檔案路徑並自動轉換為 `require()` 調用。除非使用需自行處理的 JSX 語法，否則無需在 Markdown 中手動使用 `require()`。
...
當您需要通過 CSS 修改 SVG 圖像部分屬性時，這會非常有用。例如，可根據當前主題變更 SVG 的某種顏色。
...
要使用路徑片段切換圖片顯示（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），請在自訂 CSS 中加入以下內容（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。該連結將被轉換為 `require()` 調用而非保持為 URL 格式。這種處理方式在以下兩個方面具有優勢：
