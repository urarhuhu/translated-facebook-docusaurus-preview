...
- ID 可能不夠美觀
- 您可能希望變更或翻譯文字內容，但不希望更新現有 ID

透過特殊的 Markdown 語法可設定明確的標題 ID：
...
使用 **[`write-heading-ids`](../../cli.mdx#docusaurus-write-heading-ids-sitedir)** CLI 指令為所有 Markdown 文件添加明確 ID。

:::

:::warning[避免 ID 衝突]

自動生成的標題 ID 在單一頁面中會保持唯一性，但若使用自訂 ID，請確保每個 ID 在頁面中只出現一次，否則會導致兩個 DOM 元素擁有相同 ID，這違反 HTML 語義規範，並會使其中一個標題無法被連結。
...
### 範例子章節 3 c {#example-subsection-3-c}
