...
因此，雖然你可能知道不能訪問 Node 全局變量如 `process`（[或者可以嗎？](#node-env)）或 `'fs'` 模組，但也不能隨意訪問瀏覽器全局變量。
...
這看起來像是慣用的 React 代碼，但如果你運行 `docusaurus build`，將會收到錯誤：
...
在 Webpack 構建期間，`process.env.NODE_ENV` 將被替換為實際值，可能是 `'development'` 或 `'production'`。經過死碼消除後，你將獲得不同的構建結果：
...
React 不僅是一個動態 UI 運行時——它也是一個模板引擎。由於 Docusaurus 網站主要包含靜態內容，它應該能夠在沒有任何 JavaScript（React 運行所需的）的情況下工作，僅使用純 HTML/CSS。這就是伺服器端渲染所提供的功能：將你的 React 代碼靜態渲染為 HTML，不包含任何動態內容。HTML 文件沒有客戶端狀態的概念（它純粹是標記），因此不應依賴瀏覽器 API。
...
如果您需要僅在瀏覽器中渲染某些元件（例如，因為該元件依賴瀏覽器特定功能才能正常運作），常見的方法是將您的元件包裝在 [`<BrowserOnly>`](../docusaurus-core.mdx#browseronly) 中，以確保它在 SSR 期間不可見，僅在 CSR 中渲染。
...
雖然您可能期望 `BrowserOnly` 在伺服器端渲染期間隱藏子元素，但實際上它無法做到。當 React 渲染器嘗試渲染此 JSX 樹時，它確實會將 `{window.location.href}` 變量視為此樹的一個節點並嘗試渲染它，儘管它實際上並未被使用！使用函數可以確保我們僅在需要時才讓渲染器看到瀏覽器專用的元件。
...
最後，您可以將邏輯放在 `useEffect()` 中，以延遲其執行直到首次 CSR 之後。這最適合於僅執行副作用但不從客戶端狀態_獲取_數據的情況。
