...
DocSearch 每週爬取您的網站一次（排程可從網頁介面設定），並將所有內容匯總到 Algolia 索引中。這些內容隨後透過 Algolia API 直接從您的前端查詢。

如果您的網站[不符合資格](https://docsearch.algolia.com/docs/who-can-apply)使用免費託管版的 DocSearch，或者您的網站位於防火牆後且非公開，則可以[自行運行](https://docsearch.algolia.com/docs/run-your-own/) DocSearch 爬蟲。
...
在您的申請獲批並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到您的專案中。您可以透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取。索引在部署後立即可用，因此通常不需要手動配置。

:::danger[使用推薦的爬蟲配置]

強烈建議使用我們官方的 [**Docusaurus v3 爬蟲配置**](https://docsearch.algolia.com/docs/templates/#docusaurus-v3-template)。如果您選擇不同的爬蟲配置，我們將無法提供支援。

:::

:::warning[更新爬蟲配置時]

爬蟲配置包含 `initialIndexSettings`，該設定僅在 Algolia 索引不存在時用於初始化索引。

如果您更新 `initialIndexSettings` 爬蟲設定，可以透過介面手動更新索引，但 [Algolia 團隊建議刪除索引然後重新開始爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新設定重新初始化索引。
...
然後，在 `themeConfig` 中添加 `algolia` 欄位。**[申請 DocSearch](https://docsearch.algolia.com/apply/)** 以獲取 Algolia 索引和 API 金鑰。
...
同理，瀏覽法文站點時，顯示英文文件結果也不合理。

為解決此問題，情境化搜尋會動態識別您當前瀏覽的文件版本與語言，自動生成對應的搜尋過濾條件。

- 在 `/en/docs/v1/myDoc` 頁面，搜尋結果僅包含 **英文版 v1** 文件（及其他未版本化的頁面）
- 在 `/fr/docs/v2/myDoc` 頁面，搜尋結果僅包含 **法文版 v2** 文件（及其他未版本化的頁面）

:::info

當啟用 `contextualSearch: true`（預設值）時，情境化篩選條件會與 `algolia.searchParameters.facetFilters` 提供的篩選器合併。

特殊需求時可關閉 `contextualSearch` 並自訂 `facetFilters`：
...
若僅在關閉情境化搜尋時才能獲得結果，極可能是由於 [索引設定問題](#algolia-no-search-results) 導致。
...
您仍可透過修改 `/src/css/custom.css` 檔案，使用 Docusaurus 的 [Infima CSS 變數](styling-layout.mdx#styling-your-site-with-infima) 來調整樣式。
...
[Typesense](https://typesense.org) DocSearch 的運作方式與 Algolia DocSearch 類似，差別在於您的網站會被索引至 Typesense 搜尋叢集。
...
這將在專案目錄中建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯該元件後，您會發現 Docusaurus 現在使用您自訂的 `SearchBar` 元件。
