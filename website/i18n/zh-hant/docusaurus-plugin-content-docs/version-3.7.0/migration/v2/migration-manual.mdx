...
若部署至 GitHub Pages，請執行 `yarn deploy` 而非 `yarn publish-gh-pages` 腳本。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集為插件組合包，為向後相容性我們建立了 `@docusaurus/preset-classic` 預設集，其包含 Docusaurus 1 中的大多數核心插件。
...
請參閱下方遷移指南，了解 `siteConfig.js` 中每個欄位的變更說明。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完全準備好，我們會在完成後更新此處說明。若要覆蓋 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 工具來為您選擇的主色調尋找不同的深淺變化。
...
您可以聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們可以為您更新配置並觸發網站重新爬取以恢復搜索功能（否則您可能需要等待最多 24 小時直到下次預定爬取）
...
- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本控制功能尚未移植。如果您使用版本控制功能，目前無法遷移至 Docusaurus 2。請持續關注更新。
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在改為 `docsPluginOptions.sidebarCollapsible`，且此功能現在預設啟用。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
舊版本不支援巢狀側邊欄分類，且分類僅能包含文件 ID。v2 版本允許無限層級巢狀結構，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不限於文件）。
...
此操作會將主題使用的 `<Footer />` 元件複製到網站根目錄下的 `src/theme/Footer` 目錄，後續可編輯此元件進行客製化。

請勿僅為在左側新增標誌而替換頁尾元件。v2 版本已刻意移除左側標誌並移至底部，請直接透過 `docusaurus.config.js` 中的 `themeConfig.footer` 設定：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 頁面運作機制。請注意需將 v1 的 `pages/en` 檔案移至 `src/pages` 目錄。

Docusaurus v1 的頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改從 `useDocusaurusContext` 取得 `siteConfig` 物件。

v2 版本需在每個頁面套用主題佈局元件。Layout 元件會接收元資料 props。

v2 已棄用 `CompLibrary`，需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，期間可檢查 V2 網站或參閱 https://infima.dev/ 查看可用樣式）。

可將 CommonJS 語法遷移至 ES6 的 import/export。
...
Docusaurus 2 採用 [MDX](https://mdxjs.com/) 語法，需修正現有文件中不相容的語法。常見範例如 HTML 有效的自閉合標籤 `<img>` 和 `<br>`，在 MDX 中需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
`authorFBID` 與 `authorTwitter` 欄位已棄用。這些欄位僅用於生成作者頭像，現可透過 `authors` 欄位實現。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域請務必在 `/static/CNAME` 建立該檔案。

部落格 RSS 訂閱網址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向以維持現有訂閱功能。
