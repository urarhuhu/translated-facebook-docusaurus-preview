...
在 Docusaurus v1 中，您使用 `<translate>` 翻譯頁面：
...
在 Docusaurus v2 中，您使用 `<Translate>` 翻譯頁面
...
代碼翻譯現在使用 Chrome 國際化 JSON 格式添加到 `i18n/[locale]/code.json`。
...
MDX 將 Markdown 檔案編譯為 React 組件，比 Docusaurus v1 解析器更嚴格，並會在出現錯誤時使構建失敗，而非渲染一些不良內容。

此外，HTML 元素必須替換為 JSX 元素。

這對於國際化尤其重要，因為如果您的 Crowdin 翻譯不佳且使用了無效的標記，您的 v2 翻譯網站可能無法構建：您可能需要進行一些翻譯清理以修復錯誤。

## 遷移策略 {#migration-strategies}

本節將幫助您了解如何在遷移至 v2 後保留現有的 v1 翻譯。

對於使用 Crowdin 的 Docusaurus v1 網站，有多種可能的遷移策略，各有不同的權衡。

:::warning

本文檔旨在盡力協助您進行遷移，如果您找到更好的方法，請幫助我們改進！
...
請勿在未理解 Crowdin 和 Docusaurus v2 國際化的情況下嘗試遷移。

:::

### 創建新的 Crowdin 專案 {#create-a-new-crowdin-project}

為了避免任何在生產環境中破壞您的 v1 網站的風險，一種可能的策略是複製原始的 v1 Crowdin 專案。
...
您需要修復所有錯誤直到網站能成功建置。可以透過修改本地端的翻譯MD檔案來解決，並使用`docusaurus build --locale fr`一次修復一個語系的網站。
...
如果您不介意修改現有的Crowdin專案並承擔可能搞砸的風險，可以嘗試使用Crowdin的分支系統。
