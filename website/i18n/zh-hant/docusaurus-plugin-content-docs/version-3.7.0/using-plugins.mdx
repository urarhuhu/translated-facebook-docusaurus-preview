...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委託給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；或由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供的獨立[頁面](./guides/creating-pages.mdx)。如果沒有安裝任何插件，網站將不包含任何路由。

您無需逐一手動安裝常見插件：它們可以以[預設集](#using-presets)的形式打包分發。對多數用戶而言，插件是通過預設集配置來管理的。

我們維護了一份[官方插件列表](./api/plugins/overview.mdx)，社區也開發了許多[非官方插件](/community/resources#community-plugins)。如果您想添加任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務... 請務必查閱這些列表：可能已經有人為您實現了！

如果您精力充沛，也可以閱讀[插件開發指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來學習如何自行開發插件。

## 安裝插件 {#installing-a-plugin}

插件通常是 npm 套件，因此您可以像安裝其他 npm 套件一樣使用 npm 進行安裝。

```bash npm2yarn
npm install --save docusaurus-plugin-name
```

然後在您網站的 `docusaurus.config.js` 配置檔案的 `plugins` 選項中添加該插件：
...
但插件可以通過在配置中將名稱和選項物件包裝成二元組來指定選項。這種風格通常稱為「Babel 風格」。
...
最多只能有一個插件實例是「預設插件實例」，通過省略 `id` 屬性或使用 `id: 'default'` 來實現。

:::

## 使用主題 {#using-themes}

主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目可以互換使用。唯一的細微差別是主題在插件之後載入，且[主題可以覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析規則](#module-shorthands)，因此如果您想利用簡寫功能，請確保使用正確的條目！
...
預設集（presets）是插件與主題的組合套件。舉例來說，與其讓您在設定檔中逐一註冊並配置 `@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog` 等插件，我們提供了 `@docusaurus/preset-classic` 預設集，讓您能在單一集中位置進行配置。
...
預設集通常是 npm 套件，因此您可以像其他 npm 套件一樣使用 npm 進行安裝。

```bash npm2yarn
npm install --save @docusaurus/preset-classic
```

接著，在您網站的 `docusaurus.config.js` 中的 `presets` 選項新增它：
...
預設集是一個與[插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor)具有相同結構的函式。它應返回一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，其格式與網站設定中接受的格式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
如果名稱是有作用域的（以 `@` 開頭），則會先以第一個斜線將名稱拆分為作用域與套件名稱：
...
若名稱不存在（如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（例如 `@jquery/docusaurus-plugin`）。否則將嘗試以下路徑：
