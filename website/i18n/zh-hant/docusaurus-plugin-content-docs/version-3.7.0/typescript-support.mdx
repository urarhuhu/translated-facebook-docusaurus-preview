...
Docusaurus 不會使用此 `tsconfig.json` 來編譯您的專案。添加它僅是為了提升編輯器體驗，但您仍可選擇執行 `tsc` 來自行或在 CI 上進行程式碼型別檢查。
...
對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令結尾添加 `--typescript` 旗標來取得 TypeScript 原始碼。例如，以下指令將在 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`。
