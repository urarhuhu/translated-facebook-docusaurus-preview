...
Docusaurus 提供了一些客戶端 API，這些 API 在構建您的網站時可能會有所幫助。
...
用它來包裹可能拋出錯誤的組件，並在發生錯誤時顯示一個後備內容，而不是讓整個應用程序崩潰。
...
要查看其實際效果，請點擊這裡：<ErrorBoundaryTestButton/>
...
- `fallback`: 一個可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含兩個屬性的對象：`error`，即捕獲到的錯誤，以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的回調函數（`() => void`）。如果未提供，則將渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹網站並位於佈局之上的錯誤邊界。

:::warning

`fallback` 屬性是一個回調函數，**而不是一個 React 功能組件**。您不能在此回調函數中使用 React 鉤子。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能特性稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經被加載。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後在用戶可能導航到請求的資源時使用 `onMouseOver` 事件觸發高優先級請求。
...
當需要整合無法在 Node.js 環境運行的程式碼時使用此元件，例如存取 `window` 或 `document` 物件的情況。

:::

#### Props {#browseronly-props}

- `children`: 渲染函數 prop，返回僅在瀏覽器中運行的 JSX。不會在 Node.js 中執行
...
翻譯字串將透過 [`docusaurus write-translations`](./cli.mdx#docusaurus-write-translations-sitedir) CLI 從程式碼中靜態提取，並在 `website/i18n/[locale]` 目錄下建立 `code.json` 翻譯檔案。
...
`<Translate>` 元件支援插值功能。您還可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
插件 ID 僅在單個插件被多次使用時有意義。每個插件實例可建立自己的全域資料。
...
存取特定插件建立的全域資料。根據插件名稱返回該插件所有實例的資料（按插件 ID 分類）。
