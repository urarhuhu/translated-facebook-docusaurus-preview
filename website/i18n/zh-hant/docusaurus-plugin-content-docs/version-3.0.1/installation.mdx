...
我們推薦使用 `classic` 模板，這樣您可以快速入門，並且它包含 Docusaurus 1 中的功能。`classic` 模板包含 `@docusaurus/preset-classic`，其中包括標準文檔、博客、自定義頁面和 CSS 框架（支持暗黑模式）。使用經典模板可以極快地啟動並運行，並在您對 Docusaurus 更加熟悉後進行自定義。
...
假設您選擇了經典模板並將您的網站命名為 `my-website`，您將在新建的目錄 `my-website/` 下看到以下生成的文件：
...
- `/blog/` - 包含部落格的 Markdown 檔案。若您已停用部落格外掛程式，可刪除此目錄；或透過設定 `path` 選項後變更目錄名稱。詳情請參閱[部落格指南](blog.mdx)
- `/docs/` - 包含文件的 Markdown 檔案。可在 `sidebars.js` 中自訂文件側邊欄排序。若您已停用文件外掛程式，可刪除此目錄；或透過設定 `path` 選項後變更目錄名稱。詳情請參閱[文件指南](./guides/docs/docs-introduction.mdx)
- `/src/` - 非文件類檔案（如頁面或自訂 React 元件）。您不一定要將非文件檔案嚴格放置於此，但集中存放便於後續進行程式碼檢查/處理
  - `/src/pages` - 此目錄內的任何 JSX/TSX/MDX 檔案都會轉換為網站頁面。詳情請參閱[頁面指南](guides/creating-pages.mdx)
- `/static/` - 靜態資源目錄。此處內容將直接複製到最終 `build` 目錄的根路徑下
...
若您使用 Docusaurus 為現有專案編寫文件，單體儲存庫可能是理想解決方案。此架構允許在相似專案間共享依賴項，例如您的網站可使用本地套件展示最新功能，而非依賴已發布版本。如此貢獻者能在實作功能時同步更新文件。下方為單體儲存庫的範例目錄結構：
...
若使用 Netlify 或 Vercel 等託管服務，需將網站的「基礎目錄」變更為 Docusaurus 根目錄位置（本例為 `./website`）。更多忽略指令設定詳情請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。

更多單體儲存庫說明請參閱 [Yarn 文件](https://yarnpkg.com/features/workspaces)（Yarn 非唯一解決方案，但屬常見方式），或參考 [Docusaurus](https://github.com/facebook/docusaurus) 與 [Jest](https://github.com/facebook/jest) 的實際案例。
...
預設將在 [`http://localhost:3000`](http://localhost:3000) 自動開啟瀏覽器視窗。

恭喜！您已成功建立第一個 Docusaurus 網站！請瀏覽站點內容查看現有功能。

## 建置 {#build}

Docusaurus 屬現代靜態網站生成器，需將網站建置為靜態內容目錄後部署至網頁伺服器才能檢視。執行建置指令：

```bash npm2yarn
npm run build
```

建置內容將生成於 `/build` 目錄內，可部署至 [GitHub Pages](https://pages.github.com/)、[Vercel](https://vercel.com/) 或 [Netlify](https://www.netlify.com/) 等靜態檔案託管服務。更多細節請參閱[部署文件](deployment.mdx)。
