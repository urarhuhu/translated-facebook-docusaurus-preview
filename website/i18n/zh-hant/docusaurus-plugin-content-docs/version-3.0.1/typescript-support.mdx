...
Docusaurus 不會使用此 `tsconfig.json` 來編譯您的專案。添加此檔案僅是為了提升編輯器體驗，但您仍可選擇執行 `tsc` 來自行或在 CI 上進行程式碼型別檢查。
...
對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令後方加入 `--typescript` 參數來取得 TypeScript 原始碼。例如以下指令將在 `src/theme/Footer` 目錄下生成 `index.tsx` 和 `styles.module.css`。
