...
有時您需要在網站發布公告。為此可添加公告欄——這是一個非固定且可選擇性關閉的面板，位於導覽列上方。所有配置皆位於 `announcementBar` 物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，這在將文檔網站託管於主站子目錄時特別有用——此情境下您可能不希望標誌連結至主站時開啟新分頁。

為強化深色模式支援，您可為此模式設定不同標誌。
...
請注意，下拉基底項目本身也是可點擊的連結，因此該項目可接收[純導航欄連結](#navbar-link)的任何屬性。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向 `docId` 所提供文件的連結。只要瀏覽同一側邊欄的文件，該連結就會獲得 `navbar__link--active` 類別。
...
若您使用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導覽列項目類型會顯示包含所有可用語言的下拉選單。

使用者可切換不同語言版本，同時保持在相同頁面位置。
...
若您啟用[搜尋功能](../../search.mdx)，搜尋欄預設會出現在導覽列最右側。
...
啟用此UI功能後，當使用者向下滾動頁面時會自動隱藏導覽列，向上滾動時則重新顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。可於[Infima文件](https://infima.dev/docs/components/navbar/)預覽樣式效果。
...
預設採用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。您可從[可用主題列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自訂主題，亦可為深色模式指定不同的高亮主題。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景顏色。請參閱[行標記指南](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)以獲取詳細說明。

:::

### 預設語言 {#default-language}

您可為程式碼區塊設定預設語言（當開頭三個反引號後未指定語言時生效）。請注意，必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
可透過 `themeConfig.footer` 新增商標與版權聲明。商標圖檔可置於[靜態資源目錄](static-assets.mdx)，其 URL 設定方式與導覽列商標相同。
...
簡易式頁尾僅需定義一列橫向排列的 `FooterItem` 清單。
...
用於存取色彩模式的 React 鉤子。此上下文提供切換亮/暗模式的函式，並暴露當前使用模式的布林值。
