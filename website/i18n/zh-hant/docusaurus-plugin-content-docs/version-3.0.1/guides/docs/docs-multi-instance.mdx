...
當有人編輯 iOS 文檔時，真的有必要重新構建包括未變更的整個 Android 文檔在內的所有內容嗎？

:::

### 版本化與非版本化文檔 {#versioned-and-unversioned-doc}

有時您希望部分文檔進行版本化，而其他文檔保持「全局」狀態，對其進行版本化顯得沒有必要。

我們在 Docusaurus 官方網站上就採用了這種模式：
...
我們將 `product` 實例視為最重要的實例，並通過不分配 ID 使其成為「預設」實例。
...
舉例來說，若您想為每個行動SDK（iOS和Android）設置獨立的版本下拉選單，可以這樣設定：
