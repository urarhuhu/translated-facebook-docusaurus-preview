...
自動生成的標題 ID 在單一頁面中能保證唯一性，但若使用自訂 ID，請確保同一頁面中每個 ID 只出現一次，否則會導致 HTML 語義錯誤，且部分標題將無法被連結。

:::

## 目錄標題層級 {#table-of-contents-heading-level}

每份 Markdown 文件右上角會顯示目錄。預設僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構。如需調整顯示的標題層級範圍，可針對單一頁面或全局設定最小/最大標題層級。
...
要全局設定所有頁面的標題層級，請使用 [`themeConfig.tableOfContents`](../../api/themes/theme-configuration.mdx#table-of-contents) 選項。
...
即使已設定全局選項，仍可透過 front matter 在單一頁面中覆蓋設定。
...
MDX 文件中可透過 `toc` 變數取得所有標題資訊。預設僅顯示 `h2` 和 `h3` 標題，您可透過設定 `TOCInline` 元件的 `minHeadingLevel` 或 `maxHeadingLevel` 屬性來調整顯示的標題層級。
...
請注意 `toc` 全局變數是扁平陣列結構，因此可輕鬆過濾不需要的節點或插入額外節點，重新組織目錄樹狀結構。
