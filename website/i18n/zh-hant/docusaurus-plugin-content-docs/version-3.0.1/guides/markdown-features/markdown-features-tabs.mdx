...
預設會顯示第一個標籤頁。若要覆寫此行為，可在任一標籤項目添加 `default` 屬性，或為 `Tabs` 元件的 `defaultValue` 屬性指定您選擇的標籤值。例如在上方範例中，無論是為 `value="apple"` 的標籤設定 `default`，或是為 `Tabs` 設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。
...
您可能希望同類型的標籤選擇能相互同步。例如，您想為 Windows 用戶和 macOS 用戶提供不同的操作說明，並希望透過單次點擊切換所有作業系統專屬的說明標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此設定會將選擇狀態持久化儲存在 `localStorage` 中，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 是全域命名空間的。

對於所有具有相同 `groupId` 的標籤群組，其可能值不需要相同。若某個標籤群組選擇的值在另一個同群組 ID 的標籤群組中不存在，則該標籤群組不會切換標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方的標籤群組不會改變。
...
您可能想自訂特定標籤頁組的外觀。可透過 `className` 屬性傳入字串，指定的 CSS 類別將會添加到 `Tabs` 元件：
...
可以將選取的標籤狀態持久化到 URL 的搜尋參數中。這讓您能分享已預選標籤的頁面連結——例如從您的 Android 應用程式連結到已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該標籤。
...
頁面載入時，會優先還原查詢字串中的標籤選擇（而非 `groupId` 透過 `localStorage` 儲存的選擇）。
