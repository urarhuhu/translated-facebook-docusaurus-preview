...
若您使用 [Prettier](https://prettier.io) 格式化 Markdown 文件，Prettier 可能會將程式碼自動格式化為無效的提示框語法。為避免此問題，請在起始與結束指令周圍添加空行。這也是為何此處展示的範例都在內容周圍保留空行。
...
接受的類型與上述相同：`note`、`tip`、`danger`、`info`、`warning`。您可選擇性地透過傳遞 JSX 元素、字串或標題來指定圖示：
...
您可以透過 [swizzling](../../swizzling.mdx) 自訂每種提示框類型的渲染方式。通常只需簡單的包裝元件即可達成目標。例如下方範例中，我們僅替換了 `info` 提示框的圖示。
...
- `extendDefaults`: 是否將提供的選項（如 `keywords`）合併至預設值中。預設為 `true`。
...
您可以為 `:::my-custom-admonition` 提供對應的 React 組件，方法是建立以下文件（遺憾的是，由於它不是 React 組件文件，因此無法透過 swizzling 修改）：
