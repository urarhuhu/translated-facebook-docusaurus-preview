...
地區名稱用於翻譯檔案的存放位置，以及翻譯語言環境的基礎 URL。當建置所有語言環境時，只有預設語言環境的名稱會從基礎 URL 中省略。
...
法文語言環境的所有翻譯資料都儲存在 `website/i18n/fr` 中。每個插件在對應的資料夾下提供自己的翻譯內容，而 `code.json` 檔案則定義 React 程式碼中使用的所有文字標籤。
...
JSX 元素是_物件_而非字符串。在預期字符串的上下文中使用（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會[強制轉換為字符串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，導致返回 `"[object Object]"`。雖然我們鼓勵將 `<Translate>` 作為 JSX 子元素使用，但僅在實際可行的情況下使用元素形式。
...
`docusaurus write-translations` 指令會靜態分析站點中所有 React 程式碼文件，提取這些 API 的調用，並將其聚合到 `code.json` 文件中。翻譯文件將存儲為從 ID 到翻譯訊息物件（包含已翻譯標籤和標籤描述）的映射。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定預設未翻譯訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯。

:::warning[文字標籤必須是靜態的]

`docusaurus write-translations` 指令僅對程式碼進行**靜態分析**，不會實際運行您的站點。因此，動態訊息無法被提取，因為訊息是_表達式_而非_字符串_：
...
此代碼在運行時仍能正常運作。但未來我們可能提供「無運行時」機制，通過 Babel 轉換直接將翻譯內聯到 React 代碼中，而非在運行時調用 API。因此，為確保未來相容性，應始終優先使用可靜態分析的訊息。例如，我們可將上述代碼重構為：
