...
- **模組化**：允許插件作者提供國際化支援
- **低運行負擔**：文件主要是靜態內容，不需繁重的 JS 函式庫或 polyfill
...
- **RTL 支援**：支援從右至左閱讀的語言 (阿拉伯文、希伯來文等) 且易於實作
...
部分插件會從外部資料檔案讀取內容，這些檔案需整體進行本地化。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑下建立副本來實現翻譯。

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需存放在正確的檔案系統路徑中。

每個語言區域(locale)與插件都有專屬的`i18n`子資料夾：
...
為簡易的Docusaurus網站新增法文翻譯後，目錄結構會呈現如下：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件會在其對應資料夾下提供翻譯內容，而`code.json`檔案則定義React程式碼中使用的文字標籤。
