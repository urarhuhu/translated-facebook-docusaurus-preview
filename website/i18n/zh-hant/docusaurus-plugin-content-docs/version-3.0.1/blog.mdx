...
接著，在 `docusaurus.config.js` 中新增一個部落格的連結項目：
...
在部落格文章中使用 `<!--truncate-->` 標記來表示在查看所有已發布部落格文章時將顯示的摘要。`<!--truncate-->` 上方的內容將成為摘要的一部分。請注意，截斷標記上方的部分必須是獨立可渲染的 Markdown。例如：
...
預設情況下，每個部落格列表頁面會顯示 10 篇文章，但您可以透過插件配置中的 `postsPerPage` 選項控制分頁。如果設定 `postsPerPage: 'ALL'`，則會停用分頁，並在第一頁顯示所有文章。您也可以為部落格列表頁面添加元描述以提升 SEO：
...
您也可以透過 `blogSidebarTitle` 選項修改側邊欄的標題文字。例如，如果您設定 `blogSidebarCount: 'ALL'`，可以將預設的「Recent posts」改為「All posts」：
...
此命名慣例為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若需要更精確的發布時間，可使用 front matter 設定。例如，當同一天發布多篇文章時，可依當天時間排序：
...
`readingTime` 回調函數接收三個參數：部落格內容文字（字串形式）、前端資料（鍵值對記錄）以及預設閱讀時間函數。它返回一個數字（閱讀時間分鐘數）或 `undefined`（停用該頁面的閱讀時間顯示）。

預設閱讀時間函數可接受額外選項：`wordsPerMinute`（數字，預設值：300）和 `wordBound`（從字串到布林值的函數）。若傳入 `wordBound` 的字串應被視為單詞邊界（預設為空格、製表符和換行符），則該函數應返回 `true`。
...
您可以讓 Docusaurus 網站不設專用首頁，改以部落格文章列表頁作為索引頁。將 `routeBasePath` 設為 `'/'` 即可透過根路由 `example.com/`（而非子路由 `example.com/blog/`）提供部落格服務。
...
同樣存在「僅文檔模式」供僅需使用文檔功能的用戶。詳見[僅文檔模式](./guides/docs/docs-introduction.mdx)獲取詳細說明或關於`routeBasePath`的更深入解釋。

:::

### 多博客系統 {#multiple-blogs}

默認情況下，經典主題假設每個網站僅有一個博客，因此僅包含一個博客插件實例。若需在單一網站上運行多個博客，亦可實現！您可通過在`docusaurus.config.js`的`plugins`選項中配置另一個博客插件來添加額外博客。

將`routeBasePath`設為您希望第二個博客訪問的URL路徑。請注意此處的`routeBasePath`必須與首個博客不同，否則可能導致路徑衝突！同時將`path`設為包含第二個博客條目的目錄路徑。

如[多實例插件](./using-plugins.mdx#multi-instance-plugins-and-plugin-ids)文檔所述，需為插件分配唯一ID。
