...
Docusaurus 對於配置有獨特的見解。我們鼓勵您將網站相關信息集中到一個地方。我們會保護此文件的字段，並促進該數據對象在整個網站中的可訪問性。
...
您的配置物件將對網站所有組件可用，可透過 React 上下文以 `siteConfig` 形式存取。
...
若僅需在客戶端使用這些欄位，可建立自己的 JS 檔案並作為 ES6 模組導入，無需將其放入 `docusaurus.config.js`。
...
多數情況下此配置可直接使用。如需自訂 Babel 配置（例如添加 Flow 支援），可直接編輯此檔案。變更生效需重新啟動 Docusaurus 開發伺服器。