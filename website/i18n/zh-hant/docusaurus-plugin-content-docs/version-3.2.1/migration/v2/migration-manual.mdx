...
Docusaurus 2 則簡化為 `website/build`。請確認部署配置已更新至正確讀取新建置目錄。
...
既有 D1 網站可能已有 README 文件。您可修改現有內容以反映 D2 變更，或直接複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集（preset）是插件的組合包，為保持向後相容性，我們提供 `@docusaurus/preset-classic` 預設集，其包含 Docusaurus 1 的核心功能插件。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄必須位於 `website` 內。將文檔放在網站目錄中也更為理想，因為文檔和網站其餘程式碼可以集中在同一個 `website` 目錄下。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數實現主題化。相關文檔尚未完備，後續會更新說明。若要覆寫 Infima 的 CSS 變數，請建立自訂 CSS 檔案（例如 `./src/css/custom.css`），並通過 `@docusaurus/preset-classic` 的選項全域導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 工具為您選擇的主色調找出合適的漸變色階。
...
網站元資訊（如資源、SEO、版權資訊）現在由主題處理。若要自訂，請在 `docusaurus.config.js` 中使用 `themeConfig` 欄位：
...
已棄用。請改為通過 `@docusaurus/preset-classic` 的部落格選項設定：
...
若您在 v1 中設定 `cleanUrl: false`，可能有人已發布指向 `/installation.html` 的連結。
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 頁面運作機制。閱讀後請注意：需將 v1 的 `pages/en` 檔案移至 `src/pages`。

Docusaurus v1 中，頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改用 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版需在每個頁面外層套用主題佈局。Layout 元件接收元資料 props。
...
Docusaurus 2 採用 [MDX](https://mdxjs.com/) 語法，需修正現有文件中不相容的語法。常見範例：HTML 中有效的自閉合標籤如 `<img>` 和 `<br>`，現在需明確閉合（寫作 `<img/>` 和 `<br/>`）。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域，請確保已在 `/static/CNAME` 建立該檔案。
