...
這對國際化尤其重要，因為如果您的 Crowdin 翻譯不佳且使用了無效的標記，您的 v2 翻譯網站可能會構建失敗：您可能需要進行一些翻譯清理來修復這些錯誤。
...
本文檔是盡力幫助您進行遷移的指南，如果您找到更好的方法，請幫助我們改進它！
...
首次嘗試時很可能會遇到錯誤：預翻譯可能會嘗試翻譯不該翻譯的內容（front matter、admonition、程式碼區塊等），且翻譯後的 MD 檔案可能對 MDX 解析器無效。

您需要修復所有錯誤直到網站能成功建置。可以透過修改本地端的翻譯 MD 檔案來解決，並使用 `docusaurus build --locale fr` 一次針對一個語系修復網站。

我們無法提供修復這些錯誤的終極指南，但常見錯誤原因包括：

- 未在 Crowdin 中將足夠字串標記為「隱藏字串」，導致預翻譯嘗試翻譯這些字串。
- 存在不良的 v1 翻譯，導致 v2 中出現無效標記：翻譯中包含不良 HTML 元素和未閉合標籤
- 任何被 MDX 解析器拒絕的內容，例如使用 HTML 元素而非 JSX 元素（使用 [MDX 遊樂場](https://mdxjs.com/playground/)進行除錯）

您可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並僅限於特定語言/檔案進行預翻譯。

:::tip

對有問題的 Markdown 元素使用 [`mdx-code-block`](../../i18n/i18n-crowdin.mdx#mdx-solutions)：Crowdin 較不容易搞亂程式碼區塊內的內容。
...
此解析器版本並未公開說明，您需要聯繫 Crowdin 支援以了解專案的解析器版本並固定特定版本。
...
Crowdin 有「上傳翻譯」功能，但根據我們的經驗，對 Markdown 的效果並不理想

:::

### 使用現有 Crowdin 專案 {#use-the-existing-crowdin-project}

如果不介意修改現有 Crowdin 專案並承擔搞亂內容的風險，可以嘗試使用 Crowdin 的分支系統。
...
這樣就不需要建立新 Crowdin 專案、轉移翻譯記憶庫、套用預翻譯並嘗試修復預翻譯錯誤。

您可以為 Docusaurus v2 建立 Crowdin 分支，上傳 v2 原始檔，並在準備完成後將 Crowdin 分支合併至主分支。

### 改用 Git 而非 Crowdin {#use-git-instead-of-crowdin}

可以選擇遷移離開 Crowdin，改將翻譯檔案加入 Git。
