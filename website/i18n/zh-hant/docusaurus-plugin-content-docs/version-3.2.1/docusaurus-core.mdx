...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤；`tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則會渲染 `@theme/Error`。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。

:::warning

`fallback` 屬性是一個回調函數，**不是一個 React 函數組件**。您不能在這個回調中使用 React hooks。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能功能稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經加載完成。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後在用戶可能導航到該資源時使用 `onMouseOver` 事件觸發高優先級請求。

此組件是 react-router 的 `<Link>` 組件的封裝，增加了 Docusaurus 特有的增強功能。所有屬性都會傳遞給 react-router 的 `<Link>` 組件。
...
當需要整合無法在 Node.js 環境執行的程式碼時使用此元件，例如存取 `window` 或 `document` 物件的情況。

:::

#### Props {#browseronly-props}

- `children`: 渲染函式 prop，返回僅在瀏覽器中執行的 JSX。在 Node.js 中不會執行
...
`<Translate>` 元件支援插值功能。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實現[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
用於存取 Docusaurus 上下文的 React Hook。上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
插件 ID 僅在單一插件被多次使用時有意義。每個插件實例可創建獨立的全域資料。
...
可透過 `.docusaurus/globalData.json` 檢視網站的全域資料
...
React Hook 用於存取 Docusaurus 的壞鏈結檢查 API，提供方式讓 Docusaurus 頁面回報並收集其鏈結與錨點。
