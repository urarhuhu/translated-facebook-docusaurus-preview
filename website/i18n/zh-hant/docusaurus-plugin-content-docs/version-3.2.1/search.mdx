...
DocSearch 每週會爬取您的網站一次（排程可透過網頁介面設定），並將所有內容匯總至 Algolia 索引中。這些內容隨後會直接透過 Algolia API 從前端查詢。

若您的網站[不符合資格](https://docsearch.algolia.com/docs/who-can-apply)使用免費託管版 DocSearch，或網站位於防火牆後且非公開，您可以[自行運行](https://docsearch.algolia.com/docs/run-your-own/) DocSearch 爬蟲。
...
申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到專案中。可透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬蟲。索引在部署後立即可用，因此通常不需要手動配置。
...
若更新 `initialIndexSettings` 爬蟲設定，可透過介面手動更新索引，但 [Algolia 團隊建議刪除索引後重新啟動爬蟲](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以使用新設定完全重新初始化。
...
當瀏覽 v2 文件時，顯示 v1 文件的搜尋結果會顯得突兀。有時 v1 與 v2 文件內容相似，可能導致相同查詢出現重複結果（每個版本各一筆）。

同理，瀏覽法文網站時，顯示英文文件的搜尋結果也不合理。

為解決此問題，情境化搜尋會自動識別您當前瀏覽的文件版本與語言，並動態建立對應的搜尋篩選條件。
...
若僅在停用情境化搜尋時才能取得結果，極可能是因 [索引設定問題](#algolia-no-search-results) 所致。

:::

### 自訂 Algolia 搜尋樣式 {#styling-your-algolia-search}

DocSearch 預載符合無障礙標準的主題，確保色彩與對比度達標。
...
Algolia DocSearch 支援 [多種參數](https://docsearch.algolia.com/docs/api/)，可透過 `docusaurus.config.js` 中的 `algolia` 欄位傳遞。
...
若需直接修改 Algolia 搜尋的 React 元件，請對 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 元件進行 [swizzle 操作](swizzling.mdx)：
...
我們[推薦特定爬蟲配置](#algolia-index-configuration)有其原因。若您選擇使用不同配置，我們將無法提供支援。
...
- 在自有伺服器上[自行託管](https://typesense.org/docs/latest/guide/install-typesense.html#option-2-local-machine-self-hosting)
...
您可在[社群資源頁面](https://docusaurus.io/community/resources#search)找到社群支援的[本地搜尋外掛清單](https://docusaurus.io/community/resources#search)。
