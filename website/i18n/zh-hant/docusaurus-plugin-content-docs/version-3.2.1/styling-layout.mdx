...
根據您的偏好和您嘗試建立的網站類型，有幾種方法/框架可供選擇。行為更像網絡應用程式且具有高度互動性的網站將受益於更現代的樣式設定方法，這些方法將樣式與組件放在一起。當您希望自定義或 swizzle 組件時，組件樣式設定也特別有用。

## 全局樣式 {#global-styles}

這是最傳統的樣式設定方式，大多數開發人員（包括非前端開發人員）都熟悉這種方式。對於沒有太多自定義的小型網站來說，這種方式效果很好。

如果您使用 `@docusaurus/preset-classic`，您可以創建自己的 CSS 文件（例如 `/src/css/custom.css`），並通過將其作為經典主題的選項來全局導入它們。
...
- **主題類名**。這些類名在[下一小節](#theme-class-names)中詳細列出。它們沒有任何默認屬性。在自定義 CSS 中，您應始終優先針對這些穩定的類名。
- **Infima 類名**。這些類名在經典主題中找到，通常遵循 [BEM 慣例](http://getbem.com/naming/)的 `block__element--modifier`。它們通常是穩定的，但仍被視為實現細節，因此您通常應避免針對它們。但是，您可以[修改 Infima CSS 變量](#styling-your-site-with-infima)。
- **CSS 模塊類名**。這些類名在生產環境中帶有哈希值（`codeBlockContainer_RIuc`），在開發環境中則附加了長文件路徑。它們被視為實現細節，您幾乎應始終避免在自定義 CSS 中針對它們。如果必須，您可以使用忽略哈希值的[屬性選擇器](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors)（`[class*='codeBlockContainer']`）。

### 主題類名 {#theme-class-names}

我們提供了一些穩定的 CSS 類名，用於強大且可維護的全局佈局樣式設定。這些名稱與主題無關，旨在由自定義 CSS 針對。
...
`@docusaurus/preset-classic` 使用 [Infima](https://infima.dev/) 作為底層樣式框架。Infima 提供了適合以內容為中心的網站（博客、文檔、登陸頁面）的靈活佈局和常見 UI 組件樣式。有關更多詳細信息，請查看 [Infima 網站](https://infima.dev/)。
...
若您計劃透過 iframe 在其他網站嵌入 Docusaurus 頁面，建立替代顯示模式並使用如 `https://mysite.com/docs/myDoc?docusaurus-data-mode=iframe` 的 iframe 網址會很有幫助。您需自行提供額外樣式，並決定要保留或隱藏哪些 UI 元素。

:::

### 行動裝置檢視 {#mobile-view}

Docusaurus 使用 `996px` 作為行動裝置與桌面螢幕寬度的分界點。若您想在行動檢視中呈現不同佈局，可使用媒體查詢。
...
部分 React 元件（如標頭和側邊欄）在行動檢視中會實作不同的 JavaScript 邏輯。若您在自訂 CSS 中變更斷點值，可能還需透過[替換元件](./swizzling.mdx)來更新 `useWindowSize` 鉤子的呼叫，並傳入明確的選項參數。
...
將您的樣式表文件命名為 `.module.scss` 後綴（例如 `welcome.module.scss`）而非 `.css`。Webpack 將使用 `sass-loader` 預處理您的樣式表並將其作為 CSS 模塊加載。
