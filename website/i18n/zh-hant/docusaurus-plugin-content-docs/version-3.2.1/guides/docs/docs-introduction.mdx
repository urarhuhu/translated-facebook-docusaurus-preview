...
若您已為某個文件添加 `slug: /` 使其成為首頁，則應刪除現有的 `./src/pages/index.js` 首頁，否則會出現兩個檔案映射至相同路由的情況！
...
對於僅想使用 Docusaurus 部落格功能的用戶，也有「純部落格模式」。您可使用上述相同方法，具體設定請參閱 [純部落格模式](../../blog.mdx#blog-only-mode)。
