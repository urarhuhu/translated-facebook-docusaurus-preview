...
預設會顯示第一個標籤頁，若要覆寫此行為，可在任一標籤項目添加 `default` 屬性。亦可透過設定 `Tabs` 元件的 `defaultValue` 屬性來指定預設標籤頁的標籤值。例如上方案例中，無論是為 `value="apple"` 標籤設定 `default` 屬性，或是為 `Tabs` 設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若提供的 `defaultValue` 指向不存在的值，Docusaurus 將拋出錯誤。若希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤頁選擇能相互同步。例如，您可能想為 Windows 和 macOS 使用者提供不同操作指引，並希望透過單次點擊變更所有作業系統專屬的指引標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此操作會將選擇持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不需相同。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，則該標籤群組不會變更其標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方標籤群組不會變更。
...
可以將選定的分頁狀態持久化儲存到 URL 的搜尋參數中。這讓您能分享已預選分頁的頁面連結——例如從您的 Android 應用程式連結到已預選 Android 分頁的說明文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該分頁。
...
當分頁被點擊時，URL 末端會立即加入搜尋參數：例如 `?current-os=android` 或 `?current-os=ios`。
...
當頁面載入時，會優先從查詢字串恢復分頁選擇狀態（其次才從 `groupId` 的 `localStorage` 記錄恢復）。
