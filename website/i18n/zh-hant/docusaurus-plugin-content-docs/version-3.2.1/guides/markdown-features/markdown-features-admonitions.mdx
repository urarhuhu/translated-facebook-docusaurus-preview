...
您可透過 [swizzling](../../swizzling.mdx) 自訂每種提示框類型的渲染方式。通常只需簡單的包裝元件即可達成目標。例如下方範例中，我們僅替換了 `info` 提示框的圖標。
...
預設情況下，主題不知道如何處理自訂提示框關鍵字（如 `:::my-custom-admonition`）。您需自行將每個提示框關鍵字映射到 React 元件，主題才能知道如何渲染它們。
...
您可以為 `:::my-custom-admonition` 提供對應的 React 元件，方法是建立以下檔案（由於這不是 React 元件檔案，因此無法透過 swizzling 修改）：
