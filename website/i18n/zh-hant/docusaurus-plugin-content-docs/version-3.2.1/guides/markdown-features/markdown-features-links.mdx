...
如果你想引用 **由同一個插件處理** 的其他 Markdown 檔案，可以使用目標文件的相對路徑。Docusaurus 的 Markdown 載入器會將檔案路徑轉換為目標檔案的 URL 路徑（因此會移除 `.md` 副檔名）。
...
絕對檔案路徑也可以相對於網站目錄。但請注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為如果你創建新的文件版本或進行本地化時，需要手動更新這些連結。
...
- 移動檔案時，你的編輯器可能會追蹤這些變更，某些編輯器甚至會自動更新檔案連結
