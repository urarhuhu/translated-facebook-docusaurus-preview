...
從CDN載入樣式表、字型和JavaScript函式庫對於熱門函式庫和資源來說是良好實踐，因為這能減少您需要自行託管的資源量。若您傾向自行託管`katex.min.css`（連同必要的KaTeX字型），可從[KaTeX GitHub發佈頁](https://github.com/KaTeX/KaTeX/releases)下載最新版本，解壓後將`katex.min.css`與`fonts`目錄（僅需`.woff2`字型類型即可）複製到您網站的`static`目錄中，並在`docusaurus.config.js`裡將樣式表的`href`從CDN網址替換為本地路徑（例如`/katex/katex.min.css`）。
