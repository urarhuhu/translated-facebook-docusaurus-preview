...
Docusaurus 內建支援 [MDX](https://mdxjs.com/)，允許你在 Markdown 檔案中撰寫 JSX 並將其渲染為 React 元件。

查看 [MDX 文件](https://mdxjs.com/) 了解如何使用 MDX 實現各種進階功能。

:::tip[除錯 MDX]

MDX 格式相當嚴格，你可能會遇到編譯錯誤。

使用 **[MDX 遊樂場](https://mdxjs.com/playground/)** 來除錯並確保你的語法是有效的。

:::

:::info

最流行的格式化工具 Prettier [僅支援舊版 MDX v1](https://github.com/prettier/prettier/issues/12209)。如果遇到非預期的格式化結果，可以在問題區域前添加 `{/* prettier-ignore */}`，或將 `*.mdx` 加入 `.prettierignore` 中，直到 Prettier 完整支援 MDX v3。[MDX 的主要作者之一推薦使用 `remark-cli` 搭配 `remark-mdx`](https://github.com/orgs/mdx-js/discussions/2067)。
...
注意它如何同時渲染你的 React 元件標記和 Markdown 語法：
...
由於所有文件都是使用 MDX 解析的，任何看似 HTML 的內容實際上是 JSX。因此，如果需要內聯樣式元件，請遵循 JSX 風格並提供樣式物件。
...
你也可以匯入其他檔案中定義的自訂元件或透過 npm 安裝的第三方元件。
...
`@site` 別名指向你的網站目錄，通常是 `docusaurus.config.js` 檔案所在的位置。使用別名而非相對路徑（如 `'../../src/components/BrowserWindow'`）可以避免在移動檔案或進行[文件版本控制](../docs/versioning.mdx)和[翻譯](../../i18n/i18n-tutorial.mdx)時更新匯入路徑。

:::

雖然在 Markdown 中宣告元件對於簡單情況非常方便，但由於編輯器支援有限、解析錯誤風險和低可重用性，維護起來會變得困難。當你的元件涉及複雜的 JS 邏輯時，請使用獨立的 `.js` 檔案：
...
如果你在多個檔案中使用相同的元件，不需要在每個檔案中都匯入它——可以考慮將其加入全域範圍。[詳見下文](#mdx-component-scope)
...
它將被編譯為包含 `ul`、`li`、`p` 和 `Highlight` 元素的 React 元件。`Highlight` 不是原生 html 元素：你需要為其提供自己的 React 元件實現。

在 Docusaurus 中，MDX 元件作用域由 `@theme/MDXComponents` 檔案提供。與大多數其他透過 `@theme/` 別名匯出的內容不同，它本身並非 React 元件：而是一個從標籤名稱（如 `Highlight`）映射到其實作 React 元件的記錄檔。
...
若要註冊額外標籤名稱（如上述 `<Highlight>` 標籤），建議考慮[封裝 `@theme/MDXComponents`](../../swizzling.mdx#wrapping)，這樣就不需維護所有現有映射。由於替換 CLI 尚不支援封裝非元件檔案，您需手動建立封裝器：
...
現在，您可以在所有頁面中直接使用 `<Highlight>`，無需撰寫匯入語句：
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)大體相容於 [CommonMark](https://commonmark.org/)，但更為嚴格，因為您的 `.mdx` 檔案可使用 JSX 並會被編譯成真正的 React 元件（請參閱[互動式遊樂場](https://mdxjs.com/playground/)）。
...
此功能目前為**實驗性質**，且有若干[限制](https://github.com/facebook/docusaurus/issues/9092)。
...
您必須使用 `<CodeBlock>` 而不是 Markdown 的三重反引號 ` ``` `，因為後者會將其內容按原樣輸出，而您希望在此處插入導入的文本。
...
按照慣例，使用 **`_` 文件名前綴** 不會創建任何文檔頁面，這意味著該 Markdown 文件是一個 **「部分」**，將被其他文件導入。
