...
您可能會意外建立多個應透過相同路由存取的頁面。發生此情況時，Docusaurus 會在執行 `yarn start` 或 `yarn build` 時警告重複路由（可透過 [`onDuplicateRoutes`](../api/docusaurus.config.js.mdx#onDuplicateRoutes) 配置調整此行為），但網站仍會成功建置。最後建立的頁面將可存取，但會覆蓋其他衝突頁面。解決此問題需修改或移除所有衝突路由。