...
這稱為「分層架構」：優先級較高的層級提供的元件會遮蔽優先級較低的層級，從而實現元件替換（swizzling）。假設有以下結構：
...
當匯入 `@theme/Navbar` 時，`website/src/theme/Navbar.js` 會優先被使用。這種行為稱為元件替換（component swizzling）。如果你熟悉 Objective C，其中函數的實作可以在運行時被替換，這裡的概念完全相同，只是改變 `@theme/Navbar` 指向的目標！
...
這些別名可能讓人難以理解。讓我們想像一個極其複雜的情況：三個主題/插件和網站本身都試圖定義同一個元件。Docusaurus 在內部將這些主題載入為一個「堆疊」。
...
這個「堆疊」中的元件按照「預設插件 > 預設主題 > 插件 > 主題 > 網站」的順序被推入，因此 `website/src/theme` 中被替換的元件總是位於頂部，因為它是最後被載入的。
...
`@theme-init/*` 始終指向最底層的元件——通常，這來自首次提供此元件的主題或插件。個別插件/主題可以安全地使用 `@theme-init/CodeBlock` 來取得其基本版本。網站創建者通常不應使用此別名，因為你可能希望增強的是最頂層而非最底層的元件。此外，`@theme-init/CodeBlock` 別名可能根本不存在——Docusaurus 僅在它指向與 `@theme-original/CodeBlock` 不同的元件時（即當它由多個主題提供時）才會創建此別名。我們不會浪費別名！

## 客戶端模組 {#client-modules}

客戶端模組是您網站套件的一部分，與主題元件類似。然而，它們通常具有副作用。客戶端模組可以是任何能被 Webpack `import` 的內容——CSS、JS 等。JS 腳本通常在全局上下文中運行，例如註冊事件監聽器、創建全局變量...
...
由於 Docusaurus 構建的是單頁應用，`script` 標籤僅在頁面首次加載時執行，不會在頁面切換時重新執行。這些生命週期對於需要在每次新頁面加載時執行的命令式 JS 邏輯非常有用，例如操作 DOM 元素、發送分析數據等。
...
`onRouteUpdate` 會在事件 (2) 時調用，`onRouteDidUpdate` 會在事件 (4) 時調用。它們都會接收當前位置和先前位置（可能是 `null`，如果是首次加載）。

`onRouteUpdate` 可以選擇性地返回一個「清理」回調函數，該函數會在事件 (3) 時調用。例如，如果您想顯示進度條，可以在 `onRouteUpdate` 中啟動一個計時器，並在回調中清除計時器。（經典主題已通過這種方式提供了 `nprogress` 集成。）

請注意，新頁面的 DOM 僅在事件 (4) 時可用。如果您需要操作新頁面的 DOM，可能會希望使用 `onRouteDidUpdate`，它會在新頁面的 DOM 掛載後立即觸發。
...
客戶端模組生命週期完全是命令式的，您無法在其中使用 React 鉤子或存取 React 上下文。如果您的操作是狀態驅動的或涉及複雜的 DOM 操作，應考慮[替換元件](../swizzling.mdx)替代。
