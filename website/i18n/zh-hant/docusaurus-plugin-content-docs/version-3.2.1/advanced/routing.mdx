...
Docusaurus 的路由系統遵循單頁應用程式(SPA)的慣例：一個路由對應一個元件。本節將先說明三種內容外掛(文件、部落格和頁面)內部的路由機制，接著深入探討底層路由系統的運作原理。

## 內容外掛中的路由 {#routing-in-content-plugins}

每個內容外掛都提供 `routeBasePath` 選項，用於定義外掛附加路由的基礎路徑。預設情況下，文件外掛將路由置於 `/docs` 下；部落格外掛使用 `/blog`；頁面外掛則使用 `/`。您可以將路由結構想像成這樣：
...
系統會將任何路由與此嵌套路由配置進行比對，直到找到匹配項。例如當遇到 `/docs/configuration` 路由時，Docusaurus 會先進入 `/docs` 分支，然後在文件外掛創建的子路由中進行搜尋。

修改 `routeBasePath` 可有效改變網站的路由結構。例如在[純文件模式](../guides/docs/docs-introduction.mdx#docs-only-mode)中，我們提到將文件的 `routeBasePath` 設為 `'/'` 意味著文件外掛創建的所有路由將不再有 `/docs` 前綴，但這並不影響其他外掛(如部落格)創建 `/blog` 等子路由。

接下來，我們將分別檢視這三種外掛如何組織各自的「子路由區塊」。

### 頁面路由 {#pages-routing}

頁面路由最為直觀：檔案路徑直接映射為URL，無法透過其他方式自訂。詳見[頁面文件](../guides/creating-pages.mdx#routing)。
...
文件外掛是唯一會創建**嵌套路由**的外掛。頂層會註冊[版本路徑](../guides/docs/versioning.mdx)：`/`, `/next`, `/2.0.0-beta.13`... 這些路徑提供版本上下文(包括佈局和側邊欄)，確保切換不同文件時能保持側邊欄狀態，並可透過導航欄下拉選單切換版本而不離開當前文件。使用的元件是 `@theme/DocPage`。
...
個別文件內容會渲染在導航欄、頁尾、側邊欄等由 `DocPage` 元件提供的框架之後的剩餘空間。例如本頁面 <URLPath /> 是由 <FilePath /> 檔案生成。使用的元件是 `@theme/DocItem`。
...
關於內容插件的部分就談到這裡。讓我們退一步，談談 Docusaurus 應用中的路由運作原理。
...
路由 `/docs/advanced/routing` 可能對應 `/docs/advanced/routing/index.html` 或 `/docs/advanced/routing.html`。部分託管服務商會根據結尾斜線的存在與否區分二者，且可能容忍或不容忍另一種形式。詳見 [結尾斜線指南](https://github.com/slorber/trailing-slash-guide)。
...
所有 HTML 檔案都會使用絕對 URL 參照其 JS 資源，因此為了正確定位資源，您必須配置 `baseUrl` 欄位。請注意，`baseUrl` 不會影響生成套件的檔案結構：基礎 URL 位於 Docusaurus 路由系統的上層。您可以將 `url` 和 `baseUrl` 的組合視為 Docusaurus 站點的實際位置。
...
客戶端提供 `@docusaurus/router` 來存取頁面路由，該套件重新導出 [`react-router-dom`](https://www.npmjs.com/package/react-router-dom/v/5.3.0) 功能。例如使用 `useLocation` 取得當前頁面的[位置物件](https://developer.mozilla.org/en-US/docs/Web/API/Location)，或透過 `useHistory` 存取[歷史物件](https://developer.mozilla.org/en-US/docs/Web/API/History)（功能類似瀏覽器 API 但實作不同，詳見 React Router 文件）。
...
Docusaurus 建構的[單頁應用](https://developer.mozilla.org/en-US/docs/Glossary/SPA)中，路由轉換透過 React Router 的 `history.push()` 方法實現。此操作在客戶端執行，但前提是目標 URL 必須已被路由器識別，否則路由器會攔截路徑並顯示 404 頁面。

若將 HTML 頁面置於 `static` 資料夾，它們會被複製到建置輸出中成為網站可存取內容，但不屬於 Docusaurus 路由系統。我們提供 `pathname://` 協議，讓你能以非 SPA 方式跳轉至網域其他部分，如同外部連結般運作。
...
Docusaurus 僅會移除 `pathname://` 前綴，不對內容進行其他處理。