...
使用 `npx create-docusaurus@latest website classic` 初始化新網站（如[此範例](https://github.com/facebook/docusaurus/tree/main/examples/classic)）。
...
語言環境名稱用於翻譯檔案的存放位置，以及翻譯後語言環境的基礎 URL。在建置所有語言環境時，只有預設語言環境的名稱會從基礎 URL 中省略。
...
根據語境語義選擇更合適的形式。例如，`<Translate>` 可作為 React 子元素使用，而對於需要字串的 props，則可使用回調函數。

:::warning

JSX 元素是_物件_而非字串。在需要字串的上下文中使用（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會[強制轉換為字串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，導致返回 `"[object Object]"`。雖然我們鼓勵將 `<Translate>` 作為 JSX 子元素使用，但請僅在確實適用的情境下使用元素形式。
...
`docusaurus write-translations` 命令會靜態分析網站中所有 React 程式碼文件，提取這些 API 的調用，並將其聚合到 `code.json` 文件中。翻譯文件會以從 ID 到翻譯訊息物件（包含翻譯後的標籤和標籤描述）的映射形式儲存。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定預設未翻譯的訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯起來。

:::warning[文字標籤必須是靜態的]

`docusaurus write-translations` 命令僅對程式碼進行**靜態分析**，不會實際運行您的網站。因此，無法提取動態訊息，因為訊息是_表達式_而非_字串_：
...
function ItemsList() {
  return (
    <ul>
      {/* 請勿這樣做：無法與 write-translations 命令協同工作 */}
      {items.map((item) => (
        <li key={item.id}>
...
此代碼在運行時仍能正常運作。但未來我們可能提供「無運行時」機制，通過 Babel 轉換直接將翻譯內聯到 React 代碼中，而非在運行時調用 API。因此，為確保未來兼容性，應始終優先使用可靜態分析的訊息。例如，我們可將上述代碼重構為：
...
每種語言都有一組[可能的複數類別](https://unicode-org.github.io/cldr-staging/charts/37/supplemental/language_plural_rules.html)。Docusaurus 會按照 `["zero", "one", "two", "few", "many", "other"]` 的順序排列這些類別。例如，由於英語 (`en`) 有兩種複數形式（"one" 和 "other"），翻譯訊息會用管道符 (`|`) 分隔兩個標籤。而波蘭語 (`pl`) 有三種複數形式（"one"、"few" 和 "many"），您需要按順序提供三個用管道符連接的標籤。
...
該指令會提取並初始化您需要翻譯的 JSON 翻譯文件。根目錄下的 `code.json` 包含從原始碼中提取的所有翻譯 API 呼叫，這些可能是您編寫的或由主題提供的，其中部分可能已預設翻譯完成。
...
官方 Docusaurus 內容外掛廣泛使用 Markdown/MDX 文件，並允許您對其進行翻譯。
...
也可以將每個語言版本部署為獨立的子網域，並在 CDN 層級將子網域組合成單一統一的網域：
