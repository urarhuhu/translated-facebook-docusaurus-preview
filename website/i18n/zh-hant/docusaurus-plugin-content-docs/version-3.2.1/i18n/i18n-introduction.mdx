...
為一個簡單的法語版 Docusaurus 網站進行翻譯後，檔案結構會呈現如下：
...
JSON 檔案可透過 [`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI 指令初始化。各插件會在其對應資料夾下提供翻譯內容，而 `code.json` 檔案則定義 React 程式碼中使用的所有文字標籤。

每個內容插件或主題皆不同，並**自行定義翻譯檔案位置**：
