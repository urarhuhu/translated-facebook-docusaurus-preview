...
您也可以透過 `blogSidebarTitle` 選項修改側邊欄的標題文字。例如，如果您設定了 `blogSidebarCount: 'ALL'`，可以將預設的「近期文章」改為「所有文章」：
...
此命名規則為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若需要更精確的發布時間，可使用 front matter 設定。例如，當同一天發布多篇文章時，可依照當日時間排序：
...
<details>
  <summary>混用內嵌作者與全域作者</summary>

您可主要使用全域作者，同時保留內嵌作者功能：
...
無論透過 front matter 或作者對照表宣告的作者，皆需具備名稱或頭像（或兩者兼具）。若某篇文章的所有作者皆未提供名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[Feed generation]

[RSS 訂閱](#feed)需要設定作者電子郵件，才能在訂閱中顯示作者資訊。
...
預設的閱讀時間計算可接受額外選項：`wordsPerMinute`（數字，預設值為 300）和 `wordBound`（從字串到布林值的函數）。如果傳遞給 `wordBound` 的字串應被視為單詞邊界（預設為空格、製表符和換行符），則該函數應返回 `true`。
...
您可以通過設定 `feedOptions` 來生成 RSS/Atom/JSON 訂閱。預設情況下會生成 RSS 和 Atom 訂閱。若要停用訂閱生成，請將 `feedOptions.type` 設為 `null`。
...
您可以運行不帶專用首頁的 Docusaurus 站點，改以部落格文章列表頁作為首頁。將 `routeBasePath` 設為 `'/'`，即可透過根路徑 `example.com/`（而非子路徑 `example.com/blog/`）提供部落格服務。
...
若停用文件（docs）插件，請務必刪除配置文件中其他對該插件的引用。尤其需移除導航欄中與文件相關的項目。

:::

:::tip

亦有「純文件模式」供僅需使用文件的用戶。詳閱[純文件模式](./guides/docs/docs-introduction.mdx)獲取詳細指引或更深入的`routeBasePath`說明。

:::

### 多重部落格 {#multiple-blogs}

預設情況下，經典主題假設每個網站僅有一個部落格，因此僅包含一個部落格插件實例。若需在單一網站上運行多個部落格，亦可實現！您可透過在`docusaurus.config.js`的`plugins`選項中配置另一個部落格插件來新增。

將`routeBasePath`設為您希望第二個部落格存取的URL路徑。請注意此處的`routeBasePath`必須與第一個部落格不同，否則可能發生路徑衝突！同時將`path`設為包含第二個部落格條目的目錄路徑。
...
範例可參見我們託管的第二個部落格[此處](/tests/blog)。