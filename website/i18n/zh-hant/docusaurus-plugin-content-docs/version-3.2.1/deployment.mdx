...
預設值通常運作良好。不幸的是，每個靜態託管提供商都有**不同的行為**，將相同的網站部署到不同的主機可能會導致不同的結果。根據您的主機，更改此配置可能會有所幫助。
...
將潛在敏感信息放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 文件是唯一與 Node.js 環境交互的接口（參見[我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接訪問 `process` 全局變量。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
- Jamstack 提供商（例如 [Netlify](#deploying-to-netlify) 和 [Vercel](#deploying-to-vercel)）。我們將以它們為參考，但相同的邏輯可以應用於其他提供商。
...
以下章節將介紹常見託管服務供應商及其針對 Docusaurus 網站的最佳化部署設定。Docusaurus 與這些服務無商業合作關係，此處資訊僅供參考。部分說明由第三方提供，可能未即時反映最新 API 變更。若發現內容過時，歡迎提交 PR 修正。

由於我們僅能提供盡力而為的維護，目前已停止接受新增託管選項的 PR。您可將部署指南發表於其他平台（如個人部落格或服務商官網），並聯繫我們加入相關連結。
...
完成上述設定後，網站將自動部署並在合併至預設部署分支（通常為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 網站將 `docs` 目錄置於 `website` 外層（多為舊版 Docusaurus v1 結構）：
...
若選擇將 `website` 設為 Netlify 基礎目錄，需配置 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/) 以避免 `docs` 更新時未觸發建置：
...
建議關閉 Netlify 設定中的 `Post Processing > Asset Optimization > Pretty Urls` 選項，以避免強制小寫化 URL、非必要重新導向及 404 錯誤。

**重要注意**：全局選項 `Disable asset optimization` 存在功能缺陷，實際上無法關閉 `Pretty URLs` 設定。請務必手動獨立取消勾選該選項。
...
要透過 [Vercel Git 整合功能](https://vercel.com/docs/concepts/git)部署專案，請確保程式碼已推送至 Git 儲存庫。
...
GitHub Pages 會從預設分支（通常是 `master` 或 `main`）或 `gh-pages` 分支，以及從根目錄或 `/docs` 資料夾中擷取已準備好部署的檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `設定 > Pages` 來配置此選項。此分支將被稱為「部署分支」。
...
這些欄位也有對應的環境變數，其優先級更高：`ORGANIZATION_NAME`、`PROJECT_NAME` 和 `DEPLOYMENT_BRANCH`。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（_來源分支_ 為 `main`），且您的 [發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 已設定為 [使用自訂 GitHub Actions 工作流程發佈](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的拉取請求提交至 `main` 時，執行一個確保網站成功建置的動作（但不實際部署）。此工作稱為 `test-deploy`。
...
跨儲存庫發佈的設定較為複雜，因為您需要推送至另一個需權限檢查的儲存庫。我們將使用 SSH 進行驗證。
...
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其作為 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys) 貼到部署儲存庫中。如果命令行對您無效，請複製檔案內容。在儲存部署金鑰前，勾選 `允許寫入存取` 的選項。
...
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私密金鑰，並在來源儲存庫上貼上一個名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令行對您無效，請複製檔案內容。儲存您的秘密。
...
- 來源儲存庫已設定 GitHub 工作流程，並將私密 SSH 金鑰設為 GitHub 秘密，以及
...
持續整合 (CI) 服務通常用於在每次新提交檢入到原始碼控制時執行例行任務。這些任務可以是執行單元測試和整合測試、自動化建置、將套件發佈到 npm，以及將變更部署到您的網站等任何組合。要自動化網站的部署，您只需在網站更新時調用 `yarn deploy` 腳本。以下部分介紹如何使用 [Travis CI](https://travis-ci.com/)（一個流行的持續整合服務提供商）來實現這一點。
...
2. 使用您的 GitHub 帳戶，[添加 Travis CI 應用程式](https://github.com/marketplace/travis-ci) 到您想要啟動的儲存庫。
3. 開啟您的 Travis CI 儀表板。網址類似於 `https://travis-ci.com/USERNAME/REPO`，並導航到您的儲存庫的 `更多選項 > 設定 > 環境變數` 部分。
...
[Buddy](https://buddy.works/) 是一個易於使用的 CI/CD 工具，允許您將入口網站自動部署到不同的環境，包括 GitHub Pages。
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可自動從您的 Git 儲存庫直接建置並部署您的網路應用程式至 AWS Fargate。它讓您能夠完整檢查並進行基礎架構變更，不受傳統 PaaS 的限制。
...
[Koyeb](https://www.koyeb.com) 是一個對開發者友善的無伺服器平台，可讓您在全球部署應用程式。該平台讓您能無縫運行 Docker 容器、網路應用程式和 API，並提供基於 Git 的部署、原生自動擴展、全球邊緣網路，以及內建的服務網格和服務發現功能。請查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 以開始使用。

## 部署到 Render {#deploying-to-render}

[Render](https://render.com) 提供 [免費的靜態網站託管服務](https://render.com/docs/static-sites)，包含全託管的 SSL、自訂網域、全球 CDN，以及從您的 Git 儲存庫自動持續部署的功能。只需幾分鐘即可開始使用，請遵循 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus)。
...
1. 建立 Qovery 帳戶。如果您還沒有帳戶，請訪問 [Qovery 儀表板](https://console.qovery.com) 建立一個。
2. 建立專案。
   - 點擊 **建立專案** 並為您的專案命名。
   - 點擊 **下一步**。
3. 建立新環境。
...
4. 新增應用程式。
   - 點擊 **建立應用程式**，為其命名並選擇存放 Docusaurus 應用程式的 GitHub 或 GitLab 儲存庫。
   - 定義主分支名稱和應用程式的根路徑。
   - 點擊 **建立**。應用程式建立後：
   - 導航至您的應用程式 **設定**
   - 選擇 **連接埠**
...
[Hostman](https://hostman.com/) 允許您免費託管靜態網站。Hostman 自動化所有流程，您只需連接您的儲存庫並遵循以下簡單步驟：
...
4. 部署。
   ...
   - 開始後，您將進入部署日誌。如果程式碼有任何問題，日誌中會顯示警告或錯誤訊息，說明問題原因。通常，日誌會包含您所需的所有除錯資料。
...
1. 首先，使用 npm 安裝 Surge，執行以下指令：
   ...
   npm install -g surge
   ```
2. 若要為您的網站建立生產環境的靜態檔案，請在專案的根目錄執行：
   ```bash npm2yarn
   npm run build
   ```
3. 接著，在專案的根目錄內執行以下指令：
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview)是一項服務，可直接從代碼存儲庫自動構建和部署全棧Web應用程序到Azure，簡化了CI/CD的開發者體驗。Static Web Apps將Web應用程序的靜態資產與其動態（API）端點分開。靜態資產從全球分佈的內容服務器提供，使客戶能夠使用附近的服務器更快地檢索文件。動態API通過基於事件的無服務器架構進行擴展，這種方法更具成本效益並按需擴展。只需幾分鐘即可開始，請按照[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)操作。

## 部署至Kinsta {#deploying-to-kinsta}

[Kinsta靜態網站託管](https://kinsta.com/static-site-hosting)讓您可以免費部署最多100個靜態網站，包括自定義域名與SSL、每月100 GB帶寬和260多個Cloudflare CDN位置。

只需點擊幾下即可開始，請參閱我們的[Docusaurus on Kinsta](https://kinsta.com/docs/docusaurus-example/)文章。