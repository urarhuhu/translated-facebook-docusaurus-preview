...
有時您需要在網站發布公告。為此可添加公告欄——這是一個非固定且可選擇關閉的面板，位於導航欄上方。所有配置皆位於 `announcementBar` 物件中。
...
標誌可放置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站基礎URL。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，這在將文檔網站託管於主站子目錄時特別有用（此時可能不需要標誌連結在主站新分頁開啟）。

為增強深色模式支援，可為此模式設定不同標誌。
...
可透過 `themeConfig.navbar.items` 添加導航欄項目。
...
項目行為可根據 `type` 欄位變化。以下章節將介紹所有可用的導航欄項目類型。

#### 導航欄連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下您可以使用 `activeBasePath`。若某個連結需要在多個不同路徑下保持活動狀態（例如當您在同一側邊欄下有多個文檔資料夾時），可以使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先於它——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
除了上述欄位外，您還可以指定其他任意屬性應用於 HTML 連結。
...
- [自訂 HTML 的導航欄](#navbar-with-custom-html)

請注意，下拉選單基底項目本身也是可點擊的連結，因此該項目可接收[純導航欄連結](#navbar-link)的任何屬性。
...
使用者可在不同版本間切換，同時保持停留在同一文檔（只要文檔 ID 在各版本間保持一致）。
...
若啟用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導航欄項目會顯示包含所有可用語言選項的下拉選單。

使用者可切換不同語言版本，同時保持停留在當前頁面。
...
若啟用[搜尋功能](../../search.mdx)，搜尋欄預設會顯示在導航欄最右側。

透過此特殊項目類型，您可以變更搜尋欄的預設位置。
...
可啟用此UI特效：當使用者向下滾動頁面時自動隱藏導航欄，向上滾動時重新顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。預覽效果請參閱[Infima文件](https://infima.dev/docs/components/navbar/)。
...
Docusaurus採用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現程式碼高亮，所有配置皆位於`prism`物件中。
...
預設使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。您可從[可用主題清單](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自訂主題，亦可為深色模式指定不同的高亮主題。
...
如果您使用行高亮標記語法，可能需要為深色模式的語法高亮主題指定不同的高亮背景顏色。請參考[相關文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)獲取指引。
...
您可以透過`themeConfig.footer`添加標誌和版權聲明。標誌可放置在[靜態資料夾](static-assets.mdx)中，其URL運作方式與導覽列標誌相同。
...
多欄頁尾連結會為每個欄位設定`title`和一組`FooterItem`清單。
...
用於存取色彩模式的React鉤子。此上下文包含設定亮色與深色模式的函數，並暴露當前使用模式的布林變數。
