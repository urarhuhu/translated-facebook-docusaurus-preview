...
全域資料是...全域的：其大小會影響網站所有頁面的載入時間，因此請盡量保持其小型化。在可能的情況下，優先使用 `createData` 和頁面特定資料。
...
`configureWebpack` 會被呼叫並帶入由插件載入的內容。
...
在生成客戶端套件時，修改 [`postcss-loader` 的 `postcssOptions`](https://webpack.js.org/loaders/postcss-loader/#postcssoptions)。
...
`injectHtmlTags` 會被呼叫並帶入由插件載入的內容。
