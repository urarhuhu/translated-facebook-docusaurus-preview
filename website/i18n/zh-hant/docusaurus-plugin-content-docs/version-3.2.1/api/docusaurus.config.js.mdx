...
- `localeConfigs`: 每個語言環境的個別選項。
  ...
  - `calendar`: 用於計算日期時代的[日曆](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale/calendar)。注意它不控制實際顯示的字符串格式：`MM/DD/YYYY`和`DD/MM/YYYY`都屬於`gregory`。要選擇格式（`DD/MM/YYYY`或`MM/DD/YYYY`），請將語言環境名稱設置為`en-GB`或`en-US`（`en`表示`en-US`）。
  - `path`: 該語言環境所有插件本地化文件夾的根目錄。將相對於`i18n.path`解析。默認為語言環境的名稱。注意：這不影響語言環境的`baseUrl`—基礎URL的定制正在開發中。
...
Docusaurus檢測到任何失效鏈接時的行為。

默認情況下，它會拋出錯誤，以確保您不會發布任何失效鏈接。

:::note

失效鏈接檢測僅在生產構建（`docusaurus build`）時可用。
...
Docusaurus檢測到使用Docusaurus的`Heading`組件聲明的失效錨點時的行為。

默認情況下，它會打印警告，讓您知道失效的錨點。

### `onBrokenMarkdownLinks` {#onBrokenMarkdownLinks}

- 類型: `'ignore' | 'log' | 'warn' | 'throw'`

Docusaurus檢測到任何失效的Markdown鏈接時的行為。

默認情況下，它會打印警告，讓您知道失效的Markdown鏈接。
...
擁有該儲存庫的 GitHub 使用者或組織。若您不使用 `docusaurus deploy` 指令則不需設定此項。
...
GitHub 儲存庫名稱。若您不使用 `docusaurus deploy` 指令則不需設定此項。
...
部署靜態檔案的分支名稱。若您不使用 `docusaurus deploy` 指令則不需設定此項。
...
您的伺服器主機名稱。適用於 GitHub Enterprise 使用者。若您不使用 `docusaurus deploy` 指令則不需設定此項。
...
您的伺服器連接埠。適用於 GitHub Enterprise 使用者。若您不使用 `docusaurus deploy` 指令則不需設定此項。
...
詳見[插件方法參考](./plugin-methods/README.mdx)了解 `PluginModule` 的結構。
...
一個路徑數組，可以是相對於網站目錄或絕對路徑。這些路徑下的文件將原樣複製到構建輸出中。
...
一個要加載的腳本數組。值可以是字符串或屬性-值映射的普通對象。`<script>` 標籤將插入到 HTML `<head>` 中。如果使用普通對象，唯一必需的屬性是 `src`，並且允許任何其他屬性（每個屬性應具有布爾值/字符串值）。
...
一個要加載的 CSS 源數組。值可以是字符串或屬性-值映射的普通對象。`<link>` 標籤將插入到 HTML `<head>` 中。如果使用對象，唯一必需的屬性是 `href`，並且允許任何其他屬性（每個屬性應具有布爾值/字符串值）。
...
默認情況下，`<link>` 標籤將具有 `rel="stylesheet"`，但您可以顯式添加自定義的 `rel` 值來注入任何類型的 `<link>` 標籤，不一定是樣式表。

:::

### `clientModules` {#clientModules}

一個要在您的網站上全局加載的[客戶端模塊](../advanced/client.mdx#client-modules)數組。
...
一個使用 [Eta 語法](https://eta.js.org/docs/syntax#syntax-overview)編寫的 HTML 模板，用於渲染您的應用程序。這可以用來設置 `body` 標籤上的自定義屬性、額外的 `meta` 標籤、自定義 `viewport` 等。請注意，Docusaurus 將依賴於模板的正確結構才能正常運行，一旦您自定義它，您必須確保您的模板符合上游的要求。
...
啟用時，若網站無法載入 CSS 或 JavaScript 檔案（常見問題，通常與網站配置中錯誤的 `baseUrl` 有關），將顯示警示橫幅。
...
![基本網址問題警示橫幅範例。由於樣式表載入失敗，樣式呈現原始狀態。文字內容為「您的 Docusaurus 網站未能正確載入...當前設定的 baseUrl = /（預設值）；建議嘗試 baseUrl = /build/」](/img/baseUrlIssueBanner.png)

:::warning

此橫幅需內聯 CSS/JS，以防因基本網址錯誤導致所有資源載入失敗。
