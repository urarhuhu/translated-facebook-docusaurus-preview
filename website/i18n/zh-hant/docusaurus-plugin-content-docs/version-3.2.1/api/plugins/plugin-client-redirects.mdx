...
該插件會向靜態站點寫入額外的 HTML 頁面，通過 JavaScript 將用戶重定向至現有的 Docusaurus 頁面。
...
本插件會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，當多個文件被生成至相同路徑時，據此調整日誌級別。
...
"來源"與"目標"是本插件的核心概念。"來源"指您希望創建的路徑，即待寫入的額外HTML文件；"目標"指需重定向至的路徑，通常為Docusaurus已知路由。

因此可為同一"目標"設置多個"來源"：我們將創建多個均指向同一目標的HTML文件。反之，單個"來源"絕不能對應多個"目標"：生成的HTML文件必須有確定的重定向目標。
