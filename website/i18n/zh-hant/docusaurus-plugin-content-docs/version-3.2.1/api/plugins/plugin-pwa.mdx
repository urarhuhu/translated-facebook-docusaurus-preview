...
![應用安裝過程的螢幕錄製。瀏覽器地址欄中出現一個按鈕，點擊後顯示「安裝此應用？」的對話框。點擊「安裝」按鈕後，操作系統中會打開一個新應用，顯示 Docusaurus 首頁。](/img/pwa_install.gif)
...
>
> 這樣做的主要原因是讓開發者能控制緩存，意味著他們可以決定文件何時被緩存、緩存多久，以及不通過網路直接提供給瀏覽器，從而創建可離線運行的網路應用。
>
> Workbox 通過簡化 API 和確保資源高效下載，大幅降低了預緩存的實現難度。

預設情況下，當站點作為應用安裝時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

站點預緩存後，service worker 會為後續訪問提供緩存響應。當新構建部署並伴隨新 service worker 時，新版本會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗，要求用戶刷新頁面獲取新內容。在用戶清除應用緩存或點擊彈窗中的 `reload` 按鈕前，service worker 將持續提供舊內容。
...
- `standalone`：對以獨立應用形式運行的使用者啟用（通常是安裝 PWA 後的情況）
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入到 `<head>` 標籤。技術上可透過此注入任何 head 標籤，但理想情況下用於使網站符合 PWA 規範的標籤。以下是使應用完全合規的標籤列表：
...
適用於額外的 Workbox 規則。您可在此執行任何 service worker 能做的操作，並充分利用 workbox 函式庫。程式碼會經過轉譯，因此可使用現代 ES6+ 語法。
...
當新的 Service Worker 等待安裝時，會渲染 `@theme/PwaReloadPopup` 元件並向使用者建議重新載入。您可以[替換](../../swizzling.mdx)此元件並實作自己的 UI 介面。該元件會接收一個 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時呼叫此函式。這將指示 Service Worker 安裝等待中的 Service Worker 並重新載入頁面。

預設主題包含重新載入彈出視窗的實作，並使用 [Infima 警示元件](https://infima.dev/docs/components/alert)。

![重新載入流程的畫面錄製。視窗右下角顯示警示框，內容為「有新內容可用」。點擊「重新整理」按鈕後，頁面主標題從「簡介」變更為「PWA :))」](/img/pwa_reload.gif)
