...
實務上，元件替換允許您**用自己實作的元件替換主題元件**，主要分為兩種模式：

- [**完整替換 (Ejecting)**](#ejecting)：建立原始主題元件的**副本**，您可進行完整**客製化**
- [**封裝替換 (Wrapping)**](#wrapping)：建立原始主題元件的**封裝層**，您可在此基礎上**增強功能**
...
完整替換[**不安全**](#what-is-safe-to-swizzle)元件時，可能會導致複製大量內部代碼，這些代碼現在需要由您自行維護。這可能使 Docusaurus 升級更加困難，因為若接收的 props 或使用的內部主題 API 發生變更，您需要遷移自訂內容。

**請盡可能優先選擇[封裝替換](#wrapping)**：需維護的代碼量較少。
...
- **禁止**：混寫 CLI 會阻止您混寫此元件，因為它根本不設計為可混寫
...
如果您有**強烈需求要混寫不安全元件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找解決方案使其安全。

:::

## 應該混寫哪個元件？ {#which-component-should-i-swizzle}

要達成預期效果，通常不清楚究竟該混寫哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 約有[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
2. **元件名稱**：官方主題元件採用語義化命名，通常可從名稱推斷功能。Swizzle CLI 支援輸入部分元件名稱來篩選選項（例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件）。
3. **從高層級元件開始**：元件會形成樹狀結構，每個路由會關聯一個最頂層的渲染元件（多數列於[內容外掛路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的頂層元件是 `@theme/BlogPostPage`。可先swizzle此元件，再向下追蹤樹狀結構找到目標元件。找到正確元件後，請刪除其他暫存的swizzle檔案以避免維護過多元件。
...
我們也希望了解更進階的客製化案例，歡迎[提交討論](https://github.com/facebook/docusaurus/discussions/5468)。
...
1. **使用CSS**：CSS規則與選擇器通常能滿足基礎客製化需求，詳見[樣式與版面](./styling-layout.mdx)。
2. **使用翻譯功能**：翻譯本質上也是一種文字標籤客製化方式。例如預設語言為`en`時，仍可執行`yarn write-translations -l en`後編輯生成的`code.json`，詳見[i18n教學](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**：若必須swizzle，請僅針對相關部分操作，並盡量減少自行維護的程式碼量。swizzle小型元件通常能降低升級時的**破壞性變更**風險。

相較於[完整替換(ejecting)](#ejecting)，[包裝(wrapping)](#wrapping)是更安全的選擇。

:::

## 使用`<Root>`包裝你的網站 {#wrapper-your-site-with-root}

`<Root>`元件會渲染在React樹的最頂層（高於主題`<Layout>`），且**永遠不會卸載**。非常適合用來添加不該因導航而重置的狀態邏輯（使用者認證狀態、購物車狀態等）。
