...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)之後執行，以補足缺失部分或調試遷移 CLI 輸出的問題。
...
此舉能明確區分 Docusaurus 官方套件與社群維護套件。換言之，所有官方套件均歸屬於 `@docusaurus/` 命名空間下。
...
請使用最新的 Docusaurus 2 版本（可透過 [`latest` 標籤](https://www.npmjs.com/package/@docusaurus/core)查閱）。
...
Docusaurus 1 的建構產出物位於 `website/build/<PROJECT_NAME>`。

Docusaurus 2 則簡化為 `website/build`，請確認部署配置已更新為從正確的 `build` 目錄讀取檔案。

若部署至 GitHub Pages，請改為執行 `yarn deploy` 而非 `yarn publish-gh-pages` 指令。
...
原有 D1 網站可能已有 README 文件，可修改內容以反映 D2 變更，或直接複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。

## 站點配置 {#site-configurations}

### `docusaurus.config.js` {#docusaurusconfigjs}

將 `siteConfig.js` 重新命名為 `docusaurus.config.js`。

Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集（preset）是插件的組合包，為保持向後兼容性，我們建立了 `@docusaurus/preset-classic` 預設集，其中包含 Docusaurus 1 的大部分核心插件。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完全就緒，我們會在準備好後更新此處。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
您可以聯繫 DocSearch 團隊（@shortcuts, @s-pace）尋求支援。他們可以為您更新配置並觸發重新爬取網站以恢復搜索功能（否則您可能需要等待最多 24 小時直到下一次計劃爬取）。
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 檔案，並填入您的自訂網域。執行建置命令時，`static` 資料夾中的檔案將被複製到 `build` 資料夾的根目錄中。
...
v2 版本需在每個頁面套用主題佈局。Layout 元件會接收 metadata props。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需修正不相容語法。常見範例如：HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
欄位 `authorFBID` 與 `authorTwitter` 已被棄用。這些欄位僅用於生成作者頭像，現可透過 `authors` 欄位實現相同功能。
