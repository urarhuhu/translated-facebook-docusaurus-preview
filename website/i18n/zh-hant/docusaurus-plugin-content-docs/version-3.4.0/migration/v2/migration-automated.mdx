...
- `/blog` 目錄
- `/static` 目錄
- 若您的網站使用版本控制，還會遷移 `versioned_sidebar.json` 和 `/versioned_docs`
...
遷移 CLI 會直接修改現有文件。請務先提交這些文件的當前狀態！

:::

#### 選項 {#options}

您可以為遷移 CLI 添加選項標誌來自動將 Markdown 內容和頁面遷移至 v2。通常仍需手動調整以達到理想效果。
...
建議先不帶 `--page` 和 `--mdx` 選項執行遷移，提交後再嘗試使用這些選項重新遷移。
