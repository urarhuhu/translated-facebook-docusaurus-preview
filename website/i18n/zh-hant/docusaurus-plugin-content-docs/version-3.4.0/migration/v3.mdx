...
MDX 遊樂場是您的新好朋友。它允許您理解您的內容如何**被編譯為 React 組件**，並在隔離環境中解決編譯或渲染問題。
...
`{` 字符用於開啟 [JavaScript 表達式](https://mdxjs.com/docs/what-is-mdx/#expressions)。如果您在 `{expression}` 中放入的內容不是有效的表達式，MDX 現在將會失敗。
...
> 在名稱前出現意外字符 `5` (U+0035)，預期應為字母、`$` 或 `_` 等可作為名稱起始的字符
...
> 名稱中出現意外字符 `@` (U+0040)，預期應為字母、數字、`$` 或 `_` 等名稱字符；屬性前的空白；或標籤結束符（注意：在 MDX 中建立連結請使用 `[文字](網址)`）

> 在本地名稱前出現意外字符 `/` (U+002F)，預期應為字母、`$` 或 `_` 等可作為名稱起始的字符（注意：在 MDX 中建立連結請使用 `[文字](網址)`）
...
指令會被解析，目的是由其他 Remark 插件處理。未處理的指令將被忽略，並且不會以其原始形式重新渲染。
...
新的 MDX 解析器現在嚴格遵循 CommonMark 規範。自 v0.14 版本起，CommonMark 規範針對空格和標點符號周圍的強調效果引入了新規則，這尤其會影響不使用空格分隔單詞的語言。
...
如果您想繼續使用 CommonJS 模組，可以使用動態導入作為變通方案，這樣可以在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用非同步函數](/docs/configuration#syntax-to-declare-docusaurus-config)來實現這一點。
...
Node.js 16 [已到達生命週期終點](https://nodejs.org/en/blog/announcements/nodejs16-eol)，Docusaurus v3 現在需要 **Node.js >= 18.0**。
...
您也可以更新網站的 `package.json` 文件以防止使用舊版不支援的 Node.js：
...
請閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，並檢查您自己的 React 程式碼以確定哪些元件可能受到此次升級影響。
...
這是包含破壞性變更的新主版本，我們無法保證嚴格的向後相容性。[`prism-react-renderer` v2 版本說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)並不十分詳細，但 Docusaurus 使用者需要注意三個主要變更。
...
多數 Docusaurus 使用者無需採取任何行動。使用表情符號短代碼的使用者應閱讀 [變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md)，並確認其表情符號仍能如預期呈現。
...
但請注意這是個重大依賴庫更換，可能出現細微的行為變化。

:::

### 警告提示框注意事項

由於歷史原因，我們曾支援未公開的 `:::warning` 警告提示框，其會呈現紅色樣式。
...
然而其顏色與圖標始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，修正了顏色與圖標並將其納入文檔。
...
若您曾使用未公開的 `:::warning` 提示框，請逐一確認黃色是否適用於現有場景。如需保持紅色樣式，請改用 `:::danger`。
...
此項破壞性變更僅影響 **在 v2.0.0-beta.10 版本前（2021年12月）** 啟用文檔版本化的早期 Docusaurus v2 使用者。

當創建 v1.0.0 版本時，側邊欄文件包含 `version-v1.0.0/` 前綴，而 [Docusaurus v3 不再支援此格式](https://github.com/facebook/docusaurus/pull/9310)。
...
`@docusaurus/plugin-content-blog` 現在預設將 RSS 饋送限制為最近20條內容。對於大型 Docusaurus 部落格，這是更合理的預設值以避免饋送文件過度膨脹。
...
技術上**這不屬於破壞性變更**，因這些組件**標記為不安全替換**。但許多 Docusaurus 站點已替換文檔相關組件，需注意自定義內容可能導致異常。

:::info[如何升級]

刪除所有替換組件，重新替換後在新版組件基礎上重新應用自定義內容。

或參考[拉取請求說明](https://github.com/facebook/docusaurus/pull/7966)了解新主題組件樹結構，嘗試手動修補已替換組件。

:::

## 可選變更

部分變更非強制性，但了解這些變化有助於充分發揮 Docusaurus v3 效能。
...
Docusaurus 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實現提示功能，而提供指令標籤的語法需要使用方括號：
...
- 確認所有文檔都能在 [MDX 遊樂場](https://mdxjs.com/playground/) 編譯通過，或使用 [`npx docusaurus-mdx-checker`](https://github.com/slorber/docusaurus-mdx-checker)
...
- 提供可輕鬆執行的**最小化重現範例**，建議使用 [docusaurus.new](https://docusaurus.new) 建立
- 若網站可建構，請提供展示問題的線上部署網址
- 清楚說明問題，避免僅描述模糊的「無法運作」
- 包含所有相關材料：程式碼片段、儲存庫網址、git 分支連結、完整堆疊追蹤、截圖與影片
- 以清晰簡潔的方式提出請求，展現您已付出努力協助我們協助您

您也可以考慮付費請 [Docusaurus 服務供應商](https://github.com/facebook/docusaurus/discussions/9281) 代為執行升級。若您的專案是開源的，還可於社群尋求 [免費的善意協助](https://github.com/facebook/docusaurus/discussions/9283)。