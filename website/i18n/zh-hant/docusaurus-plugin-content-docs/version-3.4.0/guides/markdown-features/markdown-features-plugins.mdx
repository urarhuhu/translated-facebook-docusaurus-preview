...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警告提示](./markdown-features-admonitions.mdx)語法也是由 Remark 插件生成，您可以為自己的使用情境實現類似功能。
...
這些都是 Remark 插件的典型應用場景，也可作為實現自訂插件的靈感來源。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（一種 JavaScript 模組系統），而 Docusaurus 支援此規範。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 設定檔來簡化這類套件的導入流程。

:::

接著，在 `docusaurus.config.js` 中導入您的插件，並透過插件或預設配置將其加入插件選項：
...
部分插件可接受專屬配置選項。此時請使用 `[plugin, pluginOptions]` 語法，如下所示：
...
以下說明**並非**建立外掛程式的完整指南，僅為示範如何使其在 Docusaurus 中運作。如需更深入的運作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 官方文件。

:::

舉例來說，我們建立一個會遍歷所有 `h2` 標題並加上 `第 X 節` 前綴的外掛程式。首先，在任何位置建立外掛程式原始檔（甚至可發佈為獨立 npm 套件後安裝）。我們將檔案放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是接收 `options` 並返回能操作 AST 的 `transformer` 函式。
...
現在你可以在 `docusaurus.config.js` 中導入此外掛程式，用法與安裝的套件完全相同！
...
`transformer` 的第二參數 [`vfile`](https://github.com/vfile/vfile) 可用來取得當前 Markdown 檔案路徑：
...
Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如上方案例中，即使所有 `h2` 標題都加上 `第 X 節` 前綴，生成的目錄仍保持原樣，因為目錄生成外掛程式先於自訂外掛執行。若需在預設外掛前處理 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`：
