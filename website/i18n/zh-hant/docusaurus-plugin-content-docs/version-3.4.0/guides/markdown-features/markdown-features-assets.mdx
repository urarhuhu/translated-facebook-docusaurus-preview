...
有時您會希望直接從 Markdown 檔案連結靜態資源（例如.docx檔案、圖片等），此時將資源與使用它的 Markdown 檔案放在同一位置會很方便。
...
您可以用三種方式顯示圖片：Markdown 語法、CJS require 或 ES 導入語法。
...
如果您想通過 CSS 修改 SVG 圖像的部分內容，這會很有用。例如，您可以根據當前主題更改 SVG 的某種顏色。
...
如果 Markdown 連結或圖片具有絕對路徑，該路徑將被視為檔案路徑，並會從靜態目錄解析。例如，如果您將[靜態目錄](../../static-assets.mdx)配置為 `['public', 'static']`，則對於以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。該連結將被轉換為 `require()` 調用而非保留為 URL，此設計具有兩項優勢：

1. 您無需擔心基礎 URL 問題，Docusaurus 會在提供資源時自動處理；
2. 圖片會進入 Webpack 的建置流程，其名稱將附加雜湊值，使瀏覽器能積極快取圖片並提升網站效能。
