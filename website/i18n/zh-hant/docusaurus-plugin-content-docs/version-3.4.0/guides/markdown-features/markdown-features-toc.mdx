...
每份 Markdown 文件右上角會顯示目錄。預設僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構。若需調整顯示的標題層級範圍，您可以針對單一頁面或全局設定最小與最大標題層級。
...
MDX 文件中可透過 `toc` 變數獲取所有標題。預設僅顯示 `h2` 和 `h3` 標題。您可透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示的標題層級範圍。
