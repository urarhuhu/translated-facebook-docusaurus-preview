...
如果你想引用 **同一個插件包含** 的另一個 Markdown 檔案，可以使用目標文件的相對路徑。Docusaurus 的 Markdown 載入器會將檔案路徑轉換為目標檔案的 URL 路徑（因此會移除 `.md` 副檔名）。
...
Markdown 檔案引用僅在來源檔案和目標檔案由同一個插件實例處理時才能運作。這是我們 Markdown 處理架構的技術限制，未來將會修正。如果你需要在插件之間連結檔案（例如從部落格文章連結到文件頁面），你必須使用 URL 連結。
