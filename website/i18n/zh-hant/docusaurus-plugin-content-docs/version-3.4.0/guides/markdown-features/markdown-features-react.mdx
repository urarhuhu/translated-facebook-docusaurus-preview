...
Docusaurus 內建支援 [MDX](https://mdxjs.com/)，允許您在 Markdown 檔案中撰寫 JSX 並將其渲染為 React 元件。
...
最受歡迎的格式化工具 Prettier [僅支援舊版 MDX v1](https://github.com/prettier/prettier/issues/12209)。若遇到非預期的格式化結果，您可以在問題區域前添加 `{/* prettier-ignore */}`，或將 `*.mdx` 加入 `.prettierignore` 中，直到 Prettier 完整支援 MDX v3。[MDX 主要作者之一推薦使用 `remark-cli` 搭配 `remark-mdx`](https://github.com/orgs/mdx-js/discussions/2067)。
...
請注意它如何同時渲染 React 元件的標記與 Markdown 語法：
...
由於所有文件皆使用 MDX 解析，任何看似 HTML 的內容實際上是 JSX。因此，若需要為元件添加行內樣式，請遵循 JSX 風格並提供樣式物件。
...
`@site` 別名指向您網站的目錄，通常是 `docusaurus.config.js` 檔案所在位置。使用別名而非相對路徑（如 `'../../src/components/BrowserWindow'`）可避免在移動檔案或進行[文件版本控制](../docs/versioning.mdx)與[翻譯](../../i18n/i18n-tutorial.mdx)時更新匯入路徑。

:::

雖然在 Markdown 中宣告元件對於簡單案例非常方便，但由於編輯器支援有限、解析錯誤風險及低重用性，維護會變得困難。當元件涉及複雜 JS 邏輯時，請使用獨立的 `.js` 檔案：
...
若您在多個檔案中使用相同元件，無需在每個檔案中重複匯入——可考慮將其加入全域作用域。[參見下方](#mdx-component-scope)

:::

### MDX 元件作用域 {#mdx-component-scope}

除了[匯入元件](#importing-components)與[匯出元件](#exporting-components)外，第三種在 MDX 中使用元件的方式是**將其註冊到全域作用域**，這將使其自動在所有 MDX 檔案中可用，無需任何匯入語句。
...
在 Docusaurus 中，MDX 元件作用域由 `@theme/MDXComponents` 檔案提供。與大多數其他 `@theme/` 別名下的匯出不同，它本身並非一個 React 元件：而是一個從標籤名稱（如 `Highlight`）映射到其實作 React 元件的記錄。

如果您[替換](../../swizzling.mdx)此元件，將會看到所有已實作的標籤，並可透過替換相應的子元件（如用於渲染[Markdown 程式碼區塊](./markdown-features-code-blocks.mdx)的 `@theme/MDXComponents/Code`）進一步自訂我們的實作。
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)大多相容於[CommonMark](https://commonmark.org/)，但更為嚴格，因為您的 `.mdx` 檔案可使用 JSX 並會被編譯為真正的 React 元件（請參閱[遊樂場](https://mdxjs.com/playground/)）。

某些有效的 CommonMark 功能在 MDX 中無法使用（[更多資訊](https://mdxjs.com/docs/what-is-mdx/#markdown)），特別是：
...
此功能為**實驗性**，目前存在一些[限制](https://github.com/facebook/docusaurus/issues/9092)。
