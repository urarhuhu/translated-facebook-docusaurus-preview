...
將潛在敏感資訊放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是唯一與 Node.js 環境交互的介面（參見 [我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接訪問 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
以下章節將介紹常見託管服務供應商及其針對 Docusaurus 網站的最佳化部署設定。Docusaurus 與這些服務無商業合作關係，此處資訊僅供參考。部分內容由第三方提供，我們可能未即時反映 API 變更。若發現內容過時，歡迎提交 PR 修正。

由於我們僅能提供有限度的維護，目前已停止接受新增託管選項的 PR。您可將部署指南發表於其他平台（如個人部落格或服務商官網），並聯繫我們加入相關連結。

:::

## 部署至 Netlify {#deploying-to-netlify}

要將 Docusaurus 網站部署至 [Netlify](https://www.netlify.com/)，請先確認以下設定：
...
若未即時設定這些建置選項，仍可於站點建立後至「Site settings」→「Build & deploy」進行調整。

完成上述設定後，網站將自動部署，並在程式碼合併至預設的 `main` 部署分支時觸發重新部署。
...
若選擇將 `website` 目錄設為 Netlify 的基礎目錄，需額外設定 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)，否則更新 `docs` 目錄時不會觸發建置：
...
建議停用 Netlify 的「Post Processing > Asset Optimization > Pretty Urls」設定，以避免網址強制小寫化、非必要重新導向及 404 錯誤。

**重要注意**：全局選項「Disable asset optimization」實際無法正確關閉「Pretty URLs」功能，請務必手動獨立取消勾選。
...
要透過 [Vercel Git 整合功能](https://vercel.com/docs/concepts/git)部署專案，請先將程式碼推送至 Git 儲存庫。
...
專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）的任何變更則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始碼檔案的分支，以及包含要透過 GitHub Pages 服務的建置輸出的分支。在以下教學中，將分別稱它們為「來源」和「部署」。

每個 GitHub 儲存庫都關聯一個 GitHub Pages 服務。若部署儲存庫名為 `my-org/my-project`（其中 `my-org` 是組織名稱或使用者名稱），部署後的網站將出現在 `https://my-org.github.io/my-project/`。若部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），網站則會出現在 `https://my-org.github.io/`。

:::info

若想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄內的任何內容都會被複製到部署用的 `build` 目錄根目錄。使用自訂網域時，您應能將 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。

更多細節可參考 GitHub Pages 文件中的 [使用者、組織與專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常是 `master` 或 `main`）或 `gh-pages` 分支，以及根目錄或 `/docs` 資料夾中擷取準備好部署的檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `Settings > Pages` 來設定此選項。此分支將稱為「部署分支」。
...
GitHub Pages 預設會為 Docusaurus URL 加上尾部斜線。建議設定 `trailingSlash` 配置（`true` 或 `false`，而非 `undefined`）。
...
1. 當有新的 pull request 提交到 `main` 分支時，會有一個動作確保網站能成功建置，但不會實際部署。此工作將稱為 `test-deploy`。
2. 當 pull request 合併到 `main` 分支或有人直接推送到 `main` 分支時，將會建置並部署到 GitHub Pages。此工作將稱為 `deploy`。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為已跳過，這無法反映實際狀態且對審查流程毫無幫助。因此，我們建議將它們作為獨立的工作流程管理。
...
跨儲存庫發布的設定較為複雜，因為您需要推送至另一個儲存庫並通過權限檢查。我們將使用 SSH 進行驗證。
...
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其作為 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys) 貼到部署儲存庫中。如果命令行對您無效，請複製檔案內容。在保存部署金鑰之前，勾選 `允許寫入訪問` 的選項。
4. 您需要將私鑰作為 [GitHub 秘密](https://help.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets)，以允許 Docusaurus 為您運行部署。
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私鑰，並在源儲存庫上貼一個名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令行對您無效，請複製檔案內容。保存您的秘密。
...
- 源儲存庫已設置 GitHub 工作流程，並將私鑰 SSH 金鑰作為 GitHub 秘密，以及
- 您的部署儲存庫已在 GitHub 部署金鑰中設置公鑰 SSH 金鑰。
...
持續整合 (CI) 服務通常用於在將新提交檢入源代碼控制時執行例行任務。這些任務可以是運行單元測試和整合測試、自動化構建、將套件發佈到 npm 以及將更改部署到您的網站的任何組合。要自動化網站的部署，您只需在網站更新時調用 `yarn deploy` 腳本。以下部分介紹如何使用 [Travis CI](https://travis-ci.com/)（一個流行的持續整合服務提供商）來實現這一點。

1. 前往 https://github.com/settings/tokens 並生成一個新的 [個人訪問令牌](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。在創建令牌時，授予其 `repo` 範圍，以便其擁有所需的權限。
2. 使用您的 GitHub 帳戶，[添加 Travis CI 應用](https://github.com/marketplace/travis-ci) 到您想要激活的儲存庫。
3. 打開您的 Travis CI 儀表板。URL 類似於 `https://travis-ci.com/USERNAME/REPO`，並導航到您的儲存庫的 `更多選項 > 設置 > 環境變量` 部分。
4. 創建一個名為 `GH_TOKEN` 的新環境變量，其值為您新生成的令牌，然後是 `GH_EMAIL`（您的電子郵件地址）和 `GH_NAME`（您的 GitHub 用戶名）。
5. 在儲存庫的根目錄創建一個 `.travis.yml`，內容如下：
...
3. 選擇 GitHub 作為 Git 託管供應商，並選取存放網站程式碼的儲存庫。
...
建立此簡單管線後，每次推送至所選分支的新提交都會使用 `yarn deploy` 將您的網站部署至 GitHub Pages。閱讀[本指南](https://buddy.works/guides/react-docusaurus)以瞭解更多關於為 Docusaurus 設定 CI/CD 管線的資訊。
...
3. 前往 `https://github.com/USERNAME/REPO/settings/keys` 並新增部署金鑰，貼上剛生成的公鑰。
4. 開啟 Drone.io 儀表板並登入。網址類似 `https://cloud.drone.io/USERNAME/REPO`。
5. 點擊儲存庫，點擊啟用儲存庫，並新增名為 `git_deploy_private_key` 的秘密，其值為剛生成的私鑰。
...
現在，每當您推送新標籤至 GitHub 時，此觸發器將啟動 drone CI 任務以發佈您的網站。

## 部署至 Flightcontrol {#deploying-to-flightcontrol}

[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可直接從您的 Git 儲存庫自動建置並將 Web 應用程式部署至 AWS Fargate。它讓您能夠完整檢視並進行基礎架構變更，不受傳統 PaaS 的限制。
...
[Render](https://render.com) 提供 [免費的靜態網站託管服務](https://render.com/docs/static-sites)，包含全託管 SSL、自訂網域、全球 CDN，以及從您的 Git 倉庫自動持續部署。只需幾分鐘即可開始，請遵循 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus)。
...
3. 配置建置設定。
   ...
   - **應用目錄**指向建置後包含專案檔案的目錄。如果您在步驟 2 中選擇了包含網站內容（或 `my_website`）目錄的儲存庫，可以將其留空。
...
4. 部署。
   ...
   - 部署完成後，您將收到電子郵件通知，並看到日誌條目。一切就緒！您的專案已上線並準備就緒。
...
您可以將 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個用於靜態網站、單頁應用（SPA）和無伺服器功能的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Layer0](https://www.layer0.co) 是一個全功能平台，用於開發、部署、預覽、實驗、監控和運行您的無頭前端。它專注於大型動態網站，並透過 EdgeJS（基於 JavaScript 的內容交付網絡）、預測性預取和性能監控提供一流的性能。Layer0 提供免費層級。只需幾分鐘即可開始，請遵循 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)。
...
[Kinsta 靜態網站託管](https://kinsta.com/static-site-hosting) 讓您可以免費部署最多 100 個靜態網站，包括自定義域名與 SSL、每月 100 GB 帶寬和 260+ Cloudflare CDN 位置。
