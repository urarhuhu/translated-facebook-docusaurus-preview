...
不過您通常不需要逐一手動安裝常見插件：它們可以透過[預設集](#using-presets)打包分發。對多數使用者而言，插件是透過預設集配置來設定的。

我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務... 請務必查閱該清單：可能已有人為您實現了！
...
不過插件可以透過「Babel 風格」的配置方式來指定選項：在配置中使用包含插件名稱和選項物件的二元組。
...
所有 Docusaurus 內容插件都支援多實例運行。例如，您可能需要使用[多文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多部落格](./blog.mdx#multiple-blogs)。這要求為每個插件實例分配唯一ID，預設情況下插件ID為 `default`。
...
最多只能有一個插件實例能成為「預設插件實例」，方式是省略 `id` 屬性，或使用 `id: 'default'`。

:::

## 使用主題 {#using-themes}

主題的載入方式與插件完全相同。對使用者而言，在安裝和配置插件時，`themes` 和 `plugins` 條目可以互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可以覆寫插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析規則](#module-shorthands)，因此若想利用簡寫功能，請務必使用正確的配置項！
...
其中 `moduleType` 為 `'preset'`、`'theme'` 或 `'plugin'` 之一，取決於模組名稱宣告在哪個欄位。第一個成功找到的模組名稱會被載入。
...
以下為註冊於 `plugins` 欄位的插件範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求插件命名規範不同，Docusaurus 允許更自由的命名方式，因此解析結果並非絕對，但會遵循上述優先順序。
