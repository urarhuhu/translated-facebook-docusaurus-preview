...
每個內容外掛都提供 `routeBasePath` 選項，用於定義外掛路由的基礎路徑。預設情況下，文檔外掛的路由位於 `/docs` 下；部落格外掛位於 `/blog`；頁面外掛則位於 `/`。您可以將路由結構想像為：
...
系統會將每個路由與此嵌套配置進行匹配，直到找到對應項目。例如當訪問 `/docs/configuration` 時，Docusaurus 會先進入 `/docs` 分支，再於文檔外掛創建的子路由中搜尋。

修改 `routeBasePath` 可有效改變網站路由結構。例如在[純文檔模式](../guides/docs/docs-introduction.mdx#docs-only-mode)中，將文檔外掛設為 `routeBasePath: '/'` 意味著所有文檔路由將移除 `/docs` 前綴，但這不影響其他外掛（如部落格）創建 `/blog` 等子路由。

接下來我們將分別解析三個外掛如何組織各自的「子路由區塊」。

### 頁面路由 {#pages-routing}

頁面路由最為直觀：檔案路徑直接映射為 URL，且無法透過其他方式自訂。詳見[頁面文檔](../guides/creating-pages.mdx#routing)。
...
- **標籤總覽頁**：`/tags`
  - 可透過 `tagsBasePath` 選項自訂路徑
  - 使用 `@theme/BlogTagsListPage` 元件
- **標籤分類頁**：`/tags/adoption`, `/tags/beta`...
  - 根據文章前置元數據中的標籤生成
  - 基礎路徑由 `tagsBasePath` 定義，子路徑可透過標籤的 `permalink` 欄位自訂
  - 使用 `@theme/BlogTagsPostsPage` 元件
- **歸檔頁**：`/archive`
  - 可透過 `archiveBasePath` 選項自訂路徑
  - 使用 `@theme/BlogArchivePage` 元件

### 文檔路由 {#docs-routing}

文檔外掛是唯一會創建**嵌套路由**的外掛。頂層註冊了[版本路徑](../guides/docs/versioning.mdx)：`/`, `/next`, `/2.0.0-beta.13`... 這些路徑提供版本上下文（包含佈局和側邊欄），確保切換文檔時能保持側邊欄狀態，並可透過導航欄下拉選單切換版本而不離開當前文檔。此處使用 `@theme/DocPage` 元件。
...
個別文檔內容會渲染在導航欄、頁尾、側邊欄等由 `DocPage` 元件提供的框架內。例如本頁面 <URLPath /> 是由檔案 <FilePath /> 生成，使用 `@theme/DocItem` 元件。

文件的 `slug` 前置元數據可自訂路由的最後部分，但基礎路由始終由插件的 `routeBasePath` 和版本的 `path` 定義。

### 檔案路徑與 URL 路徑 {#file-paths-and-url-paths}

在整個文件中，我們始終力求明確區分檔案路徑與 URL 路徑。內容插件通常會將檔案路徑直接映射到 URL 路徑，例如 `./docs/advanced/routing.md` 會轉換為 `/docs/advanced/routing`。但透過 `slug`，您可以讓 URL 完全脫離檔案結構。
...
以下目錄結構可幫助您直觀理解這種檔案→URL映射。假設所有頁面均未自訂 slug。
...
關於內容插件的說明至此。讓我們退一步，談談 Docusaurus 應用中的路由運作原理。

## 路由轉為 HTML 檔案 {#routes-become-html-files}

由於 Docusaurus 是伺服器端渲染框架，所有生成的路由都會被伺服器端渲染成靜態 HTML 檔案。若您熟悉 [Apache2](https://httpd.apache.org/docs/trunk/getting-started.html) 等 HTTP 伺服器的行為，就會理解其運作方式：當瀏覽器向路由 `/docs/advanced/routing` 發送請求時，伺服器會將其解釋為請求 HTML 檔案 `/docs/advanced/routing/index.html` 並返回該檔案。

路由 `/docs/advanced/routing` 可能對應 `/docs/advanced/routing/index.html` 或 `/docs/advanced/routing.html`。部分託管供應商會根據結尾斜線的存在與否進行區分，且可能不兼容另一種形式。詳見 [結尾斜線指南](https://github.com/slorber/trailing-slash-guide)。
...
所有 HTML 檔案都會使用絕對 URL 參照其 JS 資源，因此為了正確定位資源，您必須配置 `baseUrl` 欄位。注意 `baseUrl` 不影響輸出套件的檔案結構：基礎 URL 位於 Docusaurus 路由系統的上層。您可以將 `url` 和 `baseUrl` 的組合視為 Docusaurus 站點的實際位置。
...
Docusaurus 建構的是[單頁應用程式](https://developer.mozilla.org/en-US/docs/Glossary/SPA)，路由轉換透過 React Router 的 `history.push()` 方法在客戶端完成。此操作前提是目標網址必須已被路由器註冊，否則路由器會攔截路徑並顯示 404 頁面。

若將 HTML 檔案置於 `static` 資料夾，它們會被複製到建置輸出中而成為網站可存取的部分，但不會納入 Docusaurus 路由系統。我們提供 `pathname://` 協定，讓你能以非 SPA 方式跳轉至網域內其他路徑，效果如同外部連結。
...
`pathname://` 協定特別適合參照 static 資料夾內的內容。例如 Docusaurus 會[將所有 Markdown 靜態資源轉為 require() 呼叫](../guides/markdown-features/markdown-features-assets.mdx#static-assets)，使用此協定可保持原始連結形式，避免被 Webpack 雜湊處理。
...
Docusaurus 僅會移除 `pathname://` 前綴，不會進一步處理連結內容。