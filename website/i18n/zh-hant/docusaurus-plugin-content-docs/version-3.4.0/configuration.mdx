...
Docusaurus 對於配置有獨特的見解。我們鼓勵您將網站相關信息集中到一個地方。我們會保護此文件的字段，並促進讓這個數據對象在整個網站中可訪問。

維護良好的 `docusaurus.config.js` 可以幫助您、您的協作者以及開源貢獻者專注於文檔，同時仍然能夠自定義網站。
...
如需進一步幫助配置主題、插件和預設集，請參閱[使用插件](./using-plugins.mdx)。
...
## 從組件中訪問配置 {#accessing-configuration-from-components}

您的配置物件將對網站的所有組件可用。您可以通過 React 上下文以 `siteConfig` 的形式訪問它們。
...
對於新的 Docusaurus 項目，我們會在項目根目錄中自動生成一個 `babel.config.js`。
...
大多數情況下，此配置都能正常工作。如果您想自訂 Babel 配置（例如添加對 Flow 的支援），可以直接編輯此文件。要使更改生效，您需要重新啟動 Docusaurus 開發伺服器。