...
此插件在開發環境始終處於非活躍狀態，**僅在生產環境生效**，因為它需要基於構建輸出工作。
...
本插件會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，當多個文件將被生成至相同路徑時調整其日誌級別。
...
因此同一個"目標"可對應多個"來源"（生成多個重定向至相同目標的HTML文件），但一個"來源"不能對應多個"目標"（生成的HTML文件必須有確定的重定向目標）。
