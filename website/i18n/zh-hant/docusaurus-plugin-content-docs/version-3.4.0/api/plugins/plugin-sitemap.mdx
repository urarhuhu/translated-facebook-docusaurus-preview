...
此插件在開發環境中始終處於非活動狀態，**僅在生產環境生效**（執行 `docusaurus build` 時），因為它僅作用於建置輸出結果。
...
- [`noIndex`](../docusaurus.config.js.mdx#noIndex)：若啟用將不會生成站點地圖
...
所有官方內容插件都會為基於內容文件（Markdown、MDX 或 React 頁面組件）的路由提供元數據，但第三方插件作者可能未提供此資訊，此時插件將無法為其路由輸出 `<lastmod>` 標籤。
