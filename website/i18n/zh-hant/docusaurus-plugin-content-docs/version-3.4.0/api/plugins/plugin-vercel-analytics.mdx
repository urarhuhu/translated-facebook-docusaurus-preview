...
[Vercel Analytics](https://vercel.com/docs/analytics) 提供全面的網站訪客洞察，可追蹤熱門頁面、流量來源及地理位置、作業系統與瀏覽器等人口統計資料。

:::warning[僅限生產環境]

此插件在開發環境中始終處於非活躍狀態，**僅在生產環境** (`docusaurus build`) 啟用，以避免污染分析統計數據。
