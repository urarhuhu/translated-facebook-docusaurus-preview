...
經典主題預設支援淺色與深色模式，並提供導覽列切換功能供使用者選擇。
...
可設定預設圖片用於元標籤，特別是 `og:image` 和 `twitter:image`。
...
有時您需要在網站發布公告。為此，可添加公告欄功能。這是位於導覽列上方、非固定且可選擇關閉的面板。所有配置皆位於 `announcementBar` 物件中。
...
標誌可放置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根目錄。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，這在將文檔網站託管於主站子目錄時特別有用（此時可能不需讓標誌連結至主站時開啟新分頁）。
...
React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下您可以使用 `activeBasePath`。若某連結需在多個不同路徑下保持活動狀態（例如當您在同一側邊欄下有多個文件夾時），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向指定 `docId` 文件的連結。只要您瀏覽同一側邊欄下的文件，該連結就會獲得 `navbar__link--active` 類別。
...
使用者可在同一文件（只要文件 ID 在各版本間保持不變）切換不同版本。
...
若啟用[搜尋功能](../../search.mdx)，預設情況下搜尋框會出現在導覽列最右側。

但透過此特殊項目類型，您可以變更其預設位置。
...
可啟用此精緻的UI功能：當使用者向下滾動頁面時自動隱藏導覽列，向上滾動時再次顯示。
...
Docusaurus採用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)進行程式碼高亮顯示，所有設定皆位於 `prism` 物件中。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景顏色。請參閱[行標記說明文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)以獲得指引。
...
可透過`themeConfig.footer`在頁尾添加商標與版權聲明。商標圖檔可置於[靜態資源目錄](static-assets.mdx)，其URL設定方式與導覽列商標相同。
...
可透過`themeConfig.footer.links`在頁尾添加連結。頁尾配置分為兩種型態：**多欄式頁尾**與**簡易式頁尾**。
...
用於存取色彩模式的React鉤子。此上下文包含設定淺色/深色模式的函式，並暴露當前使用模式的布林值變數。
