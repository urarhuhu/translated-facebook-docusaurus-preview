...
- 日誌記錄函數。所有日誌記錄函數既可以作為普通函數使用（類似於 `console.log` 系列，但只接受一個參數），也可以作為模板字面量標籤使用。
  - `info`：打印信息。
  - `warn`：打印需要注意的警告。
  - `error`：打印錯誤（不一定是停止程序）表示嚴重問題。
  - `success`：打印成功訊息。
...
此外，`warn` 和 `error` 會為整個訊息著色以更好地引起注意。如果您要打印關於錯誤的大段幫助文本，最好使用 `logger.info`。
