...
[ESLint](https://eslint.org/) 是一個靜態分析程式碼的工具，能透過編輯器提示或命令列回報問題或建議最佳實踐。Docusaurus 提供 ESLint 插件來強制執行 Docusaurus 的最佳實踐。
...
這會啟用 `@docusaurus` eslint 插件並使用 `recommended` 配置。參閱下方的[支援規則](#supported-rules)以查看此配置會啟用的規則清單。

### 手動配置

若需要更細緻的控制，你也可以手動啟用插件並直接配置想使用的規則：
...
- 全部規則：啟用**所有**規則。此配置會在次要版本間變動，若想避免意外的破壞性變更，不應使用此配置。
