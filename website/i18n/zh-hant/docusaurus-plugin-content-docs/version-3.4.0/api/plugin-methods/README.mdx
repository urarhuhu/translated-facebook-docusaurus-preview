...
`context`與插件無關，同一個物件會被傳遞給用於Docusaurus網站的所有插件。`context`物件包含以下欄位：
...
`options`是[使用插件時的第二個可選參數](../../using-plugins.mdx#configuring-plugins)。`options`是插件特定的，由使用者在`docusaurus.config.js`中指定。如果導出了[`validateOptions`](./static-methods.mdx#validateOptions)函式，`options`會事先被驗證和標準化。

或者，如果預設配置包含該插件，則預設配置將負責將正確的選項傳遞給插件。具體選項由各個插件自行定義。
