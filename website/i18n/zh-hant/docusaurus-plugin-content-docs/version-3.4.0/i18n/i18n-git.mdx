...
- **維護困難**：必須保持翻譯檔案與未翻譯檔案**同步**

:::note

部分**大型技術專案**（如 React、Vue.js、MDN、TypeScript、Nuxt.js 等）使用 Git 進行翻譯。

參閱 [Docusaurus 國際化 RFC](https://github.com/facebook/docusaurus/issues/3317) 了解我們研究這些系統的筆記與連結。
...
使用 `--messagePrefix '(fr) '` 選項讓未翻譯字串突出顯示。

`Hello` 會顯示為 `(fr) Hello`，明確標示需要翻譯。
...
保持翻譯檔案與原始檔案**同步可能具有挑戰性**，特別是 Markdown 文件。

### Markdown 翻譯 {#markdown-translations}

當未翻譯的 Markdown 文件被編輯時，**維護相應的翻譯檔案是您的責任**，我們目前沒有好的方法協助此流程。

為保持翻譯站點的一致性，當 `website/docs/doc1.md` 文件被修改時，您需要**將這些修改反向移植**到 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。

### JSON 翻譯維護 {#json-translations}

為協助維護 JSON 翻譯文件，您可以再次執行 [write-translations](../cli.mdx#docusaurus-write-translations-sitedir) CLI 命令：
...
若您的翻譯存儲在 Git 中，可使用文檔和部落格插件的 `editLocalizedFiles: true` 選項。

編輯按鈕將轉而連結至本地化文件 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。