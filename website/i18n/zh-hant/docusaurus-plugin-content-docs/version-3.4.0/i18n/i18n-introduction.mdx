...
- **靈活的翻譯流程**：可使用 Git（單一倉庫、分支或子模組）、SaaS 軟體、FTP
...
為簡易Docusaurus網站新增法文翻譯後，目錄結構範例如下：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir)指令初始化。各插件會在其對應資料夾下提供翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤。

各內容插件或主題的**翻譯檔案存放位置定義不盡相同**：
