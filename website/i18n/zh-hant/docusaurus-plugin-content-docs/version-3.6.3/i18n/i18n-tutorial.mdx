...
新增一個類型為 `localeDropdown` 的 **導航欄項目**，讓使用者可以選擇所需的語言環境：
...
複製檔案後，請使用 `npm run start -- --locale fr` 重新啟動您的網站。在編輯現有檔案時，熱重載會更順暢。
...
找出 React 程式碼中所有對使用者可見的文字標籤，並使用翻譯 API 標記它們。共有兩種類型的 API：
...
JSX 元素是_物件_而非字串。在預期字串的上下文中使用（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會[強制轉換為字串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，導致返回 `"[object Object]"`。雖然我們鼓勵將 `<Translate>` 作為 JSX 子元素使用，但僅在實際有效的情況下才使用元素形式。
...
多數文檔網站本質上是**靜態的**，不需要進階的 i18n 功能（**複數形式**、**性別區分**等）。對於更複雜的用例，請使用 [react-intl](https://www.npmjs.com/package/react-intl) 這類函式庫。

:::

`docusaurus write-translations` 指令會靜態分析您網站中使用的所有 React 程式碼文件，提取這些 API 的調用，並將其聚合到 `code.json` 文件中。翻譯文件將以從 ID 到翻譯訊息物件（包括翻譯後的標籤和標籤描述）的映射形式儲存。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定預設的未翻譯訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯起來。
...
這在運行時仍能正確運作。然而，未來我們可能會提供「無運行時」機制，透過 Babel 轉換將翻譯直接內嵌到 React 程式碼中，而非在運行時調用 API。因此，為確保未來相容性，您應始終偏好靜態可分析訊息。例如，我們可以將上述程式碼重構為：
...
每種語言都有一組[可能的複數類別](https://unicode-org.github.io/cldr-staging/charts/37/supplemental/language_plural_rules.html)。Docusaurus 會按照 `["zero", "one", "two", "few", "many", "other"]` 的順序排列這些類別。例如，由於英語 (`en`) 有兩種複數形式（"one" 和 "other"），翻譯訊息會用管道符 (`|`) 分隔兩個標籤。而波蘭語 (`pl`) 有三種複數形式（"one"、"few" 和 "many"），則需按順序提供三個以管道符連接的標籤。
...
預設情況下，Markdown 標題 `### Hello World` 會自動生成 ID `hello-world`。其他文件可以用 `[連結](#hello-world)` 來連結它。然而，翻譯後標題變成 `### Bonjour le Monde`，ID 也會變成 `bonjour-le-monde`。
...
此策略 **無法** 在 GitHub Pages 上實現，因為它 **僅允許單一部署**。
...
也可以將每個語言版本部署為獨立的子網域，然後在 CDN 層級將子網域組合成統一的網域：
