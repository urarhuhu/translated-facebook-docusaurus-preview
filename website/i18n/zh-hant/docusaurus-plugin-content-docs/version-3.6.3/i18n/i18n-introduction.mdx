...
- **模組化**：允許插件作者提供國際化支援
- **低開銷運行**：文件主要是靜態的，不需要繁重的 JS 函式庫或填充程式
...
部分插件會從外部資料檔案讀取內容，這些檔案需整體本地化。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可通過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑下創建副本進行翻譯。

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需存放在正確的檔案系統路徑下。

每個語言區域和插件都有專屬的`i18n`子目錄：
...
為簡單的法語版Docusaurus網站進行翻譯後，目錄結構如下：
...
JSON檔案可通過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI命令初始化。各插件在對應目錄下管理自身翻譯內容，而`code.json`檔案則定義React代碼中使用的所有文字標籤。
