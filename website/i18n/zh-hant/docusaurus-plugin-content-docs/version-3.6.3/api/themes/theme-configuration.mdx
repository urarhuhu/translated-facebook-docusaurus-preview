...
有時您需要在網站發布公告。為此可添加公告欄，這是位於導覽列上方非固定且可選擇性關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可放置於[靜態資料夾](static-assets.mdx)。標誌URL預設設為網站基礎URL。雖然可自訂標誌URL，但若為外部連結將在新分頁開啟。此外可覆寫標誌連結的target屬性值，當文檔網站託管於主網站子目錄時特別有用，此情況下可能不需讓標誌連結至主網站時開啟新分頁。
...
React Router 應自動為連結應用活動連結樣式，但在邊緣情況下可以使用 `activeBasePath`。對於連結應在多個不同路徑上保持活動狀態的情況（例如當您在同一側邊欄下有多個文檔文件夾時），可以使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案，並且優先於它——Docusaurus 將其解析為針對當前 URL 進行測試的正則表達式。
...
除了上述欄位外，您還可以指定其他任意屬性，這些屬性可以應用於 HTML 連結。
...
如果您想連結到特定文檔，此特殊導航欄項目類型將渲染指向提供的 `docId` 的文檔連結。只要您瀏覽同一側邊欄的文檔，它就會獲取 `navbar__link--active` 類。
...
若您使用版本化文檔功能，此特殊導航欄項目類型將渲染一個下拉選單，顯示您網站所有可用版本。
...
若您使用版本化文檔功能，此特殊導航欄項目類型會連結至當前瀏覽的文檔版本（根據URL判斷），並自動回退至最新版本。
...
若您使用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導航欄項目類型會渲染一個下拉選單，顯示您網站所有可用語言選項。
...
若您使用[搜尋功能](../../search.mdx)，搜尋框預設會出現在導航欄最右側。
...
如果您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景顏色。請參考[文件指南](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
多欄頁尾連結每個欄位都有 `title` 和一組 `FooterItem` 清單。
...
一個用於存取顏色情境的 React 鉤子。此情境包含設定淺色和深色模式的函式，並公開表示當前使用模式的布林變數。
