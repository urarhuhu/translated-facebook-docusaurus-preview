...
`options`是[使用插件時的第二個可選參數](../../using-plugins.mdx#configuring-plugins)。`options`是插件專用的，由使用者在`docusaurus.config.js`中指定。如果導出了[`validateOptions`](./static-methods.mdx#validateOptions)函式，`options`會事先被驗證和標準化。
