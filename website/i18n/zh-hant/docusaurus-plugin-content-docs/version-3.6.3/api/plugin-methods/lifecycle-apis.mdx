...
插件應使用此生命週期從數據源（文件系統、遠程 API、無頭 CMS 等）獲取數據或進行一些伺服器端處理。返回值是插件所需的內容。
...
在 `contentLoaded` 中會使用 `loadContent` 載入的數據。這些數據可以被渲染為路由、註冊為全局數據等。
...
一個宣告式回調，用於創建靜態數據（通常是 JSON 或字符串），這些數據稍後可以作為 props 提供給你的路由。接收文件名和要存儲的數據，並返回實際數據文件的路徑。
...
全局數據是...全局的：其大小會影響網站所有頁面的加載時間，因此請盡量保持其體積小巧。盡可能優先使用 `createData` 和頁面特定數據。
...
`configureWebpack` 的 API 將來會修改為接收一個對象 (`configureWebpack({config, isServer, utils, content})`)
...
`configureWebpack` 會在伺服器建置和客戶端建置時被呼叫。伺服器建置會收到 `true`，而客戶端建置會收到 `false` 作為 `isServer` 參數。
...
可以指定合併策略。例如，如果你希望 webpack 規則被前置而不是後置：
...
例如，要讓你的主題載入用戶傳入的 `options` 中的 `customCss` 或 `customJs` 檔案路徑：
