...
不過您通常不需要逐一手動安裝常見插件：它們可以透過[預設集](#using-presets)以套件形式分發。對多數使用者而言，插件是透過預設集配置來設定的。

我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務...請務必查閱該清單：可能已有人為您實現了該功能！
...
不過，插件可以透過在配置中使用「Babel 風格」的雙成員元組（包含插件名稱與選項物件）來指定選項。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目可以互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可以覆寫插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[簡寫解析規則](#module-shorthands)，因此若想利用簡寫功能，請務必使用正確的條目！
...
經典預設集預設隨附於透過 [`create-docusaurus`](./installation.mdx#scaffold-project-website) 新建的 Docusaurus 網站中。它包含以下主題與插件：
...
接著，將其新增至您網站的 `docusaurus.config.js` 中的 `presets` 選項：
...
這在部分插件與主題需搭配使用時特別有用。您甚至可以將它們的選項連結在一起，例如將一個選項傳遞給多個插件。
...
若名稱具有作用域（以 `@` 開頭），則會先以第一個斜線將名稱分割為作用域與套件名稱：
...
若名稱中不含套件名（例如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（即 `@jquery/docusaurus-plugin`）。否則，系統會嘗試以下路徑：

- `[scope]/[name]`（例如 `@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（例如 `@jquery/docusaurus-plugin-content-docs`）

以下是以註冊在 `plugins` 欄位中的插件為例的說明。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求插件遵循統一命名規範不同，Docusaurus 允許更自由的命名方式，因此解析結果並非絕對，而是依照上述優先級進行匹配。
