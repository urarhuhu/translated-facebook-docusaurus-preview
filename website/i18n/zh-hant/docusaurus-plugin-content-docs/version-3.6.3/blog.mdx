...
部落格側邊欄會顯示最近的部落格文章。預設顯示的項目數量為 5，但您可以透過插件配置中的 `blogSidebarCount` 選項自訂。設定 `blogSidebarCount: 0` 將完全停用側邊欄，並移除容器，這會增加主容器的寬度。特別地，如果設定 `blogSidebarCount: 'ALL'`，則會顯示 _所有_ 文章。

您還可以使用 `blogSidebarTitle` 選項更改側邊欄的標題文字。例如，如果設定 `blogSidebarCount: 'ALL'`，您可以將預設的「最近文章」改為「所有文章」：
...
此命名約定為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若您需要更精確的發布時間，可使用 front matter。例如，若您有多篇同一天發布的文章，可依照當日時間排序：
...
使用 front matter 中的 `authors` 欄位來宣告部落格文章作者。作者至少需具備 `name` 或 `image_url`。Docusaurus 會使用 `url`、`email` 和 `title` 等資訊，但其他任何資訊皆可保留。
...
對於固定發布文章的部落格作者來說，在每篇文章中維護作者資訊可能相當繁瑣。
...
無論是透過 front matter 或作者對映宣告的作者，都需具備名稱或頭像（或兩者皆有）。若某篇文章的所有作者皆未提供名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。
...
作者頁面功能為可選項目，主要適用於多人協作的部落格。

您可透過在[全域作者設定](#global-authors)中添加`page: true`屬性，為每位作者獨立啟用此功能：
...
- 每位作者的專屬頁面（[範例](/blog/authors/slorber)），列出其貢獻的所有文章
- 作者索引頁（[範例](/blog/authors)），按`authors.yml`中的順序列出所有作者

:::warning[關於行內作者]

僅[全域作者](#global-authors)可使用此功能，[行內作者](#inline-authors)不支援。

:::

## 文章標籤 {#blog-post-tags}

標籤需在front matter中宣告，提供另一種文章分類維度。

可選擇直接定義行內標籤，或引用預先定義於[`標籤文件`](api/plugins/plugin-content-blog.mdx#tags-file)（通常為`blog/tags.yml`）的標籤。

以下範例中：

- `docusaurus`引用自`blog/tags.yml`中預定義的標籤鍵值
- `Releases`為行內標籤，因其未在`blog/tags.yml`中定義
...
Docusaurus會根據字數自動估算每篇文章的閱讀時間，並提供自訂選項。
...
`readingTime`回調函數接收三個參數：文章內容文字字串、front matter鍵值對紀錄，以及預設閱讀時間計算函數。可返回數字（閱讀分鐘數）或`undefined`（停用該頁面閱讀時間顯示）。

預設閱讀時間計算函數支援額外選項：`wordsPerMinute`數值（預設300）與`wordBound`字串判斷函數（預設識別空格、製表符和換行符為單詞邊界）。
...
您可以讓部落格文章列表頁面作為首頁，而不使用獨立的首頁。將 `routeBasePath` 設為 `'/'`，即可透過根路由 `example.com/` 提供部落格內容，而非子路由 `example.com/blog/`。
...
請務必刪除現有的首頁檔案 `./src/pages/index.js`，否則會出現兩個檔案對應相同路由的情況！

:::

:::warning

若停用文件插件，請記得刪除設定檔中其他參照文件插件的地方。特別注意需移除導航欄中與文件相關的項目。

:::

:::tip

另有「純文件模式」供僅需使用文件功能的用戶。詳見[純文件模式](./guides/docs/docs-introduction.mdx)的詳細說明或更深入的 `routeBasePath` 解釋。

:::

### 多重部落格 {#multiple-blogs}

預設情況下，經典主題假設每個網站只有一個部落格，因此僅包含一個部落格插件實例。若需在單一網站上建立多個部落格，您可以在 `docusaurus.config.js` 的 `plugins` 選項中指定另一個部落格插件來實現。
