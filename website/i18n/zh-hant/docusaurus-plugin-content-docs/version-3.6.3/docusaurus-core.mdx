...
用它來包裹可能拋出錯誤的組件，當錯誤發生時顯示回退內容，而不是讓整個應用崩潰。
...
Docusaurus 使用此組件來捕獲主題佈局內的錯誤，以及整個應用內的錯誤。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能特性稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經加載完成。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後在用戶可能導航到該資源時通過 `onMouseOver` 事件觸發高優先級請求。
...
翻譯字串將透過 [`docusaurus write-translations`](./cli.mdx#docusaurus-write-translations-sitedir) CLI 從您的程式碼中靜態提取，並在 `website/i18n/[locale]` 中建立 `code.json` 翻譯檔案。
...
`<Translate>` 元件支援插值。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實現[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
`siteConfig` 物件僅包含**可序列化的值**（即經過 `JSON.stringify()` 後仍保留的值）。函數、正則表達式等內容在客戶端會遺失。

:::

### `useIsBrowser` {#useIsBrowser}

當 React 應用在瀏覽器中完成水合作用後返回 `true`。
...
React Hook 用於存取 Docusaurus 的壞鏈結檢查 API，提供方式讓 Docusaurus 頁面回報並收集其連結與錨點。
