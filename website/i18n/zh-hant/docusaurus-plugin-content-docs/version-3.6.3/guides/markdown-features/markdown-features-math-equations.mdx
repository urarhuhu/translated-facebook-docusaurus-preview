...
2.  這兩個插件 [**僅提供 ES 模組版本**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)。建議使用 [**ES 模組**](https://flaviocopes.com/es-modules/) 設定檔：
    ...
    <summary>
      ...
      設定檔？
            ...
            若決定使用 CommonJS 設定檔，可透過動態導入和異步配置創建函數載入這些 ES 模組插件：
...
從CDN來源載入樣式表、字體和JavaScript函式庫是針對熱門函式庫與資源的良好實踐，因為這能減少您需要自行託管的資源量。若您偏好自行託管`katex.min.css`（連同必要的KaTeX字體），可從[KaTeX GitHub發佈頁](https://github.com/KaTeX/KaTeX/releases)下載最新版本，解壓後將`katex.min.css`與`fonts`目錄（僅需包含`.woff2`字體類型）複製到網站的`static`目錄中，並在`docusaurus.config.js`裡將樣式表的`href`從CDN網址替換為本地路徑（例如`/katex/katex.min.css`）。
