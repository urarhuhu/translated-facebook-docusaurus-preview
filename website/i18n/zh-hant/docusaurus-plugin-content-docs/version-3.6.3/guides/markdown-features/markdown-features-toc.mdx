...
每份 Markdown 文件會在右上角顯示目錄。預設情況下，目錄僅顯示 h2 和 h3 級別的標題，這通常足以呈現頁面結構概覽。如需調整顯示的標題範圍，您可以針對單一頁面或全局自訂最小和最大標題層級。
...
`themeConfig` 選項會影響網站上所有目錄（包括[行內目錄](#inline-table-of-contents)），而 front matter 選項僅影響右上角的目錄。您需要使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性來個別自訂每個 `<TOCInline />` 元件。
...
MDX 文件中可透過 `toc` 變數獲取所有標題資訊。預設僅顯示 `h2` 和 `h3` 級別標題。透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 可調整顯示的標題層級範圍。
...
非 Markdown 標題不會顯示在目錄中。這可以用來解決上述問題。
