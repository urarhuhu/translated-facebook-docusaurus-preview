...
有兩種方式可以添加指向其他頁面的連結：通過 **URL 路徑** 和 **檔案路徑**。
...
如果你想引用 **由同一個插件處理** 的另一個 Markdown 檔案，可以使用目標文件的相對路徑。Docusaurus 的 Markdown 加載器會將檔案路徑轉換為目標檔案的 URL 路徑（因此會移除 `.md` 副檔名）。
...
絕對檔案路徑也可以相對於網站目錄。但要注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為如果你創建新的文檔版本或進行本地化時，需要手動更新這些連結。
...
- 連結在 GitHub 界面和許多 Markdown 編輯器中仍能正常運作
- 你可以自訂檔案的 slug 而無需更新所有連結
- 移動檔案時，你的編輯器可以追蹤這些變更，某些編輯器甚至會自動更新檔案連結
- [版本化文檔](../docs/versioning.mdx) 會連結到完全相同版本的另一個文檔
- 如果你更新 [`trailingSlash` 配置](../../api/docusaurus.config.js.mdx#trailingSlash)，相對 URL 連結很可能會失效

:::warning

Markdown 檔案引用僅在來源檔案和目標檔案由同一個插件實例處理時才能運作。這是我們 Markdown 處理架構的技術限制，未來將會修復。如果你需要在插件之間連結檔案（例如從部落格文章連結到文檔頁面），則必須使用 URL 連結。
