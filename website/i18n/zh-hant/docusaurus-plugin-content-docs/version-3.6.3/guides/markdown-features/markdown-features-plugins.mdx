...
如果您試用過 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯過程有兩個中間階段：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也分為兩種形式：
...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警告標語](./markdown-features-admonitions.mdx)語法也是透過 Remark 插件生成，您可依相同邏輯實現自訂需求。
...
MDX 插件通常是 npm 套件，因此您可透過 npm 安裝，方式與其他套件相同。以[數學公式插件](./markdown-features-math-equations.mdx)為例。
...
以下說明**並非**建立外掛程式的完整指南，僅為示範如何使其在 Docusaurus 中運作。如需更深入的運作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。

:::

舉例來說，我們建立一個會遍歷所有 `h2` 標題並添加 `Section X. ` 前綴的外掛程式。首先，在任何位置建立你的外掛程式原始檔——你甚至可以將其發佈為獨立的 npm 套件並如前述方式安裝。我們會將外掛程式放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是一個接收 `options` 並返回操作 AST 的 `transformer` 的函式。
...
`transformer` 的第二個參數 [`vfile`](https://github.com/vfile/vfile) 可用於存取當前 Markdown 檔案的路徑，非常實用。
...
Docusaurus 的預設外掛程式會在自訂 remark 外掛程式之前執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上面的範例中，即使所有 `h2` 標題現在都帶有 `Section X.` 前綴，生成的目錄仍保持不變，因為目錄生成外掛程式是在我們的自訂外掛程式之前呼叫的。若你需要在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`。
