...
若要在 MDX 檔案中定義自訂元件，您必須將其匯出：只有以 `export` 開頭的段落才會被解析為元件而非純文字。
...
由於所有文件皆使用 MDX 解析，任何看似 HTML 的內容實際都是 JSX。因此，若需要為元件添加行內樣式，請遵循 JSX 風格並提供樣式物件。
...
`@site` 別名指向您網站的根目錄（通常為 `docusaurus.config.js` 所在位置）。使用別名而非相對路徑（如 `'../../src/components/BrowserWindow'`）可避免因移動檔案或進行[文件版本控制](../docs/versioning.mdx)與[國際化翻譯](../../i18n/i18n-tutorial.mdx)時需更新匯入路徑。

:::

雖然在 Markdown 中宣告元件對於簡單案例非常方便，但由於編輯器支援有限、解析錯誤風險及低可重用性，難以維護複雜邏輯。當元件涉及複雜 JS 邏輯時，請使用獨立的 `.js` 檔案：
...
若您在多個檔案中重複使用相同元件，無需在每個檔案中重複匯入——可考慮將其加入全域作用域。[參見下文](#mdx-component-scope)

:::

### MDX 元件作用域 {#mdx-component-scope}

除了[匯入元件](#importing-components)與[匯出元件](#exporting-components)外，第三種在 MDX 中使用元件的方式是**將其註冊至全域作用域**，這將使其自動在所有 MDX 檔案中可用，無需任何匯入語句。
...
它將被編譯為包含 `ul`、`li`、`p` 和 `Highlight` 元素的 React 元件。`Highlight` 並非原生 HTML 元素：您需要為其提供自訂的 React 元件實作。
...
如果您[替換](../../swizzling.mdx)此元件，您將找到所有已實作的標籤，並可以通過替換相應的子元件（如 `@theme/MDXComponents/Code`，用於渲染[Markdown 代碼塊](./markdown-features-code-blocks.mdx)）來進一步自訂我們的實作。

如果您想註冊額外的標籤名稱（如上述的 `<Highlight>` 標籤），您應該考慮[包裝 `@theme/MDXComponents`](../../swizzling.mdx#wrapping)，這樣您就不需要維護所有現有的對應。由於替換 CLI 尚不允許包裝非元件檔案，您應該手動創建包裝器：
...
從 MDX v3+（Docusaurus v3+）開始，小寫標籤名稱始終會被渲染為原生 HTML 元素，並且不會使用您提供的任何元件對應。

:::

:::warning

此功能由[`MDXProvider`](https://mdxjs.com/docs/using-mdx/#mdx-provider)提供支持。如果您在 React 頁面中導入 Markdown，您必須通過 `MDXContent` 主題元件自行提供此提供者。
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax) 大部分與 [CommonMark](https://commonmark.org/) 兼容，但更加嚴格，因為您的 `.mdx` 檔案可以使用 JSX 並被編譯成真正的 React 元件（請查看[遊樂場](https://mdxjs.com/playground/)）。
