...
當瀏覽 `doc1` 或 `doc2` 時，會顯示 `tutorialSidebar`；瀏覽 `doc3` 或 `doc4` 時則會顯示 `apiSidebar`。
...
Docusaurus 如何決定瀏覽 `commonDoc` 時顯示哪個側邊欄？答案是：它無法判斷，我們也不保證會選擇哪個側邊欄。

當您將文件 Y 加入側邊欄 X 時，會建立雙向綁定：側邊欄 X 包含文件 Y 的連結，且瀏覽文件 Y 時會顯示側邊欄 X。但有時我們需要打破這種隱性綁定：

1. _如何在側邊欄 X 建立文件 Y 的連結，卻不讓側邊欄 X 在瀏覽 Y 時顯示？_ 例如當文件 Y 如同前述範例同時出現在多個側邊欄時，如何明確指定 Docusaurus 顯示特定側邊欄？
...
這會明確告知 Docusaurus 在瀏覽 `commonDoc` 時顯示 `apiSidebar`。透過相同方法，您可讓未包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
Docusaurus 使用側邊欄來產生文件頁面底部的「下一頁」與「上一頁」分頁連結。它嚴格遵循當前顯示的側邊欄：若無關聯側邊欄，則不會產生分頁導覽。但被連結為「下一頁」或「上一頁」的文件不保證會顯示相同側邊欄：它們雖被包含在此側邊欄中，但其 front matter 可能設定了不同的 `displayed_sidebar`。

若透過 `displayed_sidebar` front matter 顯示的側邊欄未包含該文件本身，則不會顯示分頁導覽。
...
當您希望從多個側邊欄連結到同一份文件時，這個類型特別有用。文件僅屬於一個側邊欄（即註冊為 `type: 'doc'` 或來自自動生成目錄的側邊欄），但其連結會出現在所有註冊了該文件的側邊欄中。
