...
本頁面說明如何將已翻譯的 Docusaurus v1 站點遷移至 Docusaurus v2。

## 國際化差異 {#i18n-differences}

Docusaurus v2 的國際化功能在概念上與 Docusaurus v1 相當類似，僅有少數差異。
...
Docusaurus v2 使用 [MDX](https://mdxjs.com/) 解析 Markdown 檔案。

MDX 會將 Markdown 檔案編譯為 React 元件，其規範比 Docusaurus v1 解析器更嚴格，當遇到錯誤時會直接導致建置失敗，而非渲染有問題的內容。

此外，HTML 元素必須替換為 JSX 元素。

這對國際化尤其重要，因為若 Crowdin 上的翻譯品質不佳且使用了無效標記，您的 v2 翻譯站點可能無法建置：您可能需要進行翻譯清理以修正錯誤。

## 遷移策略 {#migration-strategies}

本節將協助您瞭解如何**在遷移至 v2 後保留現有的 v1 翻譯內容**。

對於使用 Crowdin 的 Docusaurus v1 站點，存在**多種可能的遷移策略**，各有不同的權衡取捨。
...
- 先在不包含翻譯內容的情況下將 v1 Docusaurus 站點遷移至 v2
- 熟悉 [Docusaurus v2 的新國際化系統](../../i18n/i18n-introduction.mdx)
- 使用全新且未翻譯的 Crowdin 專案與 [Crowdin 教學](../../i18n/i18n-crowdin.mdx)，讓 Crowdin 適用於您的 v2 站點

:::danger

請勿在未充分理解 Crowdin 與 Docusaurus v2 國際化功能的情況下嘗試遷移。

:::

### 建立新的 Crowdin 專案 {#create-a-new-crowdin-project}

為避免**生產環境中的 v1 站點發生故障風險**，其中一種策略是複製原始的 v1 Crowdin 專案。
...
- 先嘗試「100%匹配」（比「Perfect」會翻譯更多內容），並預翻譯源文件
- 下載Crowdin翻譯至本地
- 嘗試運行/建置網站並檢查是否有錯誤

首次嘗試時很可能會遇到錯誤：預翻譯可能會嘗試翻譯不應翻譯的內容（front matter、admonition、代碼區塊等），且翻譯後的MD文件可能對MDX解析器無效。

您需要修復所有錯誤直到網站能成功建置。可以通過修改本地翻譯的MD文件來解決，並使用`docusaurus build --locale fr`一次修復一個語系的網站。

沒有終極指南能解決所有錯誤，但常見錯誤原因包括：
...
您可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並限制僅對部分語系/文件進行預翻譯。

:::tip

對有問題的Markdown元素使用[`mdx-code-block`](../../i18n/i18n-crowdin.mdx#mdx-solutions)：Crowdin較不容易弄亂代碼區塊內的內容。

:::

:::note

您可能會注意到舊專案中已翻譯的內容在新專案中卻未翻譯。

Crowdin的Markdown解析器會隨時間演進，且每個Crowdin專案的解析器版本可能不同，這可能導致預翻譯無法翻譯所有字串。

此解析器版本未公開，您需要聯繫Crowdin支援以確認專案的解析器版本並固定特定版本。
...
Crowdin有「上傳翻譯」功能，但根據我們的經驗，對Markdown文件效果不佳

:::

### 使用現有Crowdin專案 {#use-the-existing-crowdin-project}

若不介意修改現有Crowdin專案並承擔搞砸的風險，可以嘗試使用Crowdin的分支系統。
...
這樣就不需要創建新Crowdin專案、轉移翻譯記憶檔、應用預翻譯並嘗試修復預翻譯錯誤。

您可以為Docusaurus v2創建一個Crowdin分支，上傳v2源文件，並在準備完成後將分支合併至主線。

### 改用Git而非Crowdin {#use-git-instead-of-crowdin}

可以完全遷移出Crowdin，將翻譯文件加入Git版本控制。
