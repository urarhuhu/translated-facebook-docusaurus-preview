...
這能明確區分 Docusaurus 官方套件與社群維護套件。換言之，所有官方套件皆歸類於 `@docusaurus/` 命名空間下。
...
Docusaurus 1 的所有建置產出皆位於 `website/build/<PROJECT_NAME>`。

Docusaurus 2 則改為 `website/build`，請確認部署配置已更新至正確讀取建置目錄。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集（preset）為插件組合包，為保持向後相容性，我們提供 `@docusaurus/preset-classic` 預設集，包含 Docusaurus 1 的核心插件。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄需位於 `website` 內。將文檔放在網站目錄中也更為理想，因為這樣文檔和網站程式碼就能集中在同一個 `website` 目錄下。

如果您正在遷移 Docusaurus v1 網站，且有待處理的文檔拉取請求，可以暫時保留 `/docs` 資料夾在原位，以避免產生衝突。

請參閱下方遷移指南，了解 `siteConfig.js` 中每個欄位的變更。
...
您可以聯繫 DocSearch 團隊（@shortcuts, @s-pace）尋求支援。他們可以為您更新配置並觸發重新爬取網站以恢復搜索功能（否則您可能需要等待最多 24 小時以進行下一次排程爬取）。
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 文件，並填入您的自訂網域。`static` 資料夾中的文件會在執行建置命令時被複製到 `build` 資料夾的根目錄。
...
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中找到，且預設為開啟狀態。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
作為應急方案，您可以使用 [@docusaurus/plugin-client-redirects](../../api/plugins/plugin-client-redirects.mdx) 來創建從 `/installation.html` 到 `/installation` 的客戶端重新導向。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需更新失效語法。常見範例如 HTML 有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須是有效的 JSX。
...
部落格的 Docusaurus front matter 欄位已從 camelCase 改為 snake_case 以與文件保持一致。
...
部落格 RSS 訂閱位址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向以維持現有訂閱有效性。
